# FrogDB Commercial License

## Overview

A Commercial License is available for organizations that:

1. Want to offer FrogDB as a managed database service without AGPLv3 copyleft obligations
2. Want to build competing database products using FrogDB technology
3. Have policies that prevent use of BSL or AGPLv3 licenses
4. Require proprietary modifications without source disclosure
5. Need enterprise support, SLAs, or indemnification

## Use Cases Requiring a Commercial License

| Use Case                       | BSL          | AGPLv3        | Commercial |
| ------------------------------ | ------------ | ------------- | ---------- |
| Managed FrogDB service (DBaaS) | ❌           | ✅ (copyleft) | ✅         |
| Building a competing database  | ❌           | ✅ (copyleft) | ✅         |
| Proprietary modifications      | ❌           | ❌            | ✅         |
| OEM/embedded licensing         | Case-by-case | ❌            | ✅         |
| Enterprise indemnification     | ❌           | ❌            | ✅         |

## License Options

### 1. Managed Service License

For cloud providers and companies offering FrogDB as a managed service.

**Includes:**

- Right to offer FrogDB as a hosted/managed service
- No copyleft obligations
- Technical support options
- Custom SLA terms available

### 2. OEM / Embedded License

For companies embedding FrogDB in their products.

**Includes:**

- Right to distribute FrogDB as part of your product
- White-label / rebrand options
- No copyleft obligations
- Volume pricing available

### 3. Enterprise License

For organizations needing additional legal protections.

**Includes:**

- All rights under BSL Additional Use Grant
- IP indemnification
- Priority support
- Custom terms available

## Pricing

Commercial licensing is priced based on:

- Use case (managed service, OEM, enterprise)
- Scale (nodes, users, revenue)
- Support level required
- Contract term

Contact us for a quote tailored to your needs.

## Contact

**Email:** nathan.m.jordan@gmail.com

**Subject line:** FrogDB Commercial License Inquiry

Please include:

1. Your company name and website
2. Intended use case
3. Expected scale (approximate)
4. Any specific requirements or questions

We typically respond within 2 business days.

---

## Frequently Asked Questions

### Do I need a commercial license to use FrogDB in my SaaS product?

**No**, as long as FrogDB is just your data layer and you're not offering database functionality as the primary service. A web app, API service, or mobile backend that uses FrogDB for storage is permitted under the BSL.

### Can I try FrogDB before purchasing a commercial license?

**Yes.** You can use FrogDB under the BSL or AGPLv3 for development, testing, and evaluation. A commercial license is only required when you want to use FrogDB in ways not permitted by those licenses.

### What if I start with AGPLv3 and later want to switch to Commercial?

You can switch at any time. The commercial license would apply to your use going forward. Note that any AGPLv3-licensed code you've already distributed would remain under AGPLv3.

### Do commercial licenses include support?

Support options are available and can be bundled with commercial licenses. We offer email support, priority response times, and dedicated support contracts depending on your needs.

### Can I get a trial commercial license?

We can discuss evaluation terms for specific use cases. Contact us to discuss your situation.

---

Copyright © 2025 Nathan Jordan. All rights reserved.
