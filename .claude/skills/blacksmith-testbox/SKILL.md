...
## FrogDB: use the `just tb-*` recipes, not raw CLI lifecycle commands

This repo wraps the testbox lifecycle so boxes are always cleaned up:

    just tb-warmup                       # warmup test-unit-tests-testbox.yml, 5-min idle timeout,
                                         #   records the ID for automatic session-end shutdown
    just tb-run "just test frogdb-core"  # sync local changes + run on the recorded box
    just tb-status --wait                # block until hydrated
    just tb-stop                         # stop all boxes recorded for this worktree
    just tb-list                         # active boxes for the org

- Prefer `just tb-warmup` over calling `blacksmith testbox warmup` directly: the wrapper
  (scripts/testbox-warmup.sh) records the box ID in the worktree's git dir so a Claude Code
...
and boots the CI environment in the background while you work:
    ...
    blacksmith testbox warmup test-unit-tests-testbox.yml
    # → tbx_01jkz5b3t9...
...
1. Warm up immediately when you receive a coding task:
   `blacksmith testbox warmup test-unit-tests-testbox.yml` → save the ID
...
If you need a longer session, increase the timeout at warmup time:
    ...
    blacksmith testbox warmup test-unit-tests-testbox.yml --idle-timeout 60

## With options

    blacksmith testbox warmup test-unit-tests-testbox.yml --ref main
    blacksmith testbox warmup test-unit-tests-testbox.yml --idle-timeout 60
