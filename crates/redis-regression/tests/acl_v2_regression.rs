...
async fn acl_selectors_basic_multiple() {
    ...
    client.command(&["ACL", "SETUSER", "sel1", "on", "-@all", "resetkeys", "nopass"]).await;

    // Add selectors: write access to write::* and read access to read::*
    assert_ok(
        &client
            .command(&["ACL", "SETUSER", "sel1", "(+@write ~write::*)", "(+@read ~read::*)"])
            .await,
    );
...
async fn acl_clearselectors_removes_secondary_selectors() {
    ...
    // Add a selector
    client.command(&["ACL", "SETUSER", "sel-del", "on", "(~added-selector)"]).await;

    // Clear selectors
    assert_ok(&client.command(&["ACL", "SETUSER", "sel-del", "clearselectors"]).await);
    ...
    let selectors_pos = items.iter().step_by(2).position(|r| {
        matches!(r, frogdb_protocol::Response::Bulk(Some(k)) if k.as_ref() == b"selectors")
    });
    assert!(selectors_pos.is_some(), "selectors field not found in GETUSER response");
    let idx = selectors_pos.unwrap() * 2 + 1;
    let sels = unwrap_array(items[idx].clone());
    assert!(sels.is_empty(), "selectors should be empty after clearselectors");
}
...
async fn acl_read_key_permission() {
    ...
    client
        .command(&["ACL", "SETUSER", "reader", "on", "nopass", "%R~read:*", "+@all"])
        .await;
...
async fn acl_write_key_permission() {
    ...
    client
        .command(&["ACL", "SETUSER", "writer", "on", "nopass", "%W~write:*", "+@all"])
        .await;
...
async fn acl_getuser_returns_selectors_field() {
    ...
    client.command(&["ACL", "SETUSER", "getuser-test", "on", ">pass"]).await;
    ...
    assert!(keys.iter().any(|k| k == "selectors"), "GETUSER should include selectors field");
}
