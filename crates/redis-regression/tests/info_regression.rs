...
async fn info_server_section_filtering() {
    ...
    assert!(text.contains("redis_version") || text.contains("frogdb_version"),
...
async fn info_all_returns_all_sections() {
    ...
    assert!(text.contains("connected_clients") || text.contains("used_memory"),
...
async fn info_keyspace_shows_db_stats_after_writes() {
    ...
    assert!(!matches!(client.command(&["INFO", "keyspace"]).await, frogdb_protocol::Response::Error(_)));
    let _ = text;
}
