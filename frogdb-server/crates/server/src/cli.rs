...
pub struct Cli {
    ...
    /// Adopt a data directory that has no valid FrogDB marker: stamp one and
    /// continue instead of refusing to start. Deletes nothing — existing data
    /// is recovered normally. Use for the first boot of a directory FrogDB did
    /// not create (one written before markers existed, or restored by hand),
    /// and for a first boot under `persistence.require-existing-data`. It is a
    /// flag rather than a config key on purpose: a persisted override would
    /// disable the wrong-directory guard forever.
    #[arg(long)]
