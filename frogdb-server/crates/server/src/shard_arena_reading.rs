...
//! # Where the tests are
//!
//! The absent-not-zero cases are unit tests below. The two end-to-end cases —
//! a real shard-shaped thread, a real arena, real bytes — live in
//! `tests/arena_reading.rs`, which declares jemalloc as its own global
//! allocator because that is what makes those bytes reach an arena at all.
...
mod tests {
    ...
    /// A shard missing from the registry — every shard under simulation, and
    /// every shard on a build whose allocator has no arenas — reports what it
    /// reported before this wiring existed.
    #[test]
