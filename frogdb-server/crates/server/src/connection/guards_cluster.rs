...
impl ConnectionHandler {
    pub(crate) fn is_cluster_exempt(&self, cmd_name: &str) -> bool {
        ...
        self.core.registry.get_entry(cmd_name).is_some_and(|entry| matches!(entry.execution_strategy(), ExecutionStrategy::ConnectionLevel(_) | ExecutionStrategy::ScatterGather { .. } | ExecutionStrategy::ServerWide(_)))
    }
    ...
    pub(crate) fn validate_cluster_slots(&mut self, cmd: &ParsedCommand) -> Option<Response> {
        ...
        let keys = if let Some(cmd_impl) = self.core.registry.get(&cmd_name) { cmd_impl.keys(&cmd.args) } else { return None; };
        ...
        match snapshot.slot_assignment.get(&first_slot) {
            ...
            Some(&owner) => {
                ...
                if self.state.readonly { let is_read_cmd = self.core.registry.get(&cmd_name).is_some_and(|c| c.flags().contains(CommandFlags::READONLY)); if is_read_cmd { return None; } }
    ...
    pub(crate) async fn check_migrating_multikey(&self, cmd: &ParsedCommand) -> Option<Response> {
        ...
        if self.core.shard_senders[shard_id].send(msg).await.is_err() { return Some(Response::error("ERR shard unavailable")); }
