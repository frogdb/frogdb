...
pub fn config_param_registry() -> &'static [ConfigParamInfo] {
    ...
    static REGISTRY: LazyLock<Vec<ConfigParamInfo>> = LazyLock::new(|| {
        ...
        // last so the golden snapshot's first 118 rows stay byte-identical.
...
mod tests {
    ...
    fn test_golden_snapshot_row_count() {
        ...
        // (`hotshards-enabled`), giving 118. The persistence hardening round
        // appended 2 (`stop-writes-on-save-error` mutable,
        // `recovery-on-decode-failure` immutable), giving 120.
        assert_eq!(GOLDEN_SNAPSHOT.len(), 120);
    }
