...
//!   - `handoff_seq`, `epoch`, `node_epoch`                       -> top-level fields
//!
...
//! `#[ignore = "pending issue NN — ..."]`-annotated citing the issue file. Tests that
//! stay in the default suite are exactly the ones whose traces do not exercise a
//! divergent path (confirmed empirically, not merely by inspection).
//!
//! ## NEW finding (unfiled): removed-node ghost fields
//!
//! `remove_node_after_demotion_succeeds_test` and
//! `remove_node_force_evicts_live_owner_test` were expected to match the model
//! exactly (no filed issue touches plain `removeNode` postconditions) but both
//! diverge and are `#[ignore]`d citing this paragraph rather than an issue number.
//! The model's `nodes` var is a *total* map over the fixed `NODES` set — `removeNode`
//! flips `member: false` but leaves `role`/`parent` at their last live value, so a
//! removed node's record survives as a "ghost". `ClusterState`'s real `RemoveNode`
//! arm (`commands.rs:215-236`) does `inner.nodes.remove(&node_id)` — true deletion,
//! matching `specs/cluster.md`'s own TR-CLUSTER-003 postcondition ("Node membership"
//! loses `node_id`) — so there is no live data anywhere to recover a removed node's
//! last role/parent from; the driver's projection necessarily defaults them. This
//! reads as a model-representation artifact (the model's *own* target spec confirms
//! real deletion; nothing suggests the ghost fields encode an intentional
//! requirement) rather than a code defect, but per the divergence protocol it is
//! reported, not silently resolved — see `task-3-report.md` for the full traces and
//! disposition options (most likely fix: the model nulls `role`/`parent` in the same
...
struct ClusterDriver {
    ...
    clock_ms: u64,
    // The model's `handoff_seq` var is a global counter bumped by every
    // `prepareHandoff`. `SlotMigration`'s per-slot handoff seq (from
    // `get_slot_migration`) mirrors it exactly in every named test below (each
    // touches a single slot), so the driver just tracks the model's counter
    // directly to project it independent of any single migration record.
    handoff_seq: i64,
}
...
impl ClusterDriver {
    ...
    fn prepare_handoff(&mut self, s: i64) {
        ...
        let now = self.tick();
        let result = self.state.apply_local(ClusterCommand::PrepareSlotHandoff {
            slot: s as u16,
...
impl State<ClusterDriver> for ClusterProjection {
    fn from_driver(driver: &ClusterDriver) -> Result<Self> {
        ...
        Ok(ClusterProjection {
            ...
            slots,
            migrations,
            handoff_seq: driver.handoff_seq,
...
impl State<ScriptedDriver> for ClusterProjection {
...
#[ignore = "pending issue 15 — graceful (force:false) Failover never prunes migrations sourced at the old primary; see .scratch/cluster-correctness/issues/open/15-graceful-failover-leaves-migrations-sourced-at-the-old-primary.md"]
...
#[ignore = "pending issue 26 — code has no drain/offset-parity barrier on the graceful failover path, so nothing refuses this call; see .scratch/cluster-correctness/issues/open/26-planned-failover-gets-a-drain-and-offset-parity-barrier.md"]
...
#[ignore = "pending issue 15 — graceful Failover never prunes migrations sourced at the old primary; see .scratch/cluster-correctness/issues/open/15-graceful-failover-leaves-migrations-sourced-at-the-old-primary.md"]
...
#[ignore = "pending issue 15 — no repatriation phase exists in code: AbortSlotHandoff has no residue/`repatriating` bookkeeping, and there is no CompleteRepatriation command; see .scratch/cluster-correctness/issues/open/15-graceful-failover-leaves-migrations-sourced-at-the-old-primary.md"]
...
fn failover_refuses_stale_fence_test() -> ScriptedDriver {
    ScriptedDriver::new(vec![
        step_("init", |d| d.init()),
        step_("failoverForced", |d| d.failover(1, 3, true)),
        step_("failoverForced", |d| d.failover(3, 1, true)),
    ])
}
...
#[ignore = "pending issue 17 — no FeedBuffer-shaped ClusterCommand exists at all (the byte-cap replication feed hold is unimplemented); see .scratch/cluster-correctness/issues/open/17-stale-source-outlives-its-write-barrier.md"]
...
#[ignore = "pending issue 20 — code's RemoveNode has no still-owns-slots guard at all and always succeeds; see .scratch/cluster-correctness/issues/open/20-force-failover-evicts-the-old-primary-from-raft-so-it-never-learns-it-lost-its-slots.md"]
...
// NEW FINDING (unfiled, no matching issue among 15/17/19/20/26): the model
// represents `removeNode` as tombstoning — `nodes` is a total map over the fixed
// NODES set, so a removed node's map entry survives with `member: false` but its
// `role`/`parent` fields untouched by the transition. The real code's `RemoveNode`
// arm (`commands.rs:215-236`) does `inner.nodes.remove(&node_id)` — a genuine
// deletion, matching TR-CLUSTER-003's own postcondition in `specs/cluster.md`
// ("Node membership" loses `node_id`) — so there is no live data anywhere in
// `ClusterState` to recover a removed node's last role/parent from. This reads as
// a model-representation artifact (the model's own target spec, `specs/cluster.md`,
// confirms real deletion is the intended semantics; nothing suggests the model's
// ghost-record fields encode a deliberate requirement) rather than a code defect,
// but per the divergence protocol this is reported rather than resolved here:
// see task-3-report.md for the full trace and disposition options.
#[quint_run(
    spec = "../../../specs/quint/cluster_migration_failover.qnt",
...
#[ignore = "UNFILED finding — model retains a removed node's stale role/parent (total-map tombstone), code truly deletes the record (matches specs/cluster.md TR-CLUSTER-003); see task-3-report.md"]
...
// membership, but hits the same ghost-record finding as the test above (node
// 1's `role` stays `Primary` in the model's tombstone, defaults away in the
// driver's projection since the real node record is gone). See the comment on
// `remove_node_after_demotion_succeeds_test` above and task-3-report.md.
#[quint_run(
    spec = "../../../specs/quint/cluster_migration_failover.qnt",
...
#[ignore = "pending issues 15/17/19/20/26 plus an UNFILED finding — any sampled trace touching failoverGraceful/Auto/Forced, feedBuffer, completeRepatriation, or removeNode (ghost-field finding, see module header) diverges immediately; see the named-run citations above"]
