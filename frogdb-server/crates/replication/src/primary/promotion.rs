...
mod tests {
    ...
    fn a_stint_heads_the_minted_id_and_freezes_the_inherited_one_at_the_boundary() {
        ...
        let plan = plan_primary_stint(&previous, hex_id('a'), 100);

        assert_eq!(plan.minted.replication_id, hex_id('a'));
        assert_eq!(
    ...
    fn the_save_offset_rises_to_the_boundary() {
        let plan = plan_primary_stint(&state_at(40), hex_id('a'), 100);
    ...
    fn the_save_offset_never_follows_a_boundary_backwards() {
        let plan = plan_primary_stint(&state_at(900), hex_id('a'), 50);
        assert_eq!(
    ...
    fn a_boundary_equal_to_the_save_offset_changes_nothing_about_it() {
        let plan = plan_primary_stint(&state_at(500), hex_id('a'), 500);
    ...
    fn a_stint_at_the_zero_boundary_still_mints_and_arms() {
        ...
        let plan = plan_primary_stint(&previous, hex_id('a'), 0);
    ...
    fn a_second_stint_shifts_the_first_stints_id_into_the_window() {
        let first = plan_primary_stint(&state_at(0), hex_id('b'), 100);
        let second = plan_primary_stint(&first.minted, hex_id('c'), 300);

        assert_eq!(second.minted.replication_id, hex_id('c'));
        assert_eq!(second.minted.secondary_id, Some(hex_id('b')));
    ...
    fn the_rollback_is_the_state_the_node_was_already_on() {
        ...
        let plan = plan_primary_stint(&previous, hex_id('a'), 900);
    ...
    fn the_decision_is_a_function_of_its_inputs_alone() {
        ...
        let first = plan_primary_stint(&previous, hex_id('a'), 400);
        let second = plan_primary_stint(&previous, hex_id('a'), 400);
        assert_eq!(first, second);
        assert_eq!(
            previous.replication_id,
            plan_primary_stint(&previous, hex_id('d'), 400)
                .rollback
                .replication_id,
