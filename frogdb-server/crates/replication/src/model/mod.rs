...
//! |---|---|---|---|---|---|
//! | `smoke` | 3 nodes, 2 writes, 1 promotion, 2 psyncs, 1 drop | 152,338 | 21 | 1.4 s | default suite |
//! | `strand` | 2 nodes, 1 write, 1 promotion, persist failures | 191 | 9 | <0.1 s | default suite |
//! | `full/deep` | 2 nodes, 4 writes, 2 promotions, 3 psyncs, 2 drops, 1 demote | 3,655,653 | 33 | 25 s | nightly / `just replication-model-check` |
//! | `full/two-primary` | 3 nodes, 3 writes, 2 promotions, 3 psyncs, 2 drops, 2 reparents | 42,383,417 | 30 | 322 s | nightly / `just replication-model-check` |
//!
...
impl Model for Promotion {
            ...
            Action::Psync(r) => {
                ...
                match select_psync_arm(&line) {
                    ...
                    Ok(PsyncArm::FullResync {
                        ...
                        // Neither half of the granted pair is adopted here: the
                        // offset resets to 0 and the dataset is replaced only
                        // when the payload lands (FM-REPLICATION-001).
                        let payload = sys.nodes[p].history[..granted_offset as usize].to_vec();
                        let pre = sys.nodes[r].state.clone();
                        sys.nodes[r].history.clear();
                        sys.nodes[r].inflight.clear();
    ...
    fn properties(&self) -> Vec<Property<Self>> {
        let mut props = vec![
            ...
            // bit for bit what it was when the arm was selected, and it is
            // serving no data (FM-REPLICATION-001's ordering clause).
            Property::<Self>::always("full_resync_adopts_nothing_early", |_, sys| {
                sys.nodes.iter().all(|n| match &n.pending {
                    None => true,
                    Some(p) => n.state == p.pre && n.history.is_empty(),
                })
            }),
            ...
            Property::<Self>::sometimes("a_replica_streams_from_a_promoted_node", |_, sys| {
