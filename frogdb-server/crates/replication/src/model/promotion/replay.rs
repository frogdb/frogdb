...
fn a_failed_promotion_leaves_the_node_unable_to_replicate() {
    ...
    // A claim models *applying* bytes that were already received, so the
    // received head has to be there first — what `ReplicaOffset::frame_advance`
    // does on the ingest path, spelled without a frame. Applying ahead of the
    // stream is a state `INV-OFFSET-1` says the node is never in, and the
    // promotion below asserts the catalog.
    live.store(4, std::sync::atomic::Ordering::Release);
