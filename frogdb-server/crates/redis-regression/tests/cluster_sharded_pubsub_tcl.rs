...
//!
//! Upstream uses `start_cluster 1 1` (1 primary + 1 replica). Primary-side
...
async fn start_primary_replica_cluster() -> (ClusterTestHarness, u64, u64) {
    let mut harness = ClusterTestHarness::new();
    ...
    harness
        ...
        .await
        .unwrap();
    let primary_id = harness.primary_ids()[0];
    let replica_id = harness.replica_ids()[0];
    (harness, primary_id, replica_id)
}
