...
//! - `XADD IDMP with invalid syntax` — redis-specific — Redis-internal feature
//! - `XADD IDMP basic addition` — redis-specific — Redis-internal feature
//! - `XADD IDMP with binary-safe iid` — redis-specific — Redis-internal feature
//! - `XADD IDMP with maximum length iid` — redis-specific — Redis-internal feature
//! - `XADD IDMP with combined options` — redis-specific — Redis-internal feature
//! - `XADD IDMP argument order variations` — redis-specific — Redis-internal feature
//! - `XADD IDMP concurrent duplicate requests` — redis-specific — Redis-internal feature
//! - `XADD IDMP pipelined requests` — redis-specific — Redis-internal feature
//! - `XADD IDMP with consumer groups` — redis-specific — Redis-internal feature
//! - `XADD IDMP persists in RDB` — redis-specific — Redis-internal feature
//! - `XADD IDMP set in AOF` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers have isolated namespaces` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers each have their own MAXSIZE limit` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers persistence in RDB` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers concurrent access` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers pipelined requests` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers with mixed IDMP and IDMPAUTO` — redis-specific — Redis-internal feature
//! - `XADD IDMP multiple producers stress test` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with invalid syntax` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO basic deduplication based on field-value pairs` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO deduplicates regardless of field order` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with single field-value pair` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with many field-value pairs` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with binary-safe values` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with unicode values` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with long values` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO argument order variations` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO persists in RDB` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with consumer groups` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO field names matter` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO with numeric field names and values` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO multiple producers have isolated namespaces` — redis-specific — Redis-internal feature
//! - `XADD IDMPAUTO multiple producers` — redis-specific — Redis-internal feature
//! - `XIDMP entries expire after DURATION seconds` — redis-specific — Redis-internal feature
//! - `XIDMP set evicts entries when MAXSIZE is reached` — redis-specific — Redis-internal feature
//!
//! XCFGSET (Redis 8.x command for IDMP config, not implemented):
//! - `XCFGSET set IDMP-DURATION successfully` — redis-specific — Redis-internal feature
//! - `XCFGSET set IDMP-MAXSIZE successfully` — redis-specific — Redis-internal feature
//! - `XCFGSET set both IDMP-DURATION and IDMP-MAXSIZE` — redis-specific — Redis-internal feature
//! - `XCFGSET IDMP-DURATION maximum value validation` — redis-specific — Redis-internal feature
//! - `XCFGSET IDMP-DURATION minimum value validation` — redis-specific — Redis-internal feature
//! - `XCFGSET IDMP-MAXSIZE maximum value validation` — redis-specific — Redis-internal feature
//! - `XCFGSET IDMP-MAXSIZE minimum value validation` — redis-specific — Redis-internal feature
//! - `XCFGSET invalid syntax` — redis-specific — Redis-internal feature
//! - `XCFGSET multiple configuration changes` — redis-specific — Redis-internal feature
//! - `XCFGSET configuration persists in RDB` — redis-specific — Redis-internal feature
//! - `XCFGSET configuration in AOF` — redis-specific — Redis-internal feature
//! - `XCFGSET changing IDMP-DURATION clears all iids history` — redis-specific — Redis-internal feature
//! - `XCFGSET changing IDMP-MAXSIZE clears all iids history` — redis-specific — Redis-internal feature
//! - `XCFGSET history cleared then new deduplication works` — redis-specific — Redis-internal feature
//! - `XCFGSET history cleared preserves stream entries` — redis-specific — Redis-internal feature
//! - `XCFGSET setting same IDMP-DURATION does not clear iids history` — redis-specific — Redis-internal feature
//! - `XCFGSET setting same IDMP-MAXSIZE does not clear iids history` — redis-specific — Redis-internal feature
//! - `XCFGSET repeated same-value calls preserve IDMP history` — redis-specific — Redis-internal feature
//! - `XCFGSET changing value after same-value sets still clears history` — redis-specific — Redis-internal feature
//! - `XCFGSET setting same value preserves iids-tracked count` — redis-specific — Redis-internal feature
//! - `XCFGSET IDMP-MAXSIZE wraparound keeps last 8 entries` — redis-specific — Redis-internal feature
//! - `XCFGSET clears all producer histories` — redis-specific — Redis-internal feature
//!
//! XINFO STREAM IDMP/iids/pids fields (Redis 8.x feature):
//! - `XINFO STREAM shows IDMP configuration parameters` — redis-specific — Redis-internal feature
//! - `XINFO STREAM shows default IDMP parameters` — redis-specific — Redis-internal feature
//! - `XINFO STREAM returns iids-tracked and iids-added fields` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids-added is lifetime counter even after eviction` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids-duplicates is lifetime counter` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids-duplicates persists after eviction` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids-duplicates with multiple producers` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids counters after CFGSET clears history` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids-added persists in RDB` — redis-specific — Redis-internal feature
//! - `XINFO STREAM returns pids-tracked field` — redis-specific — Redis-internal feature
//! - `XINFO STREAM FULL returns pids-tracked field` — redis-specific — Redis-internal feature
//! - `XINFO STREAM iids-tracked counts across all producers` — redis-specific — Redis-internal feature
//! - `XINFO STREAM returns idmp-duration and idmp-maxsize fields` — redis-specific — Redis-internal feature
//! - `CONFIG SET stream-idmp-duration and stream-idmp-maxsize validation` — redis-specific — Redis-internal feature
//!
//! XDELEX (Redis 8.x extended-delete command with KEEPREF/DELREF/ACKED, not implemented):
//! - `XDELEX should return empty array when key doesn't exist` — redis-specific — Redis-internal feature
//! - `XDELEX IDS parameter validation` — redis-specific — Redis-internal feature
//! - `XDELEX KEEPREF/DELREF/ACKED parameter validation` — redis-specific — Redis-internal feature
//! - `XDELEX with DELREF option acknowledges will remove entry from all PELs` — redis-specific — Redis-internal feature
//! - `XDELEX with ACKED option only deletes messages acknowledged by all groups` — redis-specific — Redis-internal feature
//! - `XDELEX with ACKED option won't delete messages when new consumer groups are created` — redis-specific — Redis-internal feature
//! - `XDELEX with KEEPREF` — redis-specific — Redis-internal feature
//!
//! Internal-encoding / fuzz / stress:
//! - `XADD with LIMIT consecutive calls` — intentional-incompatibility:encoding — internal-encoding (uses stream-node-max-entries)
