...
#[tokio::test]
async fn active_expiry_suppressed_during_pause_write() {
    let server = TestServer::start_standalone().await;
    let mut control = server.connect().await;
    let mut client = server.connect().await;
    ...
    // Pause WRITE — active expiry should be suppressed (matches Redis behavior:
    // PAUSE WRITE suppresses expires to prevent replication stream writes)
    assert_ok(
        &control
            .command(&["CLIENT", "PAUSE", "60000", "WRITE"])
            .await,
    ...
    // Sleep well past the TTL and several active expiry cycles
    tokio::time::sleep(Duration::from_millis(2000)).await;

    // Key should still exist: active expiry was suppressed during PAUSE WRITE.
    // Use DBSIZE instead of GET to avoid triggering passive/lazy expiry.
    let count = unwrap_integer(&client.command(&["DBSIZE"]).await);
    assert!(
        count >= 1,
        "key should survive PAUSE WRITE (active expiry suppressed), DBSIZE={count}"
    );
