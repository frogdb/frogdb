...
| Rulings | [issue 10](../.scratch/spec-gaps/issues/open/10-txn-vll-advisory-sweep.md) — promotes this `ready_tx`-closed guard to THE invariant of FM-VLL-003, not the 2s/4s timing margin |
...
| Rulings | [issue 10](../.scratch/spec-gaps/issues/open/10-txn-vll-advisory-sweep.md) — rules the shard deadline should be *derived from* `acquisition.timeout` rather than kept numerically below it, so the two cannot drift apart in a later edit |
| Pending | [issue 10](../.scratch/spec-gaps/issues/open/10-txn-vll-advisory-sweep.md) — the shard deadline is a standalone constant today (`CONTINUATION_DRAIN_TIMEOUT`, `shard.rs:37`, independent of `DEFAULT_LOCK_ACQUISITION_TIMEOUT`, `coordinator.rs:36`); the ruled derivation from `acquisition.timeout` is not built |
...
| Pending | [issue 10](../.scratch/spec-gaps/issues/open/10-txn-vll-advisory-sweep.md) — no release-token matching exists; the cancellation precondition is not yet stated in FM-VLL-002 |
