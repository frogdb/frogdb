...
Gate: 0.80
Crates: frogdb-cluster, frogdb-cluster-runtime

Phase 4 mutation gate passed (`frogdb-cluster` 99.6% on 496 mutants, `frogdb-cluster-runtime`
99.0% on 224; the four surviving mutants are all documented equivalents at the code). Behavior
changes to this area are spec-first: edit the row, update the forcing test, then the code. Before
pushing changes that touch these crates, run `just mutants-diff <crate>`. See CLAUDE.md "Locked
core areas".
...
covers slot migration interleaved with failover (TR-CLUSTER-003, 004, 008–021, 024, 025, 034,
...
Tests marked `#[ignore]` name the owning issue rather than being evidence the row already holds.
...
| Source | `cluster-runtime/src/flags.rs` (`SelfFenceGate::has_quorum`, today keyed on `HealthTable` quorum, FM-CLUSTER-055/059); election-timeout fence not yet in code. The *read* fence (FM-CLUSTER-107) reads the same verdict through `SelfFenceGate::fences_stale_reads`, so issue 27's move to CheckQuorum evidence re-bases both halves at once — there is no second evidence source to migrate. |
| Rulings | Issue 27 |
...
| Invariant | `commands.rs:816-862` clears the slot map and migrations unconditionally before it branches, so no slot or migration can survive either form. `ClusterWriter::propose_reset` snapshots the peer list *before* proposing (a committed reset empties the topology), routes through `propose`, and applies the local identity update only on `Applied`. |
...
| Observable | Disabling the fence makes writes acceptable again on the same gate instance; re-enabling it fences again. With healthy quorum the flag makes no difference. The write-fence reason reported to observability is `"cluster quorum lost"` exactly when the gate fences, and absent when it does not. The same knob and the same verdict also gate *reads* on a fenced node (FM-CLUSTER-107), so one flip arms or disarms both halves. |
...
| Observable | The remote subscriber count is 0 and no RPC is attempted. `SPUBLISH` on a standalone node returns `SpublishOutcome::Local`, meaning "deliver locally". |
...
| Forced by | `test_local_forwarder_broadcast_is_noop`, `test_local_forwarder_forward_delivers_locally`, `cluster_broadcast_is_a_noop_below_two_nodes`, `cluster_broadcast_sums_every_other_reachable_peer_once`, `cluster_broadcast_folds_an_unreachable_peer_into_zero` |
| Bug refs | — |

## FM-CLUSTER-070 — `SPUBLISH` refuses a slot the cluster cannot place
...
| Observable | Four routes, three client-visible replies. This node owns the slot → local delivery, reply is the local subscriber count. A reachable remote owner → forwarded over the bus, reply is the owner's count (`Forwarded(count)`). Nobody owns the slot, or the owner has no address → **refused**, byte-exactly `CLUSTERDOWN Hash slot <slot> not served`, and nothing is delivered anywhere. The two refusals share that reply but stay distinguishable in the outcome (`Unowned` vs `OwnerUnaddressable{owner}`) and each warn-logs naming the slot. |
| NOT observable | A best-effort local delivery reported as a subscriber count. An integer reply asserts the message reached the slot's subscribers; on a node that does not own the slot it reaches only registrations stranded by a slot move or opened in the same slot-map gap, so the count implies a health the cluster does not have. Nor a refusal text that differs from the subscribe path's — a client told `CLUSTERDOWN` by `SSUBSCRIBE` and handed a count by `SPUBLISH` on the same channel cannot tell the cluster is broken. |
| Invariant | `route_shard_channel_in` is a pure function of the slot map plus the address registry and returns a four-variant `ShardRoute`; `forward_spublish` matches it exhaustively and collapses exactly the two unplaceable arms into `SpublishOutcome::Refused`, so a new unresolvable case cannot be added as a silent local delivery. The reply is built only by `frogdb_types::redirect::clusterdown_slot`, the same constructor `SSUBSCRIBE`'s routing seam uses, and `lint-redirect-seam` rejects an inline `CLUSTERDOWN Hash slot` literal anywhere else. |
| Outcome variant | `ShardRoute::{Local, Remote, Unowned, OwnerUnaddressable}` / `SpublishOutcome::{Forwarded, Local, Refused{slot, cause}}` / `CLUSTERDOWN Hash slot <slot> not served` |
| Forced by | `cluster_forward_delivers_locally_when_this_node_owns_the_slot`, `cluster_forward_refuses_an_unowned_slot`, `cluster_forward_refuses_an_owner_this_node_cannot_address`, `cluster_forward_refusals_stay_distinguishable_by_cause`, `cluster_route_names_a_reachable_remote_owner`, `test_local_forwarder_forward_delivers_locally`, `cluster_forward_reports_the_owners_subscriber_count`, `spublish_refusal_replies_clusterdown_naming_the_slot`, `spublish_forwarded_outcome_replies_the_owners_count`, `spublish_local_outcome_defers_to_local_delivery` |
| Bug refs | fixed: [36-spublish-conflates-unowned-slot-with-local-ownership.md](../.scratch/hardening/issues/done/36-spublish-conflates-unowned-slot-with-local-ownership.md); ruled: [21-spublish-refusal-semantics.md](../.scratch/redis-feel/issues/done/21-spublish-refusal-semantics.md) |

The refusal is Redis parity, and it is parity with FrogDB's own subscribe path. Subscribers *are*
slot-routed at subscribe time — `SSUBSCRIBE` and `SUNSUBSCRIBE` both run the per-channel
`SlotMigrationCoordinator::route()` loop and answer `MOVED`/`CLUSTERDOWN` for a channel this node
does not own, same as a keyed command
(`frogdb-server/crates/server/src/connection/pubsub_conn_command.rs`). So `Unowned` and
`OwnerUnaddressable` are a slot-map gap: registrations stranded from before the slot moved off this
node, plus any client that subscribed during the same gap this `SPUBLISH` landed in. Delivering to
those best-effort was the earlier pin; it is dropped, because the publisher's receiver count must
not imply an ownership the cluster cannot assert, and because the stranded subscriber's loss is
exactly the loss Redis takes. Redis reaches the same reply from the same two states: `getNodeByQuery`
yields `CLUSTER_REDIR_DOWN_UNBOUND` — `-CLUSTERDOWN Hash slot not served` — whenever it cannot name a
node for the slot. FrogDB's text adds the slot number, the pre-existing house rendering shared with
every other slot refusal (`redirect::clusterdown_slot`); the `CLUSTERDOWN` code clients match on is
byte-identical. The surviving deviation is case 2 only: a *reachable* remote owner is forwarded to
rather than answered with `MOVED` (see the [Redis deviations](#redis-deviations) table).

This is not FM-CLUSTER-107's refusal. That one fires on Raft quorum loss and answers
`CLUSTERDOWN The cluster is down (quorum lost, stale reads refused)` for reads on a fenced node;
this one fires on a slot the local view cannot place and answers the slot-scoped text. Different
trigger, different mechanism, different string — a client can tell them apart.
...
| Outcome variant | `Option<&NodeInfo>` |
| Forced by | `a_departed_replica_is_not_a_candidate_while_a_slot_is_under_handoff`, `a_re_attached_replica_is_a_candidate_again`, `outside_a_handoff_window_a_departed_replica_is_still_a_candidate`, `a_probe_that_learned_nothing_is_treated_as_departed_under_handoff`, `the_handoff_filter_beats_the_last_resort_availability_floor`, `auto_failover_skips_the_departed_replica_of_a_slot_under_handoff`, `auto_failover_abandons_when_every_candidate_departed_under_handoff`, `a_lapsed_handoff_lease_stops_filtering_candidates`, `the_health_probe_reports_this_nodes_inbound_link_state` |
...
## FM-CLUSTER-107 — a quorum-fenced node refuses reads, and offers no staleness bound

| Field | Value |
|---|---|
| Trigger | Any command arriving on a cluster node whose self-fence gate reports quorum lost (`SelfFenceGate::fences_stale_reads`: `cluster-self-fence-on-quorum-loss` armed — the default — **and** the failure detector cannot reach a majority). The node need not be a replica and its replication link need not be down: a partitioned primary with no inbound link at all trips this. |
| Observable | The read is refused at the connection gauntlet with `-CLUSTERDOWN The cluster is down (quorum lost, stale reads refused)`, before the command runs and before the shards are consulted, and — like every gauntlet refusal — an open `MULTI` is flagged dirty so its `EXEC` aborts. Three carve-outs, in this order: a **write** is answered by the older write rung first, so it still reads `-CLUSTERDOWN The cluster is down (quorum lost, writes rejected)` (FM-CLUSTER-059) rather than a read refusal; a command carrying `CommandFlags::STALE` — the set `COMMAND INFO` advertises as `stale`, covering PING, INFO, CONFIG, COMMAND, AUTH, CLUSTER's read subcommands, SUBSCRIBE — is answered normally, so a fenced node stays diagnosable and reconfigurable; and `replica-serve-stale-data yes` reopens reads entirely (it never reopens writes). The refusal clears on the next command once the node reaches its quorum again — nothing latches. **No staleness bound is offered in either direction.** When the fence is armed, a refused read is refused for *any* age including zero: the node cannot distinguish a keyspace one millisecond stale from one a week stale, because the fact it lost is precisely the ability to know. When the fence is disarmed — by the knob, by `replica-serve-stale-data`, or by the STALE exemption — the value served carries no maximum age, no version, no read timestamp, and no way for the client to ask for one. A partitioned FrogDB node is *unavailable* or *unbounded*; it is never "at most N seconds behind". |
| NOT observable | A bounded-staleness read of any shape: no max-staleness argument, no `READ AT` timestamp, no per-reply age annotation, and no lease that would make a fenced node's read safe for a bounded window (Consul's and CockroachDB's stale-read leases). Nor a read refused by the *replication* fence sharing the same rung: `min-replicas-to-write` losing its replicas costs durability, not currency, and `QuorumChecker::fences_stale_reads` defaults to `false` precisely so a standalone primary with the write fence armed keeps answering `GET`. Nor a refusal on a node that has quorum but does not own the slot — that is a routing verdict (`-MOVED`, or the slot-coverage `-CLUSTERDOWN` of FM-CLUSTER-018..030), decided later in the gauntlet and unrelated to staleness. Nor anything about replica *apply* traffic: the replication executor never builds a `PreDispatchView` and is not gated here. |
| Invariant | One verdict drives both halves. `SelfFenceGate::fences_stale_reads()` is `self_fence_on_quorum_loss() && !inner.has_quorum()`, which is exactly `!SelfFenceGate::has_quorum()` — the value the write rung and the `/status` write-fence reason already read (FM-CLUSTER-059) — so a node can never refuse writes while claiming its reads are current, and the one knob disarms both together. The knob is read *first* and short-circuits: this runs once per command rather than once per write, and `FailureDetector::has_quorum` walks the node table, so a cluster with the fence off pays nothing. The exemption set is the `CommandFlags::STALE` bit itself, resolved per-subcommand through `Command::flags_for`, so the gate and what `COMMAND INFO` advertises cannot drift. The opt-out is `replica-serve-stale-data` — the *same* knob and wire name the replication half reads (redis-feel issue 17), deliberately not a second cluster-side setting, so one line of config governs both sources of unbounded staleness. The policy itself is `frogdb_core::command_admission::quorum_stale_refusal`, a pure function beside `stale_refusal`; the two stay separate bodies because they answer to two locked specs, and the gauntlet supplies the live inputs to each. The *evidence* behind the verdict is TR-CLUSTER-026's, unchanged: issue 27's move from TCP-probe quorum to CheckQuorum liveness re-bases both halves at once, because there is no second source to migrate. |
| Outcome variant | `Option<CommandError>` = `Some(CommandError::ClusterDownStaleRead)` |
| Forced by | `a_quorum_fenced_gate_fences_stale_reads`, `read_fencing_and_write_fencing_share_one_verdict`, `the_self_fence_knob_disarms_read_fencing_live`, `the_read_fence_short_circuits_on_the_knob`, `the_trait_default_leaves_a_non_cluster_fence_serving_reads`, `a_healthy_quorum_gates_nothing`, `a_quorum_fenced_node_refuses_a_read_by_default`, `a_stale_flagged_command_survives_the_quorum_fence`, `the_serve_stale_data_knob_reopens_a_quorum_fenced_node`, `the_cluster_fence_and_the_link_fence_name_different_mechanisms`, `test_self_fence_read_allowed_when_the_fence_does_not_claim_staleness`, `the_gauntlet_refuses_a_read_on_a_quorum_fenced_node`, `the_gauntlet_names_the_write_fence_for_a_write`, `the_gauntlet_serves_stale_flagged_commands_on_a_fenced_node`, `the_gauntlet_honours_serve_stale_data_on_a_fenced_node` |
| Bug refs | [40-read-consistency-contract-and-serve-stale-knob.md](../.scratch/cluster-correctness/issues/done/40-read-consistency-contract-and-serve-stale-knob.md) |

Distsys-review MIN-5, ruled "row + stale-serve knob" 2026-08-14 and closed in two halves. The
replication half shipped first (redis-feel issue 17): `replica-serve-stale-data`, live-mutable,
defaulting to **`no`** where Redis defaults to `yes` — a deliberate deviation, on the CockroachDB
and FoundationDB argument that answering from a snapshot of unbounded age by default trades a
silent correctness failure for an availability win the caller never asked for. That gate keyed on
the *replication link*, which left the case this row names wide open: a node fenced at the **Raft**
layer while its replication link is healthy, or a partitioned primary with no link at all, has no
link-down condition to trip and served reads ungated. The write fence had been in place since
FM-CLUSTER-059 and the code said so in as many words — `cluster.rs`'s knob documented "Reads remain
available", and a unit test pinned it — so the node refused the write that would diverge and then
answered `GET` from its pre-partition snapshot for as long as the partition lasted, which is the
half of the split-brain hazard that produces no error anywhere.

The second half of the ruling was the harder one: this row states what FrogDB *does not* offer.
Bounded staleness is the feature a reader arrives expecting — Consul's `stale` with `X-Consul-Last-
Contact`, CockroachDB's `AS OF SYSTEM TIME` with follower-read leases — and FrogDB has none of the
machinery it needs. A lease-based bound requires the fenced node to hold something the quorum
granted it before the partition and to reason about its expiry against a clock this area's preamble
refuses to trust for state transitions. So the contract is the honest binary, written down rather
than left to be inferred: fenced means unavailable, unfenced means unbounded, and an operator who
picks availability picks it by name.
...
| Shard pub/sub slot routing | `SSUBSCRIBE`/`SUNSUBSCRIBE` are slot-routed like keyed commands, answering `MOVED`/`CLUSTERDOWN`; `SPUBLISH` forwards to the slot owner over the cluster bus when it can reach one, and refuses `CLUSTERDOWN Hash slot <slot> not served` when it cannot | `SPUBLISH` answers `MOVED` for a slot another node owns, `CLUSTERDOWN` when it cannot name the owner | Narrowed to the reachable-owner case only. Forwarding spares the client a redirect round trip and is the whole remaining deviation; the unplaceable cases (no owner in the slot map, owner with no address) refuse exactly as Redis does, because a subscriber count returned from a node that does not own the slot claims a delivery it did not make (FM-CLUSTER-070, [issue 21](../.scratch/redis-feel/issues/done/21-spublish-refusal-semantics.md)). |
...
| Reads on a fenced node | Refused by default; the opt-out is `replica-serve-stale-data`, the same knob that reopens a link-down replica | `cluster-allow-reads-when-down`, default `no`, a second knob distinct from `replica-serve-stale-data` | Same default, one fewer setting. Upstream splits the two staleness sources across two knobs because its cluster state and its replication link are separate subsystems; FrogDB's answer to both is "this keyspace is of unbounded age", so it is spelled once. FrogDB's refusal also names the mechanism — `CLUSTERDOWN The cluster is down (quorum lost, stale reads refused)` where upstream answers a bare `CLUSTERDOWN The cluster is down` — because with one knob governing two fences the operator needs to know which one fired. The trigger differs too: upstream keys on cluster state `fail` (which includes slot-coverage loss), FrogDB on Raft quorum loss. See FM-CLUSTER-107. |
| Bounded / stale reads | Not offered. A partitioned node is unavailable (fenced) or unbounded (`replica-serve-stale-data yes`); there is no max-age argument, read timestamp, or follower-read lease | Also not offered — `cluster-allow-reads-when-down yes` serves unbounded staleness the same way | Identical, and stated explicitly rather than left to be inferred (FM-CLUSTER-107). Bounded staleness in the CockroachDB / Consul sense needs a lease granted before the partition plus a trusted clock to expire it, and this area's preamble rejects a wall clock gating a state transition. |
