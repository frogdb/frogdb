...
| Postcondition | before the replayed range is spliced, the replica discards every effect it holds above its claimed offset — Raft-style divergence truncation. The claimed prefix is untouched: that is what the grant asserts agreement on. Two consequences follow by construction: the granting history's frames never land on a position the replaced history wrote (so the "overwrite is the correction" case disappears rather than being relied on), and the *residue* case disappears with it — a forked position the new history never reaches, which no reconnect will ever ask for again because the replica's offset does not claim it. — **Not implemented.** Today the `+CONTINUE` accept shifts the replication id into the failover window and rewinds ClaimedOffset/ReceivedOffset to LandedOffset; it discards no keyspace state, so an overshipped tail survives the accept and is left to whatever the granting history happens to write next. |
| Source | `frogdb-replication/src/replica/connection.rs:300` (`psync`, `PsyncArm::Continue` arm — `shift_replication_id` then `set_state(Streaming)`, no truncation); `frogdb-replication/src/session_machine.rs:215` (`BeginSync::Full`, the overship that creates the tail) |
| Rulings | R5 of the quint-completeness campaign's 2026-08-20 post-execution rulings (`.scratch/formal-spec/2026-08-19-quint-completeness-campaign.md`) — ruled a new replica-side rule so the residue class is removed by construction rather than argued about case by case. Note the implementation gap this row opens: a keyspace is not a write log, so "truncate above the claim" has no in-place form — the honest implementations are to refuse the partial arm whenever the replica holds effects above its claim (degrade to `FullResyncReason::…`, costing a full resync), or to make the overship impossible by pairing the trailer offset with the data the payload actually carries. Choosing between them is a follow-up, not this row. |
| Pending | R5 — implementation + forcing tests not yet written; the rule is modelled only |
