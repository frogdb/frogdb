...
| Field | Value |
|---|---|
| Trigger | A write arriving at a shard whose measured usage exceeds `maxmemory`, under a policy other than `noeviction`. Also its negative: the same write arriving while the shard is inside its limit, or with no limit configured. |
...
| NOT observable | An eviction the rest of the system does not learn about: no path removes a key for memory pressure without the notification, the counters and the replicated removal, whichever backend chose it. An `evicted` event for a spill, or a WATCH invalidated by one. A byte count taken from an estimate rather than from the entry: the freed figure is the key's own accounted `memory_size`, read before the removal. |
