...
| Invariant | Two bounds, one per layer. The driver's is `max_attempts = 10` in `check_memory_for_write_inner`, plus `evict_one` returning `false` the moment the policy has no candidate, which exits the loop early rather than retrying. The keyspace's is `Table::cold_candidates`: the walk visits one queue at a time from the tail, reads each segment's colder neighbour *before* touching it, and a segment can be moved at most once per epoch (`reconcile` stamps `last_touch`), so `2n + 2` steps is a proven bound and is asserted as one. Within a segment, `nominate_from` stops after one lap of the victim cursor, so the ring cannot re-offer a key inside a single call. Neither bound depends on the clock advancing: a constant `epoch` freezes promotion, never nomination. |
| Outcome variant | `CommandError::OutOfMemory` / `Table::cold_candidates == 0` |
| Forced by | `an_unevictable_keyspace_refuses_writes_and_still_serves_reads`, `a_walk_with_nothing_to_take_reports_zero_rather_than_spinning`, `a_frozen_epoch_still_makes_progress`, `random_reference_and_growth_sequences_keep_the_queues_consistent` |
| Bug refs | — |
