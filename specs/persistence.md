...
## FM-PERSISTENCE-060 — a shard's replication stamp commits with the batch that carries it, both directions

| Field | Value |
|---|---|
| Trigger | A replicable write staged into a shard's `WriteBatch` once the mint moves to the persist point (replication issue 37, rulings R18/R19/R23): the batch carries a per-shard **stamp key** — the highest replication offset of any write in this batch — in a reserved per-shard metadata CF, and a crash, a `BGSAVE` cut, or a commit failure lands around the commit. Sharpened by batch sharing: single-action writes are not group-bracketed (FM-PERSISTENCE-001's one-action exception) and several of them share one storage batch, so the stamp is a **watermark over whatever the batch held**, not a bracket around one command. |
| Observable | After any recovery — RocksDB replays its own WAL to the last committed batch — a per-CF get of each shard's stamp key yields exactly the flushed coverage: the stamp equals the highest offset among that shard's recovered writes, for every shard, including a shard that never wrote (no stamp key, coverage zero). Under `should_confirm` (`durability-mode = sync` or `wal-failure-policy = rollback`), the pre-effect persist carries the stamp, so an acknowledged write's recovered stamp is at or above that write's own end offset — the ack covers data **and** stamp. |
| NOT observable | A stamp that outruns its data — the claim above the coverage, which would make a recovered node vouch for offsets whose writes the crash discarded (the issue-24 divergence class, re-armed from disk). A stamp that trails its data — coverage above the claim with no vector describing the residue, which is the recovered-stint blindness FM-REPLICATION-066's R16 refusal exists to contain, reintroduced by the very mechanism meant to end it. A commit failure that drops the staged set but leaves the stamp advanced, or commits the stamp's CF write while the data half is lost — the batch is one `WriteBatch` precisely so neither half can land alone. |
| Invariant | The stamp is written through the staging seam into a reserved per-shard metadata CF (the `search_meta_<n>` reserved-tier precedent), so it rides the same `WriteBatch` as the data it describes and RocksDB's batch atomicity — which spans CFs — carries both or neither. The persist seam takes the offset as a parameter, so the stamp names what the *caller* minted rather than re-deriving it. Commit failure drops the staged set and the stamp together — atomicity holds in both directions, success and failure alike. What stays true around it: cross-shard groups remain one batch per shard, each stamped only with its own half's watermark (FM-PERSISTENCE-001's cross-shard non-guarantee is unchanged, and issue 35's torn-group mend depends on per-shard stamps reflecting exactly the per-shard halves); `wal_watermark.rs` / FM-PERSISTENCE-035 tracks RocksDB sequence numbers — a different quantity, untouched. |
| Outcome variant | n/a (storage-layer invariant; surfaces as recovered coverage, not as a reply) |
| Forced by | MISSING ([gap: 37-mint-at-persist-and-primary-stamps.md](../.scratch/replication-correctness/issues/open/37-mint-at-persist-and-primary-stamps.md)) — the forcing tests (batch atomicity both directions in `frogdb-persistence`; crash-after-ack stamp coverage under `should_confirm`) land with the implementation; spec row first per the spec-first flow. |
| Bug refs | `.scratch/replication-correctness/issues/open/37-mint-at-persist-and-primary-stamps.md`; parent PRD `.scratch/replication-correctness/issues/open/36-offset-stamped-batches-restart-bias.md` |
