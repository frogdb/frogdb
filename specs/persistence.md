...
| Invariant | The empty-directory refusal is checked after the marker read and after `contains_files`, so a marked directory satisfies it without consulting the setting at all; `--force-fresh-data-dir` short-circuits both this refusal and FM-PERSISTENCE-051's. An unfinished install satisfies it too, and only it reaches the extra probe — the setting is the only reader of `pending_install` ([FM-PERSISTENCE-059](#fm-persistence-059--identity-is-stamped-before-the-install-and-an-install-waiting-to-finish-is-not-an-empty-directory)), so an ordinary boot pays nothing for it. The setting is a plain config param (settable from file and environment); the override is not, for the reason in FM-PERSISTENCE-051's invariant. |
| Outcome variant | `RecoveryPhase::VerifyDataDir` |
