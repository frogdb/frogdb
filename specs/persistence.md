...
| `wal_watermark` | format tag + decimal `u64` + xxh3-64 of that decimal, in the `frogdb_wal_watermark` side-file (`rocks/wal_watermark.rs:63-108`); a body whose digest does not verify — torn, zero-filled, truncated, or written by a build using another format tag — reads back as *absent*, so no filesystem's data-vs-rename ordering is load-bearing | `RocksStore::record_wal_watermark`, called from two independent sites (TR-PERSISTENCE-010): the `Periodic`-mode out-of-band `durable_sync` tick (`rocks/mod.rs:345`) and a `Sync`-mode commit's direct in-line call from the flush thread (`wal/flush.rs:206-208`); also on fresh open (`rocks/mod.rs:295`), and re-baselined by `wal_watermark::detect_and_reset` on an existing open (`rocks/mod.rs:294`, `wal_watermark.rs:159-197`) | On disk, best-effort (never itself fsynced — see `wal_watermark.rs:115-119`) | Yes, but may lag the true durable point — by construction it never leads it |
...
| `backup_dir` | `<data-dir>/backup/<db>_backup_<seq>` directory(ies) | rename 1 of the install (`frogdb-persistence/src/rocks/checkpoint.rs`), named via `backup_dir_name` (`staged.rs`) from the sequence `BackupCounter::reserve_next` reserved | On disk | Yes, until `prune_backups` removes it (`staged.rs`, `BACKUP_RETENTION = 1`) |
| `backup_counter` | the monotone `u64` in `<data-dir>/backup/counter` (scratch: `counter.tmp`) | `BackupCounter::record`, called by `reserve_next` before the reserved sequence names anything (`frogdb-persistence/src/rocks/staged.rs`) | On disk, temp-file-then-rename publish | Yes — and when it is *not* (an operator copied the backups without it), the sequence is recovered from the highest backup on disk, never restarted at 0 |
...
| `write_buffer_manager` | one `rocksdb::WriteBufferManager` held by `RocksMemory` (`frogdb-persistence/src/rocks/memory.rs`), owned by the `RocksStore` for the DB's lifetime | `RocksMemory::install`, once per `RocksStore::open_with_warm_metrics`; installed on `db_opts`, where RocksDB reads it (a manager set on the CF options is discarded by `open_cf_descriptors`) | N/A — a process-lifetime C++ object, not persisted data | Budget = `RocksConfig::memtable_budget_bytes()` (`write_buffer_size × max_write_buffer_number`), `allow_stall = true`; independent of the column-family count ([FM-PERSISTENCE-060](#fm-persistence-060--memtable-memory-is-bounded-by-one-process-wide-write-buffer-manager-not-by-a-per-column-family-product)) |
| `block_cache` | `Option<rocksdb::Cache>` on the same `RocksMemory`; `None` when `block_cache_size == 0` | `RocksMemory::install`, once per open; installed via `BlockBasedOptions::set_block_cache` on the one `block_opts` | N/A | Capacity = `RocksConfig::memory_budget_bytes()` (`block_cache_size + memtable_budget_bytes()`), because memtable bytes are costed to it ([FM-PERSISTENCE-061](#fm-persistence-061--one-process-wide-block-cache-sized-as-the-sum-of-the-two-operator-knobs)). `None` leaves RocksDB's own per-table-factory default |
| `persistence_budget` | `frogdb_memory::Budget` for `Subsystem::Persistence`, created at `RocksStore` open and adopted by **shard 0's** `MemoryBroker` (`MemoryBroker::adopt`) so the subsystem appears once per process, not once per shard | `RocksMemory::refresh` reconciles the held `Charge` against the engine's reported usage (WBM usage + cache usage); the ticker in `server/subsystems.rs` drives it | In-memory only | No — limit is `RocksConfig::memory_budget_bytes()`; an over-budget reading is a `Refused` counted at `frogdb_memory_budget_refusals_total{subsystem="persistence"}` |
...
| Postcondition | `wal_watermark` is `fetch_max`-advanced to `covered_seq`, a value the caller snapshotted (`RocksStore::latest_sequence_number()`) *before* starting the write or flush it now reports complete — (a) the pre-flush snapshot inside `durable_sync`, (b) the pre-write snapshot at the top of `RocksSink::commit` — never a fresh read taken after the fact, via an atomic temp-write-then-rename that is deliberately never fsynced. The rename makes the *directory entry* atomic; the **body** is made self-verifying instead of assuming a filesystem orders the data write ahead of the rename (ext4's `auto_da_alloc` heuristic does, XFS/btrfs/ext4-`data=writeback` do not, and the operator picks the filesystem) — it carries a format tag and an xxh3-64 digest of the sequence text, and any body that fails to verify reads back as absent rather than as a sequence (FM-PERSISTENCE-035). `fetch_max` rather than a blind store: independent concurrent callers (each shard's own sync commit, plus the periodic tick) report out of order, and a lower snapshot must not regress a higher one. (a) and (b) both terminate at the same write function; (b) is the direct in-line call the comment at `flush.rs:186-195` explains: "any concurrent shard's still-unsynced write can land in the gap between our fsync finishing and that read, inflating the claimed durable point past what is actually on disk". (b) additionally rests on an **upstream** invariant, load-bearing on the durable-ack path and therefore named here rather than assumed: the in-line record is only sound if a `sync = true` commit's own fsync really covered it, which requires that RocksDB never merge that write into a concurrent `sync = false` write group. `write_thread.cc::EnterAsBatchGroupLeader` (LevelDB-inherited) enforces exactly this — a follower whose sync flag differs from the leader's is refused admission and starts its own group — so one shard's unsynced commit can never downgrade another shard's synced one behind a shared WAL file. A RocksDB upgrade that relaxed the rule would silently invalidate (b); the forcing test below is the regression guard |
| Source | `frogdb-persistence/src/rocks/checkpoint.rs:23-43` (`record_wal_watermark`, taking `covered_seq: u64`); (a) `rocks/mod.rs:353-371` (the snapshot inside `durable_sync`, taken alongside `pending`); (b) `wal/flush.rs:186-217` (`commit`; pre-write snapshot at `:196`, rationale comment `:186-195`, the recording call at `:217`); `wal_watermark.rs:120-127` (`write`, the temp-then-rename), `:142-149` (`fetch_max`), `:80-108` (`digest`/`encode`/`decode`, the self-verifying body) |
| Forced by | `concurrent_mixed_flag_commits_never_downgrade_a_sync_mode_write` (two flush engines, `Sync` and `Async`, committing into one `PageCacheSink`: every flag survives the interleaving intact and the `Sync` engine's acked writes all survive a crash), `only_a_synced_rocks_sink_commit_records_the_durable_watermark`, `a_racing_shard_write_after_the_fsync_does_not_inflate_the_watermark` |
| Rulings | [issue 26](../.scratch/spec-gaps/issues/done/26-distsys-review-minors-sweep.md) (MIN-6): the no-torn-body argument used to rest on ext4's `auto_da_alloc` without saying so. Replaced by a checksummed body, which holds on any filesystem an operator picks — the row states the mechanism rather than the mount option. (MIN-11): (b)'s group-commit assumption was uncited and untested — the upstream rule is now named above and pinned by a concurrent mixed-flag test. |
...
| Postcondition | `<data-dir>/backup` is created (and the root synced, publishing its name), then a sequence is reserved and durably recorded (`BackupCounter::reserve_next`), then `<data-dir>/db` is renamed into the backup root as `backup_dir_name(db_name, seq)`, then the backup root and the data-dir root are synced. Both operands are inside the data dir, so the mount point is never a rename operand ([FM-PERSISTENCE-057](#fm-persistence-057--staging-and-backup-live-inside-the-data-directory-the-mount-point-is-never-renamed)); the name comes from the counter, never the clock ([FM-PERSISTENCE-058](#fm-persistence-058--backup-names-come-from-a-persisted-monotone-counter-not-the-clock)) |
| Source | `frogdb-persistence/src/rocks/checkpoint.rs` (rename-1 guard, `fs.rename`, `fs.sync_dir`); `rocks/staged.rs` (`BackupCounter`, `backup_dir_name`) |
...
| Postcondition | five outcomes from `verify`: (1) marker present and readable → boot continues with the existing `database_id` (`data_dir.rs:115-123`); (2) no marker, no foreign entries → fresh init mints a new `database_id` (`:189-196`); (3) no marker, foreign entries present → refuses **unconditionally**, naming them (`:132-144`) — `--force-fresh-data-dir` is not an escape from this one (R6); (4) marker unreadable → refuses, unless `--force-fresh-data-dir` *and* the directory is excused-only, in which case the marker is re-minted (`:74-113`); (5) no marker, no foreign entries, but `persistence.require-existing-data` is set → refuses unless `--force-fresh-data-dir`, even though the directory is empty (`:146-187`) — an empty directory is what an unmounted volume and a genuine first boot both look like, and this branch is what tells them apart when the operator has declared the deployment past its first boot |
| Source | `frogdb-recovery/src/data_dir.rs:70-196` (`verify`) |
| Rulings | — |
...
| Source | `frogdb-persistence/src/rocks/wal_watermark.rs:159-197` (`detect_and_reset`, metric increment at `:167`, re-baseline write at `:194-196`); `rocks/mod.rs:293-297` (open-time call sites: `detect_and_reset` on an existing db, `wal_watermark::write` to seed on a fresh one); `mod.rs:188` (recovery mode) |
| Rulings | — |
...
| Field | Value |
| --- | --- |
| Precondition | no marker file and no foreign entries (TR-PERSISTENCE-035's fresh-init branch) |
...
| Postcondition | it is read and accepted if `layout_version <= DATA_DIR_LAYOUT_VERSION`; a version from the future refuses boot; **any** marker-read error — unreadable, malformed, or a future layout version alike — refuses unless `--force-fresh-data-dir` *and* the directory holds nothing FrogDB did not write, in which case all of them are bypassed and the marker is re-minted; beside foreign entries the refusal stands and names them (R6) |
| Source | `frogdb-persistence/src/data_dir.rs:76,144` (`DataDirMarkerError`, `read`); `frogdb-recovery/src/data_dir.rs:74-113` (the force-bypasses-every-read-error branch, gated on the foreign-entries probe) |
| Rulings | — |
...
| Field | Value |
|---|---|
| Trigger | A `CoreMsg::ExecTransaction` whose pre-apply footprint on the shard — the batch's own retained command bytes, plus (under `rollback` failure policy) the rollback snapshots it would take of every key its write commands name — would push the shard's `TxnBuffering` budget (`txn-buffer-limit`, per core, default 512 MiB) past its limit. Reached by every batch: the connection releases its queue-time charge (FM-TXN-054) before the shard round trip, so the shard's charge is the single accounting for the bytes from here on — the case that trips is a limit lowered by `CONFIG SET` between queuing and `EXEC`, a batch queued on a different core than the one it runs on (an unpinned connection, a replica-fed stream), or a rollback-snapshot footprint larger than the queued bytes. |
...
| Invariant | `ShardWorker::execute_transaction` computes the footprint and `grow`s a fresh `Charge` on the shard's own `Subsystem::TxnBuffering` budget **after** every admission gate (routing fence, rate limiter, watch check, minted-fence verify) and **before** the apply loop; a `Refused` returns `TransactionResult::Error` with the OOM text and falls through none of the apply, persist or write-effect paths. The charge is a local dropped at the end of the call, so every exit — refusal, a WAL rollback, a panic isolated by `panic_guard`, a normal commit — releases it. The charge site and its forcing tests live in `frogdb-core` (where staging lives), like FM-PERSISTENCE-001's shard-side tests, so this row sits outside the `frogdb-persistence` mutation gate. The queue-time half of the same bound is [txn.md FM-TXN-054](txn.md#fm-txn-054--a-multi-that-outgrows-the-transaction-buffer-is-poisoned-at-queue-time); the replica-side pending group is [replication.md FM-REPLICATION-045](replication.md#fm-replication-045--an-unterminated-replicated-multi-is-bounded-and-abandoned-never-accumulated). **Known gap:** because the connection releases its charge before the round trip and this one opens fresh, the batch is charged to nobody while it sits in the shard's channel — the same window FM-TXN-054 names, seen from the receiving end, and the reason the limit is not a bound on resident bytes under a slow shard; carrying the `Charge` in `CoreMsg::ExecTransaction` and growing it here instead is [memory-architecture issue 29](../.scratch/memory-architecture/issues/open/29-txn-charge-through-shard-channel.md). |
...
| Forced by | `a_transaction_past_the_buffer_limit_applies_nothing`, `a_transaction_under_the_buffer_limit_applies_and_releases_its_charge`, `the_rollback_snapshot_footprint_counts_toward_the_buffer_charge`, `test_txn_buffering_budget_row_is_published_per_shard` |
...
| Forced by | `test_sync_mode_zero_acked_write_loss`, `test_fsync_seam_sync_flag_matches_mode`, `test_sync_mode_crash_recovery`, `test_durability_mode_gates_crash_loss`, `confirm_snapshots_sequence_then_flushes_once`, `sync_mode_under_continue_policy_flushes_before_the_reply`, `sync_mode_acked_write_survives_a_crash_under_continue_policy`, `sync_durability_selects_confirm_independently_of_the_failure_policy`, `concurrent_mixed_flag_commits_never_downgrade_a_sync_mode_write` |
...
| NOT observable | A double install; a backup made for a no-op boot; a boot that hangs or errors because a previous install was interrupted. The *boot* around the install failing to reach it: phase 0 refusing the interrupted state as an empty directory, or the finished install presenting as a different database — both are [FM-PERSISTENCE-059](#fm-persistence-059--identity-is-stamped-before-the-install-and-an-install-waiting-to-finish-is-not-an-empty-directory), which is what makes "the next boot finishes the install" true of the boot and not only of `load_staged_checkpoint`. |
...
| Forced by | `test_load_staged_checkpoint_crash_after_backup_recovers`, `test_load_staged_checkpoint_idempotent_after_success`, `test_load_staged_checkpoint_installs_and_backs_up_old_db`, `test_load_staged_checkpoint_first_sync_no_existing_db`, `test_load_staged_checkpoint_absent_marker_is_noop`, `staged_checkpoint_is_installed`, `a_crash_between_the_install_renames_boots_to_the_same_database` |
| Bug refs | none |
...
| Observable | After a successful install, exactly one `<db>_backup_<seq>` remains under `<data-dir>/backup` — the newest. Older ones are deleted. A leftover backup from an interrupted install is kept rather than pruned away, and never blocks a new install. |
...
| Invariant | `BACKUP_RETENTION = 1`: one generation covers the story the backup exists for (recover the immediately-previous database if a sync installed bad data); every additional generation is a full database copy with no recovery story attached. Retention that thin only holds because the install never consumes a payload it cannot open ([FM-PERSISTENCE-024](#fm-persistence-024--an-incomplete-staged-checkpoint-is-refused-without-touching-the-live-database)), so the single generation is a rollback for *bad data*, not for a broken copy. The ordering key is the monotone sequence in the name ([FM-PERSISTENCE-058](#fm-persistence-058--backup-names-come-from-a-persisted-monotone-counter-not-the-clock)), never a clock reading, so no clock step can invert retention order. A directory that carries the backup prefix but whose sequence does not parse is **refused**: it is neither a retention candidate nor a deletion candidate, it is logged and counted in the prune outcome. Deleting a directory whose name this code cannot explain is the one outcome ruled out — it used to parse as sequence 0, sort oldest, and be removed. |
| Outcome variant | `PruneOutcome { removed, refused }` (warn-only on failure) |
| Forced by | `test_load_staged_checkpoint_prunes_older_backups`, `test_load_staged_checkpoint_crash_recovery_keeps_lone_backup`, `test_prune_backups_orders_numerically_not_lexically`, `test_prune_backups_noop_within_retention`, `prune_refuses_a_backup_name_it_cannot_parse`, `an_unparseable_backup_neither_blocks_nor_survives_being_deleted_by_an_install` |
| Bug refs | `.scratch/spec-gaps/issues/done/27-backup-naming-monotonic-counter-prune-refuses-unparseable.md` (distsys-review MIN-7) |
...
## FM-PERSISTENCE-058 — backup names come from a persisted monotone counter, not the clock

| Field | Value |
|---|---|
| Trigger | Two installs whose wall clocks do not cooperate: an NTP correction (or a VM restore) steps the clock backwards between them, or two full syncs land inside the same second — the ordinary case for a replica that resyncs, fails, and resyncs again. |
| Observable | Rename 1 names the displaced database `<db>_backup_<seq>` from `<data-dir>/backup/counter`, a `u64` that only ever increases: the reserved sequence is written to `counter.tmp`, fsynced, renamed onto `counter`, and the backup root fsynced — all *before* the name is used. Consecutive installs therefore produce strictly increasing names whatever the clock did, and retention ([FM-PERSISTENCE-026](#fm-persistence-026--exactly-one-previous-database-backup-survives-an-install)) keeps the generation that is genuinely newest. A counter file that is missing or unreadable is recovered from the highest backup directory present, so naming resumes *above* what is on disk. |
| NOT observable | A backup name that repeats or goes backwards. Two installs in one second colliding on a name — which would fail the second install's rename with `ENOTEMPTY`, exactly when a resync is already in trouble. Retention deleting the newer generation because a clock step made it sort older. Naming restarting at 0 next to backups that already carry higher sequences (an operator who copied `backup/` without the small file, a filesystem that lost it), which would collide with a directory sitting right there. A reserved sequence being reused after a crash: the reservation is durable first, so a crash between reserving and renaming burns a number, and burning is the safe direction. An install failing because the counter could not be *parsed* — a hygiene file does not fail a resync; the on-disk backups answer instead. |
| Invariant | `BackupCounter::reserve_next` takes the maximum of both sources of truth (the counter file and the highest existing backup) and adds one, then publishes it through the same write → fsync → rename → fsync-dir rule every other durable name here follows ([FM-PERSISTENCE-023](#fm-persistence-023--the-renames-that-publish-a-checkpoint-are-fsynced-on-both-sides)). This is the campaign's no-wall-clock-in-state principle applied to persistence: a name that orders state is a logical counter, and a clock reading is never an ordering key. The counter deliberately is *not* named with the backup prefix — `prune_backups` considers directories only, and a counter that could be mistaken for a backup is a counter that can be pruned. A failed publish removes its own scratch file, so `counter.tmp` never survives as a name an operator has to explain. |
| Outcome variant | n/a (a reservation failure surfaces as the install's `io::Error`) |
| Forced by | `the_first_backup_is_named_from_the_counter_not_the_clock`, `two_installs_in_the_same_second_get_distinct_backup_names`, `a_backup_named_under_a_moved_clock_does_not_outrank_a_new_one`, `a_lost_counter_file_recovers_above_the_existing_backups`, `reserving_a_backup_sequence_is_published_like_every_other_durable_name`, `staged_checkpoint_install_fsyncs_every_directory_it_changes` |
| Bug refs | `.scratch/spec-gaps/issues/done/27-backup-naming-monotonic-counter-prune-refuses-unparseable.md` (distsys-review MIN-7) |

---

# Engine memory — one budget for RocksDB's C++ allocations

RocksDB's memtables and block cache are allocated by the storage engine, outside every Rust
figure `INFO memory` reports. Before memory-architecture R10 they were sized by a *product* —
`write_buffer_size` × `max_write_buffer_number` × shards × column-family tiers — so a node could
sit well inside `maxmemory` and still be killed by the OOM killer. The two rows below fix the
shape of the bound. They live in this spec rather than in [`memory.md`](memory.md) because both
are observable on the durability path: one moves *when a write is admitted*, the other decides
what a reader of storage pays to read a block. The complementary question — who is *charged* for
those bytes, and how the charge appears in the per-subsystem breakdown — is attribution rather
than durability and waits for `specs/memory.md` to lock.

## FM-PERSISTENCE-060 — memtable memory is bounded by one process-wide write-buffer manager, not by a per-column-family product

| Field | Value |
|---|---|
| Trigger | A write-heavy workload against a store with many column families: `num_shards` shard CFs, plus a warm CF and a search-meta CF per shard when those tiers are on. Each CF has its own memtables, and each memtable grows to `write_buffer_size` before it flushes. |
| Observable | Total live memtable bytes stay under **one** number — `write_buffer_size × max_write_buffer_number`, the budget a single `WriteBufferManager` is created with at open — no matter how many column families the store has. The manager's budget is identical for a 1-shard and a 64-shard store, and its usage is a single process-wide figure (`RocksMemory::usage().write_buffer_bytes`) rather than a per-CF one. A write arriving with the manager at capacity **stalls** until a flush frees room: `allow_stall = true`, so the writer waits rather than allocating past the budget. That stall is the `Backpressure` disposition the persistence subsystem declares, realized by the engine. |
| NOT observable | Memtable memory scaling with the column-family count — the product this row exists to remove. A second `WriteBufferManager` anywhere in the process: the manager is created once, at the single `RocksStore` open, and every CF's `Options` is given that one handle (a per-open manager would put each `RocksStore` on its own budget and re-multiply). A write that is *refused* rather than stalled: the memtable bound is backpressure, and turning it into a client-visible error is a different disposition than the one declared. A stall that outlives the flush that clears it — the manager releases the writer as soon as usage drops back under the budget. The manager silently disabling itself for a `0` budget: `write_buffer_size` and `max_write_buffer_number` are both non-zero by construction (`RocksConfig::default`), and a zero product would make `enabled()` false, which is why the arithmetic is a named function with its own test rather than an inline multiply. |
| Invariant | `RocksMemory::install` builds exactly one `WriteBufferManager` per `RocksStore::open_with_warm_metrics` call, from `RocksConfig::memtable_budget_bytes()` (= `write_buffer_size × max_write_buffer_number`), with `allow_stall = true`; `Options::set_write_buffer_manager` is called once on the **`db_opts`** handed to `open_cf_descriptors`, so every column family the DB opens — existing, created-on-first-enable, and `default` alike — is accounted against that one manager. The DB-level placement is load-bearing: the manager is a `DBOptions` field, and `open_cf_descriptors` slices each descriptor's `Options` down to its column-family half, so a manager installed on the CF options is discarded at open — memtables then go unaccounted, no stall applies, and `get_usage` stays at zero (which reads as a satisfied bound). `memtable_usage_stays_under_the_write_buffer_budget_across_shards` asserts a *non-zero* usage for exactly that reason. `set_write_buffer_size` stays as the per-CF *flush trigger* (how big one memtable gets before it rolls), which is a different knob from the process-wide *ceiling* this row is about; the ceiling is what stopped being a product. Where a block cache is installed the manager is built with `new_write_buffer_manager_with_cache`, costing memtable bytes to that cache so one capacity covers both halves ([FM-PERSISTENCE-061](#fm-persistence-061--one-process-wide-block-cache-sized-as-the-sum-of-the-two-operator-knobs)). RocksDB's own wiki documents the two ways to use a manager (limit memtables, or cost them to the block cache); FrogDB uses both at once, which is the configuration ScyllaDB and CockroachDB also run their engines under — one global ceiling, no per-shard division. |
| Outcome variant | `frogdb_rocksdb_write_buffer_bytes` / `frogdb_rocksdb_write_buffer_limit_bytes` |
| Forced by | `the_memtable_budget_is_the_write_buffer_product`, `one_write_buffer_manager_bounds_memtables_across_shards`, `the_write_buffer_budget_does_not_scale_with_the_column_family_count`, `memtable_usage_stays_under_the_write_buffer_budget_across_shards`, `a_zero_block_cache_still_installs_the_write_buffer_manager`, `the_metrics_tick_publishes_the_engine_counters` |
| Bug refs | [memory-architecture issue 06](../.scratch/memory-architecture/issues/done/06-rocksdb-jemalloc-budget.md) (PRD ruling R10) |

## FM-PERSISTENCE-061 — one process-wide block cache, sized as the sum of the two operator knobs

| Field | Value |
|---|---|
| Trigger | Any read that misses the memtables and must pull an SST block, on a store with more than one column family; and an operator setting `block_cache_size_mb` — including setting it to `0`. |
| Observable | Every column family in the process shares **one** block cache. `rocksdb.block-cache-capacity` reads the same on every CF, and that capacity is `block_cache_size + memtable_budget` — the *sum* of the two operator knobs, because memtable bytes are costed to the same cache ([FM-PERSISTENCE-060](#fm-persistence-060--memtable-memory-is-bounded-by-one-process-wide-write-buffer-manager-not-by-a-per-column-family-product)), so the sum is the single enforced number rather than two ceilings that add up behind the operator's back. With `block_cache_size_mb: 0` no cache is installed at all and RocksDB's own default (a private 8 MB LRU per table factory) applies — `0` means "engine default", never "a cache with no room in it". |
| NOT observable | A cache per column family, or a cache per `RocksStore` open. A capacity of `block_cache_size` alone while memtables are *also* costed to it, which would silently shrink the read cache by the memtable budget as writes arrived. A `0` that installs a zero-capacity cache: that is a cache which evicts every block it stores, and it is the failure mode the pinning test exists to prevent. A **strict-capacity** cache: `rocksdb 0.24` exposes no constructor for one (the C API has `rocksdb_cache_create_lru_with_strict_capacity_limit`, but `Cache` has no public raw-pointer constructor and the crate does not re-export `librocksdb_sys`), so the cache half is bounded by *eviction*, and this row does not claim a refusal it cannot deliver. What is refused instead is the `Budget` charge: `RocksMemory::refresh` reconciles the persistence charge against the engine's reported usage, and an over-budget reading is a `Refused` counted at `frogdb_memory_budget_refusals_total{subsystem="persistence"}`. **Operator note:** that reconciliation runs on a 10 s tick and counts the *condition*, not an excursion — while pinned usage sits above capacity every tick counts another refusal, so the counter climbs monotonically through one steady over-capacity period rather than stepping once per excursion. Alert on the condition (the gauge against capacity), not on the counter's rate, until [memory-architecture issue 31](../.scratch/memory-architecture/issues/open/31-budget-refusal-prefix-and-counting.md) changes the counting. Index and filter blocks remain outside the cap (`cache_index_and_filter_blocks` stays off, as before this row); that is a read-latency trade with its own evidence requirement. |
| Invariant | `RocksMemory::install` builds at most one `rocksdb::Cache` per `RocksStore` open, with capacity `RocksConfig::memory_budget_bytes()` (= `block_cache_size + memtable_budget_bytes()`), and only when `block_cache_size > 0`; `BlockBasedOptions::set_block_cache` is called once on the shared `block_opts`, which is installed on the one `cf_opts` every CF descriptor clones (the cache *is* a column-family concern — it reaches RocksDB through the table factory, unlike the write-buffer manager, which is DB-level). The cache and the manager are held on `RocksStore` for the lifetime of the DB — `rocksdb`'s `OptionsMustOutliveDB` rule — so neither can be dropped while a column family still points at it. `block_cache_size == 0` takes the other arm: no `set_block_cache` call, and the manager is built standalone (`new_write_buffer_manager`) so memtables are still bounded even with no cache to cost them to. |
| Outcome variant | `frogdb_rocksdb_block_cache_bytes` / `frogdb_rocksdb_block_cache_capacity_bytes` / `frogdb_rocksdb_block_cache_pinned_bytes` |
| Forced by | `the_memory_budget_is_the_sum_of_the_two_operator_knobs`, `block_cache_capacity_is_the_summed_budget`, `block_cache_size_zero_leaves_the_rocksdb_default`, `every_column_family_shares_one_block_cache`, `an_over_budget_engine_footprint_is_refused_and_counted`, `the_charge_follows_the_engine_footprint_up_and_down`, `the_persistence_charge_tracks_the_reported_engine_footprint`, `the_store_metrics_tick_publishes_the_engine_gauges` |
| Bug refs | [memory-architecture issue 06](../.scratch/memory-architecture/issues/done/06-rocksdb-jemalloc-budget.md) (PRD ruling R10) |
...
| Invariant | The order is the body of `recover`, readable top to bottom, with the three real dependencies stated in the phase modules: the marker verdict must precede everything that writes; the install is filesystem rename surgery that must precede the open; and the install is what carries `replication_metadata.json` into the data dir for phase 5 to find. Phase 0 is both halves of the verdict — decide, then create the directory and stamp it — and *both* run before the install, so nothing is ever renamed into a directory that does not yet carry its identity ([FM-PERSISTENCE-059](#fm-persistence-059--identity-is-stamped-before-the-install-and-an-install-waiting-to-finish-is-not-an-empty-directory)). Phases 0-4 are gated on RocksDB-backed persistence; phase 5 is role-gated and phase 6 cluster-gated, both independent of persistence. |
...
| Invariant | `frogdb_persistence::data_dir::foreign_files` walks the tree and collects *files* at any depth; directories recurse, a missing directory contains nothing, and symlinks count as content without being resolved. It skips exactly the top-level entries FrogDB itself owns (`DataDirLayout::is_own_artifact`) — the install scratch the layout names, `backup`, `staging`, and `staging.*` ([FM-PERSISTENCE-057](#fm-persistence-057--staging-and-backup-live-inside-the-data-directory-the-mount-point-is-never-renamed)), plus the identity marker `frogdb_data_dir` itself — and nothing else: a payload an operator staged into a fresh data directory is not evidence of a wrong directory, while `db/` is still scanned in full so an unmarked *database* refuses, and `frogdb_data_dir.tmp` is still content because a failed publish's leftover is not an excuse ([FM-PERSISTENCE-049](#fm-persistence-049--the-data-directory-carries-an-identity-marker-published-atomically)). The marker is excused because the only branch that reaches the probe with a marker present is the unreadable-marker one, where counting FrogDB's own byte as foreign would leave no directory the fresh-start flag could re-stamp ([FM-PERSISTENCE-050](#fm-persistence-050--an-unreadable-or-future-layout-marker-refuses-the-boot)). `NotFound` is the only listing failure that answers "no files": every other one (a `data-dir` that is a file, a permissions error, an IO error) propagates, because a directory whose contents are unknown must not read as empty. The walk stops at the caller's `limit` — a first-`limit`-files probe, so the cost is bounded by the answer rather than by the tree — and the names it returns are relative to the data directory, which is what lets a refusal say *which* entries are in the way ([FM-PERSISTENCE-051](#fm-persistence-051--a-populated-data-directory-with-no-marker-refuses-the-boot)). The rule is deliberately about what *this process did not write* rather than an allowlist of tolerated names, which would need an entry per orchestrator. |
| Outcome variant | n/a (the `RecoveryPhase::VerifyDataDir` success path) |
| Forced by | `contains_files_counts_files_at_any_depth_and_ignores_empty_directories`, `contains_files_treats_a_symlink_as_content`, `contains_files_propagates_a_listing_failure_that_is_not_absence`, `foreign_files_ignores_the_install_scratch_but_not_the_database`, `foreign_files_names_entries_up_to_the_limit`, `a_fresh_data_dir_boots_and_stamps_the_marker`, `pre_created_empty_subdirectories_are_still_a_first_boot`, `fresh_boot_creates_empty_shards`, `foreign_files_reports_a_missing_directory_as_empty`, `an_unreadable_directory_must_not_read_as_empty` |
| Bug refs | `.scratch/hardening/issues/done/11-wrong-data-dir-boots-as-a-fresh-database.md`; `.scratch/spec-gaps/issues/done/32-force-fresh-data-dir-refuses-foreign-entries.md` (R6: the flag becomes a fresh-start tool, not an override) |

## FM-PERSISTENCE-049 — the data directory carries an identity marker, published atomically

| Field | Value |
|---|---|
| Trigger | Any RocksDB-backed boot: the marker is (re)written in `RecoveryPhase::VerifyDataDir`'s second half, before the staged-checkpoint install. |
...
| Forced by | `marker_round_trips_through_an_atomic_publish`, `minted_markers_are_unique_and_carry_the_current_layout`, `a_missing_marker_is_absent_not_an_error`, `a_failed_stamp_leaves_no_temporary_file`, `an_installed_checkpoint_leaves_the_data_dir_marked`, `the_data_dir_is_stamped_before_the_install_can_touch_it` |
...
| Observable | Startup fails during `VerifyDataDir` naming the resolved absolute path, the underlying cause, and `--force-fresh-data-dir` as the way to re-stamp a directory that holds nothing but FrogDB's own artifacts. With the flag and such a directory the read error is a `WARN` and the marker is re-minted; with the flag and entries FrogDB did not write, the boot refuses anyway and the refusal names them. A marker from an *older* layout reads normally — reading an older layout is the migration path. |
| NOT observable | "Unreadable" collapsing into "absent". That collapse is the fail-open bug in miniature: the absent branch is the one that initializes a fresh database, so a single corrupt byte in the marker would license overwriting a real database. A directory left permanently unbootable *by its own marker* — the flag re-stamps rather than requiring the flag forever, once the directory is FrogDB's own. A future layout being opened under this build's rules and hoping. A re-stamp minting a FrogDB identity over somebody else's bytes: an unreadable marker says nothing about who wrote the rest of the directory, so the re-stamp is gated on the same foreign-entries probe as [FM-PERSISTENCE-051](#fm-persistence-051--a-populated-data-directory-with-no-marker-refuses-the-boot) (R6). |
| Invariant | `DataDirMarker::read` returns `Ok(None)` for `ErrorKind::NotFound` and nothing else; every other outcome is a `DataDirMarkerError` variant, and the phase treats all of them as refusals. Unknown *fields* are deliberately tolerated (forward compatibility is `layout_version`'s job, not `deny_unknown_fields`'). Under `--force-fresh-data-dir` the read error is downgraded to a `WARN` and the marker re-minted **only** when `foreign_files` reports the directory excused-only; otherwise the refusal stands and names the entries. The marker file itself is excused by that probe ([FM-PERSISTENCE-048](#fm-persistence-048--a-genuinely-empty-data-directory-is-a-first-boot-and-files-are-what-empty-means)) — an unreadable marker is still FrogDB's own byte — which is what leaves the re-stamp reachable at all. |
| Outcome variant | `RecoveryPhase::VerifyDataDir` / `DataDirMarkerError::{Unreadable, Malformed, FutureLayout}` |
| Forced by | `an_io_error_other_than_not_found_is_an_error_not_an_absent_marker`, `a_malformed_marker_is_an_error_not_an_absent_marker`, `a_marker_from_a_future_layout_is_refused`, `a_corrupt_marker_refuses_the_boot`, `force_fresh_data_dir_re_stamps_a_corrupt_marker`, `force_fresh_data_dir_refuses_to_re_stamp_beside_foreign_entries`, `foreign_files_ignores_the_install_scratch_but_not_the_database` |
| Bug refs | `.scratch/hardening/issues/done/11-wrong-data-dir-boots-as-a-fresh-database.md`; `.scratch/spec-gaps/issues/done/32-force-fresh-data-dir-refuses-foreign-entries.md` (R6: the flag becomes a fresh-start tool, not an override) |
...
| Field | Value |
|---|---|
| Trigger | Boot against a directory that holds entries FrogDB did not write but carries no FrogDB marker: a mistyped `persistence.data-dir`, a container that lost its bind mount, a volume mounted somewhere else, a directory that belongs to another program — or a FrogDB database written before markers existed. `--force-fresh-data-dir` does not change the trigger; it is not an escape from this row. |
| Observable | Startup fails during `VerifyDataDir` before anything is written, naming the *resolved absolute* path (a relative `data-dir` is resolved against a working directory the operator may not know — which is the confusion this row exists to end), the marker file it looked for, **the offending entries** (relative to the data directory, up to eight and then "and more"), the three shapes this failure usually is, and the way out: point `data-dir` elsewhere, or move those entries away. The message says in so many words that `--force-fresh-data-dir` does not override this. Nothing is created, nothing is renamed, nothing is stamped — with or without the flag. |
| NOT observable | The original bug: "nothing decoded here" and "I am not looking where you think I am" reading as the same observation, so a wrong `data-dir` came up as a healthy empty keyspace and the WAL/snapshot cadence began persisting over the real state ([FM-PERSISTENCE-029](#fm-persistence-029--an-empty-data-directory-boots-fresh-instead-of-failing) is the *genuinely* empty case; the column-family guards of [FM-PERSISTENCE-030](#fm-persistence-030--a-shard-count-change-refuses-to-start)/[031](#fm-persistence-031--disabling-the-warm-tier-on-a-tiered-database-refuses-to-start)/[032](#fm-persistence-032--a-failed-column-family-enumeration-is-never-coerced-into-fresh-database) cannot help, because they compare a *persisted layout* against the configured one and a directory that was never FrogDB's has no layout to disagree with). A *replica* refusing only after a full resync has already repopulated the directory — that is not a refusal, it is the operator's data replaced with the primary's. **An adopt path.** `--force-fresh-data-dir` used to turn this refusal into an adopt-and-restamp; R6 removed it, because "this really is my first boot" and "these bytes are not mine" are indistinguishable to a flag, so one flag must not resolve both — CockroachDB refuses to init into a directory that is not its own for the same reason. The flag deleting anything, then or now: the operator moves the foreign bytes out and retries. A persisted `force-fresh-data-dir = true` disabling the guard forever — the flag is a CLI flag with no config-file or environment spelling, and no longer disarms this guard at all. A refusal that says only *that* the directory is occupied: it names what is in it, because "which directory am I actually looking at" is the question the operator has to answer. |
| Invariant | The phase runs *first* in `recover`, ahead of the staged-checkpoint install, ahead of the RocksDB open, and — because recovery completes before `init_replication` dials anyone — ahead of any resync. The refusal is unconditional on `force`: `verify` bails whenever `foreign_files` returns a non-empty list, and `force` is consulted only by [FM-PERSISTENCE-050](#fm-persistence-050--an-unreadable-or-future-layout-marker-refuses-the-boot)'s re-stamp and [FM-PERSISTENCE-052](#fm-persistence-052--persistenceequire-existing-data-turns-an-empty-directory-into-a-refusal)'s empty-directory refusal, both of which are themselves gated on the directory being excused-only. `force_fresh_data_dir` is `#[serde(skip)]` on `PersistenceConfig` and set only from `main.rs`, so `deny_unknown_fields` makes a config-file attempt a loud error rather than a silent, permanent disarm. Pre-release: this is a breaking change to a shipped flag, taken deliberately (R6). |
| Outcome variant | `RecoveryPhase::VerifyDataDir` |
| Forced by | `an_unrelated_file_in_the_data_dir_refuses_the_boot`, `the_data_dir_guard_runs_before_a_staged_checkpoint_can_install`, `force_fresh_data_dir_refuses_beside_foreign_entries`, `force_fresh_data_dir_is_not_settable_from_the_config_file`, `foreign_files_names_entries_up_to_the_limit`, `an_unreadable_directory_must_not_read_as_empty`, `a_refusal_names_a_handful_of_entries_then_says_and_more` |
| Bug refs | `.scratch/hardening/issues/done/11-wrong-data-dir-boots-as-a-fresh-database.md`; `.scratch/spec-gaps/issues/done/32-force-fresh-data-dir-refuses-foreign-entries.md` (R6: the flag becomes a fresh-start tool, not an override) |
...
| NOT observable | A default that refuses first boots. The knob exists because of an honest limit: an unmounted volume presents as an empty directory and is **indistinguishable from a first boot by inspection** — no marker can be read from a filesystem that is not there. Only the deployment knows it is past its first boot, so only the deployment can say so. A `require-existing-data` node that can never be provisioned — the flag is the provisioning path, and stamping the marker retires it after one boot. The flag carrying that escape any further than this row: it does not reach [FM-PERSISTENCE-051](#fm-persistence-051--a-populated-data-directory-with-no-marker-refuses-the-boot)'s refusal (R6), and the two never collide because a directory with no foreign entries is exactly the directory this row is about. |
| Invariant | The empty-directory refusal is checked after the marker read and after the foreign-entries probe, so a marked directory satisfies it without consulting the setting at all, and a directory holding somebody else's bytes has already refused under FM-PERSISTENCE-051 before the setting is read. `--force-fresh-data-dir` short-circuits *this* refusal and only this one. An unfinished install satisfies it too, and only it reaches the extra probe — the setting is the only reader of `pending_install` ([FM-PERSISTENCE-059](#fm-persistence-059--identity-is-stamped-before-the-install-and-an-install-waiting-to-finish-is-not-an-empty-directory)), so an ordinary boot pays nothing for it. The setting is a plain config param (settable from file and environment); the flag is not, for the reason in FM-PERSISTENCE-051's invariant. |
| Outcome variant | `RecoveryPhase::VerifyDataDir` |
| Forced by | `require_existing_data_refuses_an_empty_data_dir`, `force_fresh_data_dir_overrides_require_existing_data`, `an_install_waiting_to_finish_satisfies_require_existing_data` |
| Bug refs | `.scratch/hardening/issues/done/11-wrong-data-dir-boots-as-a-fresh-database.md` |

## FM-PERSISTENCE-059 — identity is stamped before the install, and an install waiting to finish is not an empty directory

| Field | Value |
|---|---|
| Trigger | A boot against a data directory whose only content is an install: a crash between the install's two renames (backup present, `staging` present, no `db/`), a crash or a failure between the install and the stamp, or the documented operator restore — a checkpoint copied into `<data-dir>/staging` on a freshly provisioned volume. |
| Observable | The verdict's second half publishes the marker (creating the data directory if this is a first boot) *before* the install renames anything, so the boot that finishes an interrupted install comes up carrying the `database_id` the directory had beforehand, with the pre-crash database still recoverable under `backup/`. Under `persistence.require-existing-data`, a complete staged payload or a `db_backup_*` directory satisfies the setting and the boot proceeds — logged as such — with no `--force-fresh-data-dir`. |
| NOT observable | A fresh `database_id` for a directory whose database was already there: identity may be minted once, never re-derived, so every re-mint presents the same data as a *different* database and defeats the cross-check the id exists for. A `require-existing-data` refusal standing between the node and an install it was about to complete — the only way past that refusal is the flag that mints, so the refusal and the re-mint are the same failure with one step in between. A database at `db/` in a directory with no marker as the *ordinary* outcome of a crash, which the next boot would refuse under [FM-PERSISTENCE-051](#fm-persistence-051--a-populated-data-directory-with-no-marker-refuses-the-boot). A half-arrived download excusing an unmounted volume: an incomplete `staging` is not data, and `require-existing-data` still refuses. |
| Invariant | `recover` runs `verify` → `stamp` → `install_staged`, and `stamp` is what creates the directory; nothing renames into a data directory that is not yet stamped. This is CockroachDB's `StoreIdent` rule — write identity first, read it before anything else, never re-mint it — applied to the data-directory marker. The emptiness question stays two questions on purpose: `foreign_files` answers "did somebody else write here?" and skips `staging*`/`backup`/the marker ([FM-PERSISTENCE-048](#fm-persistence-048--a-genuinely-empty-data-directory-is-a-first-boot-and-files-are-what-empty-means)), while `rocks::staged::pending_install` answers "is there an install here to finish?" and looks at exactly those names. Only the second one may satisfy `require-existing-data`, and only for a payload that would really install (`StagedCheckpoint::is_complete_db`) or a backup that really is a displaced database (prefix match, so a sequence that will not parse is not reported as absent — the mirror of prune's refusal, [FM-PERSISTENCE-026](#fm-persistence-026--exactly-one-previous-database-backup-survives-an-install)). |
| Outcome variant | `RecoveryPhase::VerifyDataDir` |
| Forced by | `a_crash_between_the_install_renames_boots_to_the_same_database`, `an_install_waiting_to_finish_satisfies_require_existing_data`, `the_data_dir_is_stamped_before_the_install_can_touch_it`, `an_unfinished_install_is_not_an_empty_data_directory` |
| Bug refs | `.scratch/spec-gaps/issues/done/20-mid-install-crash-recovery-preserves-database-id.md` (distsys-review MAJ-17) |
