...
- D2: the Unit Tests regressions (cluster handoff/migration/finalization tests) are filed as
  `.scratch/memory-architecture/issues/35` — a regression from that directory's issue 18 —
...
- D5: the issue 35 regression is fixed spec-first in `frogdb-cluster` as a derived budget —
  ...
  `cluster.max-threads` stays 2; no testbox confirmation; issue 36 (jemalloc arenas in test
  binaries) stays separate. Carved as memory-architecture/37, sequenced after 23.
