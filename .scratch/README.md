...
| [build-toolchain](build-toolchain/) | active | 1 | 0 | build/cross-compilation/release-path defects, where dev builds stay green but a shipping path is broken; opened 2026-09-02 with `just cross-build` failing on usearch under zig |
| [roadmap](roadmap/) | active | — | — | roadmap + unfinished/follow-up items (migrated from the retired `todo/`); not an issue tracker |

Totals: **106 open, 113 done**. `done/` holds only closed issues still referenced from
