# 37: slot-handoff drain/prepare budget derived from the heartbeat interval, not a fixed 50 ms

Status: needs-triage
Type: AFK
Origin: issue 35 investigation + build-toolchain D5, 2026-09-04
...
[issue 35](../done/35-cluster-tests-regressed-after-output-buffer-series.md).
...
one: `TRYAGAIN … source did not drain in 50ms`, then `SETSLOT NODE failed`. Issue 35 bisected
...
barrier is a new ruling (issue 35 "open rulings"; both CockroachDB and FoundationDB derive
...
cluster suite's green is attributable to one change. Issue 36 stays separate.
