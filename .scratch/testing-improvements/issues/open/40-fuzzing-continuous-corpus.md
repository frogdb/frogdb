...
## Reopened — 2026-07-28 (round-2 audit)

The resolution above landed in `81a4f910` and was then **regressed** by
`2e2ea8bb ci: run everything except the testbox on GitHub-hosted runners`, which deleted the
`schedule` trigger. Evidence:

- `.github/workflows/workflow_gen/src/workflow_gen/workflows/fuzz.py:16` now reads
  "**Campaign** (`fuzz-campaign`, manual dispatch — the nightly cron was removed)", and the
  module docstring's design section no longer mentions `schedule` at all.
- `.github/workflows/fuzz.yml` `on:` block is `workflow_dispatch` + `pull_request` only —
  no `schedule:` key. `git log -S'41 2 * * *'` on the DSL module returns exactly the two
  commits above (added, then removed).

Compounding failure — killing the cron silently disabled the PR gate too:

- `corpus-replay` (PR job) uses `actions/cache/restore` (`fuzz.yml:93`), restore-only by
  design so PRs never write cache entries.
- The **only** writer of the `fuzz-corpus-` cache was `fuzz-campaign` (`fuzz.yml:42`,
  combined `actions/cache`), which now runs only on manual dispatch.
- So with no scheduled campaign, no corpus is ever written; the PR replay restores nothing,
  degrades to its documented "cold corpus → no-op per target" path, and passes green while
  testing zero inputs. The regression gate reports success without executing a single case.

Net state: back to the original bug (34 targets effectively unfuzzed), but now with a
green PR check claiming otherwise — strictly worse than before `81a4f910`.

### Additional acceptance criteria for the reopen

- [ ] `schedule` trigger restored in the DSL module (not hand-edited in `fuzz.yml`), and
      `just workflow-gen --check` passes.
- [ ] `corpus-replay` fails loudly, or emits an explicit warning annotation, when the
      restored corpus is empty for **all** targets — a green replay over zero inputs must
      not be indistinguishable from a real pass.
- [ ] A regression guard so the next "move everything to hosted runners"-style sweep cannot
      silently drop a cron: assert the expected trigger set per workflow in the workflow-gen
      test suite.

Round-2 tracks the same work as infrastructure item **I10** (`.scratch/testing-improvements-round2/INFRASTRUCTURE.md`, tier C).
Do not file a new issue for I10 — it is this one.
