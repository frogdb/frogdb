...
There is a known, already-filed real bug — `.scratch/concurrency-testing/issues/11`
...
- [x] Nightly `ops_per_client` cap revisited/raised once this guard plus the issue-11 fix are both
      ...
      `.scratch/concurrency-testing/issues/11`) — **satisfied 2026-07-31**, cap raised
      75 → 150; see the addendum below

## Blocked by

Related to (not blocked by) `.scratch/concurrency-testing/issues/11` —
...
- `.scratch/concurrency-testing/issues/11` — Finding A (the underlying bug)
