# Slot migration redesign — source-authoritative-until-commit (v33)

Status: revision 33 — **candidate, UNSOUND-pending**: not approved, and not
claimed sound until a review round finds nothing structural. Review v33 (of
revision 32) found **0 CRITICAL** / 5 MAJOR / 4 MINOR — an **eleventh**
consecutive zero-CRITICAL round, and the **twenty-fourth** rung of the fix-text
...
and this changelog. **Every published number of revision 32 re-derived exactly
against revision 32** — six strict families × four layers (24/24), the three
residue commands × five layers (15/15), every probe, both subtraction
decompositions, all three whole enumerations, the membership cross-check
(**4**), the containment facts and both ladders. **All five MAJORs are in the
labelling and disposition layer, not the arithmetic**: revision 32 measured what
it said it measured, but three of the five sentences describing *which sites*
those measurements landed on were falsifiable against this file, and one of the
three was copied word for word out of the v32 review's own prescribed fix.
**V33-M1 (MAJOR)** — the containment argument's two worked exhibits were sited
by the **wrong changelog blocks**. The extended stage-variant span was
attributed to the v23 block, which carries no family-D hit at all; the plain
variant was attributed to the v26 block. The spans are in the **v26** block's
summary of V24-M3's finding and in the **v20** block's V20-C1
*premise-withdrawn* marker respectively, verified line-for-line. The labels were
adopted **verbatim from the v32 review's own prescribed-fix sentence** — a
reviewer-as-source rung, recorded as such rather than smoothed over, because a
ladder that re-labels its own instances stops counting. Both exhibits are
re-sited, the provenance is
recorded at the exhibits, and the two `(m)` blocks are now listed as
**load-bearing meta** in the standing obligations: a changelog block that an
argument quotes cannot be pruned without re-deriving the argument.
**V33-M2 (MAJOR)** — the `Pi` residue paragraph restated its meta count as
**24** four lines below the enumeration sentence that says **26** and the layer
row that measures 26. The prose number is corrected, and a **prose-restatement**
obligation is added: when a layer moves, every prose sentence that repeats that
layer's number must be re-read, not only the table cell. A sweep of every prose
restatement of a layer count against the final tables is part of this revision's
re-run.
**V33-M3 (MAJOR)** — the widened F probe's non-meta list named **gate 3 (d)'s
producer paragraph**, which carries no occurrence of the narrowed spelling, and
omitted **gate 3 (a)'s fence-operand paragraph**, which carries one. The list is
corrected, every entry re-anchored by re-reading the line it names, and the
co-location sentence that had inherited the mis-anchoring is rewritten: two of
the seven sit on a family-F hit line, three sit one line from one, and **two sit
beside none**.
**V33-M4 (MAJOR)** — the widened probe dispositioned its seven non-meta hits
**all `(a)`** while family F's own entry disposes five of the same sites `(b)`
for the same repair: one table said both things about one site, which is the
exact category V31-M1 was raised for. All seven are **`(b)`, revision 28**.
Revision 27 carries the absolute spelling at the ancestor of **every one** of the
seven and the narrowed spelling at **none** of them, and the v28 entry below
records the narrowing at **seven** sites in its own words. Family F counts five
because only five repairs left a copy of the absolute phrase behind; two
replaced it outright, which subtracts from F's count without making the site
unrepaired. The rule is stated with the disposition rule: **a narrowing repair
transfers `(b)` to whichever command spells the narrowed text**, because `(a)`
and `(b)` attach to the *site*, not to the occurrence that happens to match.
(The v33 review prescribed a five-`(b)`-plus-two-`(a)` split, calling two of the
sites narrowed from birth; revision 27 falsifies that, so the split is not
taken.)
**V33-M5 (MAJOR)** — `Pv`, the residue command revision 32 added so that the
survives-voice class would be measurable, **did not measure its own class**: all
three of its alternations were written singular, and the plural inflection of the
head noun escapes every one of them. Five folded occurrences sit outside its hit
set, four of them non-meta and three of those four reached by no recorded
command at all — one being §0's live statement of the corrected premise, in the
sentence that settles which case this design is in. The
first two alternations now carry the plural; the plural of the third was measured
and reads **zero** at every layer, so it stays singular and the measurement is
recorded rather than the guess. All five layers are republished, the hit set is
re-enumerated whole, and an **inflection check** — plural and participle of the
head noun, before a residue command is published — joins the standing
obligations.
Minors, all applied: **m1** seven false **section anchors**, all of the form
"§0's gate-1 …" for a gate that lives in *Replicated migration record*, not §0.
This is the **third consecutive round** of the anchor class (V30-m3, V32-m1),
so the section-anchor check is promoted from a repeated fix to a standing
obligation. **m2** the two extended containment spans were called **normative**;
both are inside dated changelog blocks and are therefore `(m)`, corrected in both
places the claim appears. **m3** `Pv` reached the **preserve** voice only by
co-location with a hit on the same 12,000-character table row; a fourth
alternation is added so the voice is reached by pattern instead of by luck, in
preference to rewording a row that many other counts depend on. **m4** the
**toolchain** is now recorded beside the deviations: counts are produced with
BSD `grep` under `LC_ALL=C` from the repository root, but the requirement is the
POSIX **leftmost-longest** semantics rather than that binary — a leftmost-first
engine answers the containment questions wrongly — and review v33 re-ran
thirteen of the recorded commands under a different POSIX engine with **13/13**
identical figures.
**One revision-32 disposition is overturned by this revision**: the widened F
probe's seven non-meta hits move from `(a)` to `(b)` (V33-M4). Every other part
(i) and part (ii) disposition stands, and every previously published number that
moved did so only because this changelog and the rule block grew.
...
Dated 2026-08-18. Review v32 (of
...
revision 31 and **fourteen** at 32, seven non-meta throughout, including the **definitional** statement at gate 1's
...
**Two folded spans of family D are extended this way, both of them `(m)` sites
inside dated changelog blocks** (the v26 block, `restarts the node-local` inside `restarts the node-local stage counter`;
the v20 block, inside `restarts the node-local counter`) — **six** at revision 32 once this
...
**The sweep's mechanism passed a second time, and its naming passed for the
first time**: all six recorded commands re-derived their published totals at all
four layers, and the named-site ↔ hit-line correspondence — the check that had
failed three rounds running — held exhaustively across both parts and both
`(a)`/`(b)` lists. **Both MAJORs are in one organ**: part **(ii-b)** of the
sweep table and the V30 changelog's summary of it, and both are false claims
about this document's own content — the family the last four rounds have found,
now one layer further in.
**V31-M1 (MAJOR)** — part (ii-b) said the **revision-24 changelog block**'s
V24-M3 entry "quotes the withdrawn premise inside the entry that withdraws it".
It does not. That entry states the **corrected** fact — "neither mandated
sequence restarts the stage counter" — in the same negated construction as §0's
bullet listed immediately above it, and the list's own header called all three
of its entries statements of the corrected fact. So one list said both things
about one site. This is not a wording slip:
quotation-inside-a-withdrawal is a **disposition-bearing** category here — it is
the stated ground of family A's blast-radius revision-20 `(a)` and of family C's
`(a)` class — so attributing it to a site whose text falsifies it is a false
disposition claim in the one table whose whole purpose is that dispositions be
checkable against text. The entry now says what the site does, the V30
changelog's repetition of the false half is corrected with it, and a **genuine**
instance of the construction is named in its place.
**V31-M2 (MAJOR)** — the V30 changelog claimed part (ii) "enumerates **in
full**". Part **(ii-a)** did; part **(ii-b)** did not. (ii-b) asserted "three"
statements of the corrected fact in a spelling no family carries, found by
**reading**; eleven further live sites sat in the same normative or model-level
register — gate 1's wipe-sequence derivation (**the site the rest of the
document defers to**), §0's two-write clearing rule, §0's necessity
re-derivation, harmlessness paragraph, (β)-path re-derivation and
retired-enumeration paragraph, the record-binding row, two *Quint* paragraphs,
the *Testing* forcing-test producer, and the gate-1 V24-M3 sweep passage.
Family D requires `restarts`; these spell
it `restart`/`restarting`/`restarted`, invert it, or change the subject to the
**generation**. An exemplar set presented as an enumeration is the exact
...
deviation** — the `folded` command's space squeeze and family E's bounded
wildcard — while asserting the hardening in a form that family E's own published
numbers contradict (folded **11** below occurrences **12**); both deviations are
now named in that block with a pointer to the rule that states them in full, and
the "every occurrence beyond `occurrences` is a site the line command cannot
see" claim carries its exception, so a reader auditing revision 30 from its
changelog is not stranded; **m2** family **F** called the *Transitions*
paragraph "gate 1's counter-survives re-derivation" while families **A** and
**D** call the same paragraph "the deleted-write re-derivation" — gate 1's
derivation is a different passage, in §0 — so family F now uses A and D's name;
the correspondence itself was intact and did not change.
**No revision-30 disposition is overturned by this revision.** Every part (i)
disposition stands; part (ii-b) gains sites and gains a mechanism, and the three
history-block sites revision 30 dispositioned by reading keep their `(c)`,
`(c)` and `(a)`.
**The cumulative blast radius is unchanged by this revision** — still five rows
Amended across six declared amendments carrying seven invariant deltas, none
performed here. **No LOCKED spec file is edited by this revision.**

Dated 2026-08-18. Review v30 (of
...
`occurrences` is a site the line command cannot see, **except where a bounded
wildcard merges two line-matches into one**, and each is enumerated in
...
**Two deviations are recorded with the rule, and both are named here so a
reader auditing this revision from its changelog is not stranded** (**V31-m1**,
which found this block naming neither): **(d1)** the `folded` command squeezes
runs of spaces (`tr -s ' '`) — **required**, not cosmetic, because this file is
indented and an unsqueezed fold leaves a wrapped phrase separated by the next
line's indent and therefore finds nothing; **(d2)** family **E**'s folded
pattern **bounds** its wildcard to at most eighty characters, because `-o` over
a single folded line makes an unbounded wildcard greedy across the whole file —
and the consequence is that family E's folded number comes in **below** its
occurrence count (**11 against 12** in this revision), since one bounded
leftmost-longest match can swallow a following one. That is the exception to
"every occurrence beyond `occurrences` is a site the line command cannot see":
where it bites, the family entry locates the merge and shows both halves are
`(m)`. Both deviations are stated in full at the rule.
...
neither restarts the stage counter", the v24 changelog block's V24-M3 entry
**stating the corrected fact** in the *stage*-qualified spelling family D does
not carry, and the Quint state bullet's "Neither mandated wipe sequence
restarts the real counter") — with the folded command now mechanical, part
**(ii-a)** enumerates **in full** rather than by exemplar. **Both halves of that
sentence were wrong as revision 30 wrote them, and are corrected here by
V31-M1 and V31-M2**: revision 30 described the v24 site as "the v24 changelog's
quotation of the withdrawn premise", which the site's own text falsifies — it
states the negation, not the premise — and it claimed the whole of part (ii)
enumerated in full while **(ii-b)** was still three exemplars found by reading,
missing **eleven** further live sites in the same register (and one hit in an
unrelated sense). **m3** every "§1" anchor that
...
moved again: no settled ruling was touched, no LOCKED row was edited, the
record-binding conjunct (V20-C1) and its four-artefact witness stand as revision
28 left them. **Every one of the five MAJORs is in the apparatus, and three are
in the apparatus revision 28 built to stop exactly this class of failure** — the
mechanical scope sweep, whose whole premise is that it is re-runnable and which
did not re-derive.
**V29-M1 (MAJOR)** — the six-family sweep's disposition table did not re-derive:
four of six family counts disagreed with its own declared revision-27 baseline,
all six disagreed with the file the rule names ("grep **this file**"), the table
listed sites the stated patterns do not produce, and it omitted one the patterns
do. **The table is rebuilt from scratch**, re-run against **this file** after
every edit of this revision, with the **exact command line recorded beside each
family**, the counting unit stated, and the hits split into two parts: **grep
output** (dispositioned one by one) and **sites found by reading that the stated
patterns do not catch** (listed separately as evidence the phrase families are
incomplete, never folded into the grep counts). The rule gains the three
statements that make a count checkable — counting unit, file, and the convention
for hits that live inside the rule, the table and the changelog (a fourth
disposition, **(m)** meta, which is counted and disposed as a class). The
revision-29 run, reproduced at the table with its commands: **A 78 = 39/13/0/26,
B 5 = 2/0/0/3, C 17 = 7/0/0/10, D 16 = 4/0/0/12, E 8 = 2/0/0/6, F 11 =
3/5/0/3**, plus **four** part-(ii) sites the patterns miss.
**V29-M2 (MAJOR)** — the sweep's single `(c)` disposition was wrong in both
dimensions: the site is §1 normative argument text, not a history entry, and
revision 28 had **struck** its defective disjunct and added the label, which is
the table's own definition of `(b)`. **Disposition overturned to `(b)`**, and the
rule now requires every `(c)` entry to **name the changelog block** the
historical text belongs to, so "history entry" is checkable rather than asserted.
**V29-M3 (MAJOR)** — the record-binding conjunct's fence-freshness row still
carried the struck producer disjunct ("durability loss **or** out-of-band
restore") **unlabelled, in normative fence-operand text**, under an `(a)`
disposition revision 28 refused to grant the identical phrase at three other
sites. **Disposition overturned**; the cell now carries V28-M1's own repaired
wording — the producer is a node-local **durability loss**, and the restore is
named only to say it rewinds the same value and is excluded **by the act**. The
row is also a family-B hit and is dispositioned as one.
**V29-M4 (MAJOR)** — V28-M3 repaired Table A *to a rule §0 does not state*. §0's
clearing rule wrote `stage_counter_state = Trusted { value: 0 }` alone, while
Table A, the V20-C1 fixture's `max(0, 0) + 1 = 1`, the necessity trace's
*restarted counter* and ext-18 all rest on a second write, `stage_counter = 0`.
The disagreement V28-M3 closed had been re-opened in the opposite direction, with
the table rather than the rule carrying the load. **The repair is completed at
the rule**: §0 now writes **both fields in one atomic write**, its existing
two-path soundness argument is stated to be argued of both fields at once, and
the two-write form is propagated to the boot partition's S-a/S-b/S-c and
non-member cells and to gate 3 (d)'s trace. The fixture's arithmetic is
**unchanged** — it becomes derivable rather than asserted. (This is an
**inversion**, and it is recorded as one: revision 28 corrected the table to a
rule the normative section did not state; revision 29 completes the repair where
the rule is.)
**V29-M5 (MAJOR)** — V28-M11's rewind-versus-destruction discriminator was
carried to two of the exclusion's three statements. The third — the
record-binding conjunct's fence-freshness row, the site §0's own scope sentence
delegates to — still stated the **unnarrowed** act criterion (which excludes this
design's own mandated `<data-dir>/raft` removal, the sequence its recovery rows
prescribe and its forcing test drives) and **positively denied** the
discriminator's value-referencing form. Both defects repaired at the row, with
V28-M9's point kept in its correct scope (the exclusion turns on the **act**
rather than on the **class of id**, while the act itself is identified by whether
it rewinds a spent-or-floored durable value or destroys a vehicle). A **standing
cross-check** is added at the fence-freshness discipline: the exclusion has
exactly **three** statements, named by content anchor, and a revision touching
one touches all three.

Minors, all applied: **m1** the fix-text ladder stated **once**, as an explicit
list of rungs with their review numbers, every later ordinal read off the list,
and the contradicting prose ordinal ("twenty-first") removed — V28-m14's +1 was
right and its "resolves the whole ladder" claim was not; **m2**
`inv_no_permanent_fence` replaced by `witnessFenceClears` at the one live site
that outlived the restatement; **m3** V28-m3's own entry de-garbled (a clause
truncated mid-phrase and a restarted sentence); **m4** the `ResetCluster`
**sub-span** citations unified to the three the document's own note declares and
the tree confirms — `:833-854`, `:855-859`, `:826-829` — retiring `:855-858` and
`:832-855`; **m5** V19-m2's FM-CLUSTER-100 routing given the inline supersession
marker V22-m3's narrowing owes it, in the v20 entry's style; **m6** the dangling
ordinal into the retired enumeration ("the tenth member of the V24-M3 swept set")
replaced with a pointer to the disposition table.
**Two revision-28 dispositions are overturned by this revision**, both recorded
at their sites: the fence-freshness operand cell's `(a)` → **fixed** (V29-M3;
grounds: normative fence-operand text cannot be held to a weaker standard than
the changelog entries V28-M1 repaired), and family A's sole `(c)` → **`(b)`**
(V29-M2; grounds: the site is normative and its text was in fact changed).
**The cumulative blast radius is unchanged by this revision** — still five rows
Amended across six declared amendments carrying seven invariant deltas, none
performed here. **No LOCKED spec file is edited by this revision.**

Dated 2026-08-16. Review v28 (of
revision 27) found **0 CRITICAL** / 11 MAJOR / 17 MINOR — a **sixth**
consecutive zero-CRITICAL round. Nothing structural moved: the record-binding
conjunct (V20-C1), its producer, its class and its arm stand exactly as revision
27 left them, and no settled ruling was touched. What failed was the apparatus
*around* them — a scope sweep declared complete at ten sites that in fact had
twelve, a negative control still standing on a producer the scope excludes, an
untrusted-clear table cell writing one value where §0 writes two, and a Quint
mutation still asserting a harm the model's own guards make unreachable.
**Revision 28 rebuilds the fourth artefact and retires the enumeration.** The
witness set has **four** members — necessity trace, forcing-test fixture,
negative control, and Quint mutation (17) — and all four now cite the same
producer (a node-local counter loss whose multi-member exit is `FORGET` +
re-`MEET`), the same class (`ordering`) and the same arm (**4b**); revision 27
rebuilt the first three and called the set complete at three while the fourth
still asserted the struck harm. The scope sweep is no longer an enumeration:
it is a **mechanical, re-runnable rule** — six phrase families, three
dispositions, stated with the current disposition table — because a hand-counted
list of sites is a claim that goes stale silently, and this one did twice.
**V28-F0 (the round's scaffolding, and the entry the rest of this round hangs
off)** — five of the eleven MAJORs (V28-M1, M2, M9, M10, and half of M11) and
three of the minors (m1, m2, m12) are **residues of one sweep that was declared
complete and was not**: revision 26's scope sweep over the out-of-band restore.
Revision 27 declared its swept set had **ten** members; it has **twelve**. Rather
than correct the count a third time, revision 28 **retires the enumeration** and
replaces it with a **mechanical, re-runnable rule** — six phrase families
(**A** `out-of-band` / `out of band`; **B** `restored out of band`; **C** `below
a value` / `already spent` / `below the ids`; **D** `the wipe restarts` /
`restarts the counter` / `restarts its counter` / `restarts the node-local`;
**E** `no in-protocol operand` / `leaving no …operand`; **F** `touches no
filesystem`), each hit taking one of three dispositions — **(a)** correct as written, **(b)** repaired this round, **(c)**
historical and marked, never silently edited. The current disposition table is
stated at the sweep's own section (§ the "conservative rather than wrong"
paragraph) with per-family counts, and it is the artefact that replaces the
count: **A 54 = 47/6/1, B 2 = 1/1/0, C 10 = 9/1/0, D 8 = 6/0/2, E 3 = 2/1/0,
...
live table is revision 31's, run against revision 31 with its commands recorded.
...
running it rather than of trusting the last person who counted.

**V28-M1 (MAJOR)** — gate 3 (a)'s fence-operand paragraph still carried the
out-of-band restore as a **load-bearing disjunct** of the producer, at the very
site the other two sweep sites cite as their authority; revision 27 declared the
half dropped at three sites and executed it at two. Struck: the producer is a
node-local **durability loss**, and the restore is named only to say it rewinds
the same value from outside the failure model, so nothing rests on it. The
paragraph also carried the qualifier V27-M3 withdrew ("from a copy whose counter
reads below the ids already spent"), which goes with it.
**V28-M2 (MAJOR)** — the §0 counter bullet still gave the **withdrawn reason**
("no in-protocol operand survives the restore"), attributed to the very bullet
that withdrew it as false. Replaced by the act-based exclusion, with the
mint-time floor named as the belt that survives either producer.
**V28-M3 (MAJOR)** — Table A's *Untrusted clear* row wrote
`stage_counter_state = Trusted { stage_counter }` where §0's rule writes
`Trusted { value: 0 }` **and** `stage_counter = 0`. Two different writes, and
this table is the artefact whose entire purpose is to be the single enumerated
statement of what each writer writes. §0's form is the one with a soundness
argument attached and the only defensible one — the state exists precisely
because the counter is *not* trusted, so adopting its value as `Trusted` would
launder it through the exit. The V20-C1 fixture now cites this row for its `0`
operand rather than restating it.
**V28-M4 (MAJOR)** — the boot-ordering rule and the pending-record reconcile
guard were each stated alone and prescribed **different things** at exactly the
state the V20-C1 forcing fixture pins, with no stated resolution. Resolved by
**precedence** (the boot report wins; the fixture's `S1'` `Demotion` report is
**held unproposed** until the boot report applies) rather than by forbidding
staging before boot-apply — staging is node-local and proposes no Raft command,
so the boot rule, which governs *proposals*, never reaches it. Same entry, the
two **edge-shaped damping rules** — `ReportRunIdentity`'s single-proposal rule
and `AttestReplicaSynced`'s trigger — gain an explicit **re-arm on
non-observation**, because a rule shaped over an outcome a dropped proposal
never delivers is a rule settled ruling 3 (*drops-stop-signaling*) forbids.
Both re-arms are level-triggered liveness rules with **no wall clock and no
deadline**.
**V28-M5 (MAJOR)** — the terminal lost-registration reply named "the lost
registration" and never said *which*, so the negative control asserted an
observable no rule determined. The operand is bound at the arm:
**`refused_payload.observed_registration_seq`**, and its agreement with the
staging clause's `record.staged_registration_seq` is now **derived** from the
record-binding conjunct rather than asserted.
**V28-M6 (MAJOR)** — Quint mutation (17) still asserted the **struck**
destructive-adoption harm, which ext-18's own `completeAdoption(n)` guard makes
unreachable, so the mutation **killed nothing** — the exact modelling defect the
section's evidence bar names. Rebuilt: the kill is the foreign disposition
itself, the property is the **new** `inv_no_live_record_disposed_by_a_foreign_refusal`
(the record-binding conjunct's attribution modelled as a *property*, not only as
the guard's own condition), and the schedule now names the **class** (`ordering`)
and the **arm** (**4b**, absent stored run identity). What the mutation does not
evidence — an acked-write loss — is stated. The witness set is counted at
**four** artefacts throughout, and `wipe(n)` is relabelled from "declared
over-approximation" to what it now is: the modelled form of the forcing test's
own producer.
**V28-M7 (MAJOR)** — the normative `parent_seq` **reading discipline** still
named `run_identity` as the mandatory pairing partner, ten lines below the
passage that removed it (V18-C1) and in contradiction with its own
fence-freshness row. A reading discipline is the sentence a future reader greps
before adding a second reader, so it was prescribing the defect: a
node-local-origin partner. Corrected to `nodes[n].registration_seq`.
**V28-M8 (MAJOR)** — the V22-C2 negative control was still driven by an
**out-of-band restore**, the producer §0 declares outside the failure model —
the same defect V27-M3 had already fixed in the forcing test one entry above.
Re-based on an in-model **node-local durability loss that loses the store's last
atomic write** (reads back valid, stamped at or above `N`, `Trusted { value: j }`
with `j < k`, `NodeInfo` cell intact) — the same producer *class* V20-C1's own
trace rests on. The re-basing makes a **joint C1 + C2 regression** expressible,
now asserted: §0's six-step composed trace cannot complete, at *either* of its
two independent kill points.
**V28-M9 (MAJOR)** — two sites said the refused-id **residue** was "declared
outside this design's failure model", transferring the exclusion from an **act**
to a **class of id**; read literally that deletes the counter-loss containment
this design calls "the state that cannot be created". Both restored to the
act-versus-value split.
**V28-M10 (MAJOR)** — two changelog entries stated premises later rounds
withdrew or overturned, with no inline marker: the v20 entry's restart premise
(withdrawn by V24-M3) and the v23 entry's "conservative rather than wrong" label
(overturned by V24-M3, which found half the sites *asserting* the restart as
fact). Marked at both, per the layered-history discipline. The sweep's own count
moves ten → twelve here, and then out of the document entirely per V28-F0.
**V28-M11 (MAJOR)** — the act-based exclusion criterion, as written, proved too
much: it also caught this design's **own** mandated out-of-band
`<data-dir>/raft` removal. Discriminator added — the excluded act is one that
**rewinds** a durable value the protocol has already spent or floored against;
it is **not** an out-of-band **destruction of a vehicle**, which is what the (α)
sequence performs at a node already removed from membership and stopped.

Minors, all applied: **m1** the V20-C1 producer's "or out-of-band restore"
disjunct struck and labelled out of scope; **m2** the "wiped and re-staged"
vocabulary replaced with the producer V24-M3 established actually exists;
**m3** `inv_no_permanent_fence` restated as the bounded witness
`witnessFenceClears` (three model steps, no fairness quantifier, no deadline),
one statement instead of two; **m4** ext-18's refusal **observation and
disposition scheduled as one atomic action**, with the window that hides
declared in the structural-limits list; **m5** folded into M6's class-and-arm
naming; **m6** two FM-attribution errors corrected (FM-CLUSTER-057 was in no
revision's list; -059 is revision 23's, not revision 24's); **m7** the
`ResetCluster` apply arm cited with four different spans, all four corrected to
the span read from the tree, `frogdb-server/crates/cluster/src/commands.rs:816-862`;
**m8** the two "issue 29" referents qualified by tracker and topic;
**m9** "the outcome is the same in all twenty" corrected — the *classification*
is uniform, the outcomes differ — and the below-`N` S-e disposition stated to
restore the thirty-cell totality claim; **m10** the boot-ordering count stated
with its members, (0) and (i)-(v) including (iv-b); **m11** the `data_dir.rs`
attribution corrected (`:289-314` is `stamp_with`, not `stamp`) with the three
entities disambiguated; **m12** "touches no filesystem" narrowed at **seven**
sites — the review named three — to "touches neither the identity store nor its
counter", the absolute being falsified by V23-M2's own discard-mark amendment;
**m13** revision 13's finding count nine → ten; **m14** the consecutive-round
ladder resolved from the entries themselves (v21 describes the pattern without
numbering it, which makes v21 the twelfth and v22 the thirteenth — revisions
22-27 were each short by one, and no later entry numbers itself, so the single
correction resolves the whole ladder); **m15** V25-M1's promotion to **BLOCKING
prerequisite** carried, and the restatement replaced with a pointer per V24-m3's
stated-once rule; **m16** every bare-filename code citation given its
repo-relative prefix; **m17** the supersession marker moved **to** the
superseded assertion; **m18** (this round's own, the residual-B improvement) a
forward pointer from the narrowed absent-operand exception to fence-freshness
leg (a), the route the reader who needs it actually arrives by.
**The cumulative blast radius is unchanged by this revision** — still five rows
Amended across six declared amendments carrying seven invariant deltas
(FM-PERSISTENCE-048 one, -049 two, -057 two, -059 one, TR-CLUSTER-035 one),
none performed here. **No LOCKED spec file is edited by this revision**; the
amendments are declared, never applied.

Dated 2026-08-15. Review v27 (of
revision 26) found **0 CRITICAL** / 3 MAJOR / 3 MINOR — a **fifth** consecutive
zero-CRITICAL round — and **all three MAJORs sat in one organ**: the
record-binding conjunct's (V20-C1) supporting apparatus. The conjunct itself and
its §0 derivation were upheld; what failed was this document's demonstration
that the conjunct is *necessary* and its test that it is *forced*. Revision 26
re-based both onto an **out-of-band restore** of the node's directory and did
not re-check them against gate 3 (d)'s own arm table, so the necessity trace
claimed a harm the table makes unreachable, the negative control's two conjunct
forms landed on the **same arm**, and the producer sat outside the failure model
§0 declares. **Revision 27 rebuilt three artefacts — necessity trace,
forcing-test fixture, negative control — on one producer, one class and one
arm**, and declared the set complete at three; **the set has four members, and
revision 28 rebuilt the fourth** (V28-M6 — Quint mutation (17), which was still
asserting the harm this round struck).
**V27-M1 (MAJOR)** — the necessity trace at gate 3 (d) asserted that the
unpaired conjunct let a cross-registration refusal "bind the live record, clear
it, drop the fence and answer the client, after which the still-in-flight report
could be admitted". `RefusalClass` is *the first conjunct that failed in the
declared evaluation order*, so a cross-registration refusal's class is
`ordering` or `fence`, and arms **1 and 3 are class-gated to refuse-whole** and
therefore unreachable for it; the "could be admitted" clause was refuted by this
document's own fixture assertion two lines below it. The trace is **re-derived
against the arm table**: the class is `ordering` by the fence-freshness row's
**leg (a)** (a stale `Demotion` meeting a re-created cell whose `run_identity`
is still absent), the sub-case is **4b** (absent stored value), and the harm is
exactly 4b's declared disposition — a **live** record cleared fsynced, the
whole-node fence dropped, and the initiating client answered a **terminal**
lost-registration error, all on a **dead registration's** verdict. The "could be
admitted" clause is struck; the refusal-class join table's rationale is narrowed
from "arms 1/3/4b/5" to the arms a cross-registration refusal can actually
reach, **4b and 5**. The second candidate the review floated — an
in-registration refuse-whole mis-bind — is **examined and deliberately not
used**, with the reason recorded at the trace: it is an argument for the pair's
*first* component, not its second.
**V27-M2 (MAJOR)** — the negative control had no teeth. Under revision 26's
fixture ordering the dead refusal's class was `fence`, and `fence` selects
**arm 2**, where "the record and fence persist" — under **both** conjunct
forms, so the only divergence was a spurious re-proposal and the asserted
acked-write loss could not fire. This is V26-M3's vacuous control relocated
rather than closed. The fixture is rebuilt so the class reaches a **clearing**
arm: the refusal is delivered while the re-created cell's `run_identity` is
still **absent**, which is the fence-freshness row's own leg (a) — class
`ordering`, stored value absent, **arm 4b**. The control now diverges on the
terminal state *and* on the client's reply, and the selected arm is named **by
name** in the test's prose so it is checkable against the arm table rather than
against prose.
**V27-M3 (MAJOR)** — the record-binding row's **Scope** sentence attributed a
qualifier to §0 that §0 does not carry ("below a value the *surviving*
registration already spent"), and revision 26's fixture was admissible only
under that narrowed form; its in-doc reconciliation redefined the scope instead
of satisfying it. Both are resolved by **re-basing the fixture on the producer
that is inside the failure model** — the Scope sentence's own first clause is
"these rules cover counter **loss**" — namely a node-local **durability loss**
composed with §0's own mandated **multi-member exit** (`CLUSTER FORGET` at a
survivor, then re-`MEET`), which deletes the node's cell (so `admitted_stage` is
`None` and the mint-time floor has nothing of this node's to read), writes
`stage_counter_state = Trusted { value: 0 }` on the fresh registration, and
moves `registration_seq` 11 → 12 from the survivors' untouched generation (the
(α) shape). The invented reconciliation paragraph is **struck**, the qualifier
is removed from every restatement so all sites read alike, and **§0's stated
reason is repaired**: the exclusion of an out-of-band restore rests on the
**act** — an operator mutation of node-local durable state that bypasses every
interface this design fences — and *not* on the claim that every operand was
rewound with it, which is false in the multi-member case, `registration_seq` and
`admitted_stage` being replicated and re-derived from the cluster untouched. The
forcing test is renamed
`refusal_of_a_dead_stage_cannot_bind_a_live_record_after_a_forget_and_rejoin`
at every site.
**V27-m1** — the solo-bootstrap containment cited `cluster_init.rs` lines that
carry nothing load-bearing; corrected at both sites to `:445` (the membership
read), `:447` (the guard) and `:453` (the guarded `initialize`). The substance
was verified correct and is unchanged.
**V27-m2** — the V24-M3 swept-set enumeration re-declared completeness at nine
and omitted the fourth site revision 26 actually edited; it is **ten** now, with
the revision-20 blast-radius entry named as a **history-entry correction**
rather than a normative site.
**V27-m3** — that history entry edited historical text without saying which
revision edited it; it is marked "corrected in revision 26 (premise withdrawn by
V24-M3)", matching the rename note four lines above it.
**The cumulative blast radius is unchanged by this revision** — still five rows
Amended across six declared amendments carrying seven invariant deltas
(FM-PERSISTENCE-048 one, -049 two, -057 two, -059 one, TR-CLUSTER-035 one),
none performed here.
Dated 2026-08-15. Review v26 (of
revision 25) found **0 CRITICAL** / 3 MAJOR / 3 MINOR — a fourth consecutive
zero-CRITICAL round — and **all three MAJORs are one defect class**: V24-M3's
finding that *neither mandated wipe sequence restarts the node-local stage
counter*, and V25-M2's that `registration_seq`'s strict growth holds **within a
generation only**, are both correctly worked into §0 and were then contradicted,
as bare assertions of fact, inside **gate 3** — the machinery those findings
underwrite — and inside gate 3's CRITICAL-level regression witness.
**V26-M1 (MAJOR)** — two gate-3 sites (the (a) rule that `stage_id` is not a
fence operand, and the (d) necessity argument for the record-binding conjunct's
second component) still asserted that the mandated wipe restarts the counter.
Both are **re-based on the producer §0 already derived** — a node-local
**durability loss** or an **out-of-band restore** of the node's directory — and
each now records that the mandated sequences do **not** produce that shape, so
the false premise cannot be re-imported. The V24-M3 sweep's enumeration, which
declared itself complete and was not, is **extended by three and says so**:
nine members, the correction visible rather than silent (**and nine was still
short — revision 27 makes it ten**, V27-m2).
**V26-M2 (MAJOR)** — the record-binding conjunct's concluding step derived the
pair's separation from "the post-wipe record's value is strictly greater than
the pre-wipe report's", the unqualified universal this document itself calls
false on the (β) single-member wipe. It is rewritten as the two-path discharge
§0 owns: (α) strict growth **within the surviving generation**; (β) destruction
of the log every undelivered report lived in, so there is no counterparty to
compare against — citing the leg-(iv) split and the canonical writer-join row
rather than restating a universal it may not assert.
**V26-M3 (MAJOR)** — the conjunct's forcing test arranged a collision the true
premise makes **unconstructable** (a post-wipe stage mints `stage_id = 2`), and
its negative control degenerated to a vacuous pass, so the regression witness
for V20-C1 had silently stopped witnessing. Re-based on the out-of-band restore
— nothing is ever admitted, so `admitted_stage` stays `None` and the mint-time
floor computes `max(0, 0) + 1 = 1` — renamed
`..._after_a_rewound_stage_counter`
at every site (**superseded in revision 27**, V27-M3: that producer is outside
the failure model §0 declares, and the name went with it), with the registration
change (`11 → 12`) kept so both components of the pair are exercised, and the
control's teeth restated at the control.
**V26-m1** — the abandon precondition's survivor check is a read of **one
member's applied state** and commit precedes apply, so it can report "still a
member" after the entry committed. The caveat, a freshness prescription (prefer
a member confirmably in the current leader's majority) and a fail-closed tie
rule (disagreement between two survivors reads as "committed", mark stays
armed) are added at the mechanism and at both recovery rows; the residue is the
already-priced gate-1 refusal with its re-issue escape.
**V26-m2** — the foreign-`raft/` blast enumerated two outcomes and the pair was
not exhaustive: a **valid FrogDB Raft store copied from another node** opens
cleanly and is neither. Outcome (iii) is added with its containment named —
gate 1's join-empty half refuses the `MEET`, and `cluster_init.rs` does not
solo-bootstrap over a store carrying membership (`:445`, `:447`, `:453` —
citations corrected in revision 27, V27-m1; revision 26 wrote `:440`/`:448`,
neither of which is a load-bearing line).
**V26-m3** — the residue control's name pre-judged outcome (i) while its body
says the outcome must be determined; renamed to
`..._is_adopted_and_the_store_open_outcome_is_recorded` at both sites, with the
body now enumerating all three outcomes and each one's asserted consequence.
**Also in revision 26, numbered here in revision 27** (the v27 flag-4 note — the
change was traceable only through the review document, and this document's own
convention is that every change is findable from the changelog): the **owed
follow-ups residuals list** gains the `--force-fresh-data-dir`-beside-foreign-
entries pointer (V25-m6) and the FM-CLUSTER-006 stale-citation correction, so a
reader scanning the round's residues finds them in one place.
**The cumulative blast radius is unchanged by this revision** — still five rows
Amended across six declared amendments carrying seven invariant deltas
(FM-PERSISTENCE-048 one, -049 two, -057 two, -059 one, TR-CLUSTER-035 one),
none performed here.
Dated 2026-08-15. Review v25 (of
revision 24) found **0 CRITICAL** / 3 MAJOR / 6 MINOR. It conceded revision
24's four MAJOR fixes in substance — the 048 widening, the α/β split, the
counter-survives sweep and the mark-before-propose ordering all stand — and
**all three MAJORs are again failures to propagate revision 24's own
discoveries** into the text that depends on them.
**V25-M1 (MAJOR)** — revision 24's mark-before-propose ordering creates a crash
window (mark durable, entry not committed, node down) and routed it to the
**stale-mark residue's** recovery row, saying it "carries the same recovery
row". It cannot: both of that row's exits presuppose a node that is **up** and a
boot that has **not happened**, while here the node is down and *the boot is the
wipe*. The recovery row is **split in two** — Row A (node up, both exits, now
with the not-committed precondition "abandon" always needed) and Row B (node
down, in-band surface **NONE**, the only exit being removal of the mark on the
**stopped** node) — the down-state enumeration goes **three → four** with the
new member's distinguishing property (the only one of the four whose *boot* is
the destructive act), Table C's crash cell is re-routed, and issue 25's
solo-bootstrap behaviour is **promoted from a recorded dependency to a BLOCKING
prerequisite** of the mark mechanism, stated in split-brain terms at the
mechanism, in the blast radius and in the sequencing plan. Forcing test
`a_crash_between_the_mark_and_the_commit_leaves_a_recoverable_node` with its
negative control.
**V25-M2 (MAJOR)** — revision 24 discovered the α/β split (the single-member
in-band wipe restarts the registration generation) and then left "strictly
greater" asserted **universally** at every other site that states it, including
the `registration_seq` writer-join row itself and the `AttestReplicaSynced`
fence's cannot-return argument. The scope is now stated **once**, at the writer-join
row — strictly greater **within a generation**; the generation itself is
restarted by (β), whose containment is the destruction of every counterparty —
and every other site cites it. `Trusted { value: 0 }`'s soundness is re-derived
from the `self ∉ nodes` disjunct plus destroyed-vehicle containment, because
across (β) the *inequality* disjunct is false: the new `registration_seq`
**collides** with the latched one. That yields **the wedge guard**, an ordering
rule pinning the applied-state read after the wipe and **before** any
solo-bootstrap self-insertion — without it the node is wedged in
`stage-counter-untrusted` permanently, in the one configuration with no other
member to `FORGET` it. Forced by
`the_untrusted_state_clears_across_a_single_member_reset_when_the_new_registration_seq_collides`.
**V25-M3 (MAJOR)** — the 048 widening falsifies the excused-set sentences of
**two other LOCKED rows**, which restate 048's list in their own words:
FM-PERSISTENCE-057 ("skips the install scratch names (`staging*`, `backup`) and
*only* those") and FM-PERSISTENCE-059 ("skips `staging*`/`backup`"). An
implementer following the sequencing plan would land code making both false, and
`just lint-spec` cannot catch prose falsity. A **sixth declaration** is added
carrying both deltas, with 059's `pending_install` half deliberately **not**
widened, and sequencing item 0 gains the disposition of the existing test
`foreign_files_ignores_the_install_scratch_but_not_the_database`.
**V25-m1**: the mark-write seam is `handle_reset_command`
(`connection/cluster.rs:161-186`, seam `:172-174`, ahead of `:175`), not the
`:46-55` call site inside the shared Raft-op dispatch.
**V25-m2**: `is_install_scratch` is `frogdb-server/crates/persistence/src/data_dir.rs:139-145`; `:146` closes the
`impl`.
**V25-m3**: `<data-dir>/frogdb_node_identity` **survives** the (β) wipe — added
to that path's vehicle enumeration and trip-wire, discharged by `self ∉ nodes`
rather than by destruction.
**V25-m4**: "the round's two spec-first obligations" was false in its own
revision (a "third" is named later); counts are now stated with their members.
**V25-m5**: excusing `raft` **prunes the subtree** (the excused-name `continue`
at `frogdb-server/crates/persistence/src/data_dir.rs:376` precedes the `:378` `is_dir` branch) — which is
simultaneously what makes the 048 amendment work and what lets a directory whose
only entry is a **foreign** `raft/` be adopted; priced, bounded at
`ClusterStorage::open`, and given a control.
**V25-m6**: `--force-fresh-data-dir` beside foreign entries is recorded as an
**owed follow-up**, with two candidate rules and no ruling this round.
**The cycle's cumulative blast radius is now five rows Amended across six
declared amendments carrying seven invariant deltas** — FM-PERSISTENCE-048
(one), FM-PERSISTENCE-049 (two), FM-PERSISTENCE-057 (two), FM-PERSISTENCE-059
(one), TR-CLUSTER-035 (one) — none performed here, each declared with its
invariant delta, its forcing test and its position in the sequencing plan.
Dated 2026-08-15. Review v24 (of
revision 23) found **0 CRITICAL** / 4 MAJOR / 3 MINOR. It conceded that
revision 23's *findings* were right — the raise's missing writer, the reset's
missing wipe, the missing stamp on persistence-disabled nodes, the missing
`FutureLayout` row — and **all four MAJORs are failures to propagate revision
23's own two discoveries**: that moving the marker phase puts `verify` in front
of directories it has never seen, and that the mandated wipe does not restart
the stage counter. Neither is a new hazard; both are conclusions revision 23
drew and then did not carry into the text that depends on them.
**V24-M1 (MAJOR)** — moving `verify`/`stamp` out of the `rocks_backed` gate
puts `verify`'s **populated-directory refusal**
(`recovery/src/data_dir.rs:110-121`) in front of every persistence-disabled and
`fake`-WAL node, whose data directory holds `raft/` and
`replication_state.json` with no marker. Revision 23 saw the collision and
called it "a second-order benefit"; it is the **upgrade path of every such node
in existence**, and as written the move refuses their boots — including every
sim-level node this design's own forcing tests run on. Fixed by a **fourth
declared LOCKED-row amendment** (FM-PERSISTENCE-048: the excused set gains
exactly `raft` and `replication_state.json`), with the alternative horn excluded
by name, the retained property given its own control, the S-a upgrade cell
re-derived, the forcing control corrected to assert the real sequence, and the
residual refusal given a recovery row — this design's **third** down-state rule.
**V24-M2 (MAJOR)** — the record-binding conjunct's leg (iv) claims a strictly
greater `registration_seq` "across the wipe", but revision 23 split the wipe
into two sequences and the claim holds for only one of them. On the
**single-member, in-band** sequence the generation lives in the state machine
whose log the sequence discards, so the node re-registers at
`registration_seq = 1` — equal to its first registration. Re-derived over both
paths: (α) multi-member keeps the monotonicity argument; (β) single-member is
carried by a **destruction of the vehicle** — the same discard destroys every
undelivered report — stated as a design-owned rule with a trip-wire on any
change that preserves the log across a reset, and given its own forcing test.
**V24-M3 (MAJOR)** — revision 23 discovered that neither mandated sequence
restarts the stage counter, and then left a dozen contrary claims standing as
"conservative rather than wrong". Half of them do not assume the restart, they
**assert it as fact**, and one used it to replace a sound monotone argument with
a weaker one. Swept: the collision-freedom paragraph, both fence-freshness rows,
the `admitted_stage` writer cell, §1's deleted-write argument, and the Quint
model — every site **re-derived from the true premise** rather than deleted, and
the model's counter reset retained as a **declared over-approximation** whose
real referent (a cell deletion composed with a node-local store loss) is named,
so mutation (17)'s kill is shown to rest on a schedule the system admits.
**V24-M4 (MAJOR)** — revision 23 placed the Raft discard mark "before the
`+OK`", which is **after** the apply, where LOCKED FM-CLUSTER-006's infallible
reset arm leaves an IO failure nowhere to go but a `+OK` with a silently
disarmed wipe. Both orderings are now analysed and **mark-before-propose is
chosen**, at the `handle_reset_command` interception ahead of `propose_reset`,
with its literal refusal token, its stale-mark residue surfaced as
`CLUSTER INFO`'s `raft_discard_pending` and given two operator exits, all four
crash windows re-derived, FM-CLUSTER-006 confronted by name and found
**unbreached**, and TR-CLUSTER-035's amendment re-shaped from its Postcondition
to its **Precondition**.
**V24-m1 (MINOR)**: `<data-dir>/frogdb_raft_discard` had no writer join —
**Table C** is added to the fifth discipline, and FM-PERSISTENCE-057 is
**Amended** (four paths become six) rather than merely cited.
**V24-m2 (MINOR)**: the raise now states that `created_at_unix_ms` rides
forward verbatim beside `database_id`, with a forcing test.
**V24-m3 (MINOR)**: `recovery/src/data_dir.rs:110-121` is cited wherever
`verify`'s outcome set is stated, and the issue-25 solo-bootstrap dependency is
stated **once**, in the implementation plan, with every other mention pointing
at it.
**The cycle's cumulative blast radius is now four rows Amended across five
declared amendments** — FM-PERSISTENCE-048, FM-PERSISTENCE-049 (twice),
FM-PERSISTENCE-057, TR-CLUSTER-035 — none performed here, each declared with its
invariant delta, its forcing test and its position in the sequencing plan.
Dated 2026-08-15. Review v23 (of
...
only, its source (`frogdb-server/crates/cluster/src/commands.rs:816-862`) touches no filesystem, and
...
to depend on the reset. (**Verdict overturned by V24-M3 in the next entry**, and
marked here in revision 28 — V28-M10: the "conservative rather than wrong" label
was half false, because half the sites *asserted* the restart as fact rather
than assuming it, and revision 24 swept them.)
...
**thirteenth** consecutive round whose findings live in the previous round's fix
text (**V28-m14**, corrected in scope by **V29-m1**). **The ladder is stated
once, here, as a list of rungs; every later ordinal is read off this list and is
not restated in prose anywhere else**: v15 = **sixth**, v16 = **seventh**,
...
v30 = **twenty-first**, v31 = **twenty-second**, v32 = **twenty-third**,
v33 = **twenty-fourth**. The rungs
...
numbering itself; from v22 on, each rung is this list's successor and nothing
more. The streak was never broken, and revisions 22-27 were each short by one.
**What V28-m14 got right and what it got wrong** (V29-m1): the missing +1 at
*this* entry is right, but the claim that "this single correction resolves the
whole ladder" is false — it carried review v28's pre-correction "twentieth"
forward as a **twenty-first**, when the pre-correction v28 ordinal was itself two
...
v30 eighth, v31 ninth, v32 tenth, v33 eleventh.)
...
declaration rather than by FM-CLUSTER-100 — V22-m3. **Further amended in
revision 27, and stated correctly here only in revision 28** (V28-M9): the
scope declaration excludes an **act**, not a class of id. The refused-id
residue is **inside** the failure model and is closed by §0's fail-closed
staging refusal on the counter-**loss** producer — the untrusted state refuses
every mint, so the re-used id cannot be minted at all — and it falls outside
the model **only** in the out-of-band-restore case, by the act (V27-M3; see the
record-binding conjunct's fence-freshness row). Revisions 22-27 wrote "the
residue is scoped out", which under a collapsed reading would delete the
counter-loss containment this design calls "the state that cannot be created".)
...
**node-local counter the TR-CLUSTER-005 rejoin wipe restarts** (**premise
withdrawn by V24-M3** — no mandated sequence restarts the node-local counter:
`CLUSTER RESET HARD` touches neither the identity store nor its counter and the
multi-member sequence removes `<data-dir>/raft` alone. The defect V20-C1 names
is **real**, on the counter-**loss** producer instead; only the vehicle written
here was wrong. Marked in revision 28, V28-M10 — this entry asserted the premise
as fact and was missing from the V24-M3 swept set the document declared
complete) — to the sole
...
whose cell is *carried* rather than deleted (`frogdb-server/crates/cluster/src/commands.rs:844`/`:852`, with the
running node's `self_node_id` following the HARD re-key, `frogdb-server/crates/cluster/src/writer.rs:250`); the
...
`ResetCluster`**, `frogdb-server/crates/cluster/src/commands.rs:837`), by splitting arm 4 into its *transient*
...
epochs. (**Superseded in revision 22 and marked here in revision 29** —
V22-m3/V29-m5: FM-CLUSTER-100 does **not** govern the out-of-band restore; that
row says only that the replicated handoff generation is *carried* by the
snapshot vehicles it names. The restore is scoped out by this document's own
declaration in `registration_seq_gen`'s §0 bullet, **by the act** — see the
standing three-statement cross-check at the fence-freshness discipline. The
sentence above is kept in its original words as history, like the v20 entry's
marked claim, and is not the live routing.) A **third permanent obligation** is adopted alongside the writer join
...
`NodeInfo` (`frogdb-server/crates/cluster/src/commands.rs:233`) and `ResetCluster` clears the map
(`frogdb-server/crates/cluster/src/commands.rs:833-854`); the NodeId is derived from the cluster-bus address by
...
destroyed by a counter wipe, so only `replid` distinguishes there) — **the
second operand is replaced by `nodes[n].registration_seq` in revision 18**
(V18-C1; marked here so the entry cannot be read as current, per V28-M7); the three
...
the cap-ambiguity issue 29 at the deadlines row, restate the wipe path's two monotonicity legs
...
passages (V13-M6). Revision 13 resolved all **ten** findings (V28-m13 —
revisions 13 through 27 wrote "nine"; the entry's own header reports 1 CRITICAL
/ 6 MAJOR / 3 MINOR and it enumerates V13-C1 plus V13-M1…M6, which is ten. Every
sibling entry gets the arithmetic right) — centrally by the
...
applied to a new name — and, since that row makes `DataDirLayout` the **single
owner** of the paths it counts, "applied to a new name" is a **row amendment**,
declared in the blast radius (V24-m1: four paths become six, the identity store
and the discard mark) rather than a free reading — never the replication state file, whose save
...
nodes today (`frogdb-server/crates/recovery/src/cluster.rs:23-25`, gated on `cluster.enabled` and nothing
...
- **Both snapshot vehicles carry it**, under the same wholesale-adoption clause as the
  ...
  **an out-of-band restore of a node's node-local directory is outside this design's
  failure model. The exclusion is by the *act*: an operator mutation of node-local
  durable state that bypasses every interface this design fences — no command, no
  transition, no boot rule observes it happen — and a design cannot fence an edit
  made behind its back.**
  **The discriminator, stated because the criterion above otherwise proves too
  much** (V28-M11 — as written the criterion also caught this design's *own*
  mandated out-of-band `<data-dir>/raft` removal, which is prescribed and reasoned
  about in-model throughout): the excluded act is an out-of-band mutation that
  **rewinds a durable value the protocol has already spent or floored against**,
  handing a live registration a value some earlier life already used. It is **not**
  an out-of-band **destruction of a vehicle** — removing `<data-dir>/raft` at a node
  already removed from membership and stopped (the (α) sequence) destroys the log
  rather than rewinding any value: it leaves no stale value behind to be re-spent,
  and it is the same destruction-of-the-counterparty argument this section makes for
  (β) below. The test is **what the act does to the value space**, not whether the
  act is out-of-band — a rewind is excluded, a destruction is in-model. Every
  out-of-band step this design *mandates* is a destruction and is therefore admitted
  by the criterion; the restore is the only rewind and is the only thing excluded.
  The exclusion is **categorical** *within that class*: it covers every
  out-of-band restore of the node-local directory, whatever value it rewinds to and
  whichever registration spent that value. **The reason it does *not* rest on, and
  why** (V27-M3 — revision 21 justified the exclusion with "every operand this
  design could floor against was rewound by the same act", and that is **false** in
  the multi-member case, so it is withdrawn rather than kept as a convenient
  sentence): the restore does rewind every node-local durable value at once —
  identity store, incarnation, applied Raft state — but `registration_seq` and
  `admitted_stage` are **replicated**, minted and carried by the cluster
  (`registration_seq` by the fresh arm of `AddNode`, `commands.rs:132`, from
  `ClusterStateInner.registration_seq_gen`), so the restored applied state is
  overwritten from the log the survivors hold and **both operands come back
  untouched**. An in-protocol operand demonstrably survives, and the mint-time
  floor reads it — which is exactly why the `FutureLayout` row below can say a
  re-minted directory cannot re-issue an **admitted** id. What no operand floors is
  narrower and is stated where it bites: the node-local `stage_counter`, and within
  it an id that was **minted and then refused**, never admitted and so never
  reachable by the floor. The rule stands on the act, not on the arithmetic. What
...
- **The property, and it is a writer-enumerating one**: the counter never decreases,
  ...
  **Scope** (V25-M2): the property is **within a generation** — the generation is the
  applied state of *this* Raft log, so it survives the multi-member out-of-band wipe
  (α: the survivors hold it) and is **restarted** by the single-member in-band wipe
  (β: `FLUSHALL` + `CLUSTER RESET HARD` + restart destroys the log, and the node
  re-registers at `registration_seq = 1`). What contains (β) is not this property but
  the **destruction of every counterparty** that could hold a snapshot to compare
  against. The full statement is at the `registration_seq` writer-join row of the fifth
  discipline; every "strictly greater" in this document is scoped by it.
...
absent-operand exception below, narrowed by V18-M1). **Forward pointer** (V28-m18,
the v28 residual-B improvement): this narrowed rule is what **leg (a)** of the
record-binding conjunct's fence-freshness row runs on — the reader who most needs
it arrives from there, so the rule is stated here and cited, never restated.

**Composition with the pending-record reconcile guard** (V28-M4 — each rule was
stated alone, and at the state the V20-C1 forcing fixture pins they prescribed
different things with no stated resolution): while a record is pending **and** this
node's boot `ReportRunIdentity` has not applied, **the boot report takes
precedence**. The reconcile guard's "nothing but the staged report itself … and
**nothing else**" governs what the *reconcile* may propose; it was never written
against the boot report, which is the precondition of this node proposing anything
at all. Concretely, and this is the whole of the composition:

- Staging a record is **node-local** — mint, record fsynced, whole-node fence
  raised — and proposes no Raft command, so the boot rule does not reach it and a
  record may be staged in this window.
- The record's own `ReportRunIdentity` is **held unproposed** until the boot report
  has applied. The hold is the boot rule's, not the reconcile guard's.
- The boot report itself may be re-minted and re-proposed in the window (that is
  the boot rule's own refusal-and-retry behaviour), and the reconcile guard's
  "nothing else" does not forbid it.
- When the boot report applies, the held report is proposed. Nothing else changes.

The alternative resolution — forbidding a record to be staged at all until the boot
report applies — was considered and rejected: it would owe gate 3 (a) a new refusal
token and a new recovery row for a window the whole-node fence already holds
fail-closed, and it would buy nothing the hold does not already buy.
...
- **The pending-transition record across these flows**
  ...
  **The rule is keyed on the *effect*, not on the transitions this node
  ...
  > registration** — the token is `-ERR CLUSTER REPLICATE staged flip lost its
  > registration (<seq>)`, and the operand this clause names is
  > **`record.staged_registration_seq`**, the only registration operand it has
  > (it is level-triggered on applied state and holds no refused payload).
  > **Why that is byte-for-byte the reply arm 4b gives, argued rather than
  > asserted** (V28-M5 — revision 27 asserted the commutation and the two arms
  > name *different* operands, so the agreement has to be derived): arm 4b names
  > **`refused_payload.observed_registration_seq`**, this clause names
  > **`record.staged_registration_seq`**, and the **record-binding conjunct**
  > (V20-C1) makes those two equal on **every** path where 4b can fire — a
  > refusal whose payload pair does not equal the record's pair selects no arm at
  > all and is a log-only no-op. So on the paths where both mechanisms are live
  > the operands are the same value and the bytes are the same bytes; on the
  > paths where they differ, only one of the two mechanisms fires. The
  > commutation is therefore a **consequence** of the conjunct, not an
  > independent assumption — which is additionally a second, independent
  > argument that the conjunct's second component is necessary (stated again at
  > the necessity trace). And **the whole-node fence drops with the
  ...
  **both** durability outcomes. **Re-derived in revision 24 on the counter's
  actual fate** (V24-M3): revision 23 established, while re-deriving the wipe
  sequences at gate 1, that **neither mandated sequence destroys the node-local
  identity store**, so neither restarts the stage counter — `CLUSTER RESET
  HARD` touches neither the identity store nor its counter, and the out-of-band
  step removes `<data-dir>/raft` and nothing else. (**V28-m12**: revisions 23-27
  wrote "touches no filesystem at all" here and at five other sites. That
  absolute is false under this design's *own* amendment — V23-M2 gives
  TR-CLUSTER-035's HARD path a **durable discard-mark** obligation, which is a
  filesystem write. The claim the argument needs is the narrower one now
  written, and it survives the amendment.) Revisions 17 through 23 wrote this
  paragraph under the opposite belief, and the correction runs in a direction
  the document did not anticipate: **V17-m3's own correction is withdrawn**.
  Revision 16 said "`stage_id` is **not** reset by any of these flows"; V17-m3
  called that false "on the mandated wipe path that destroys node-local durable
  state along with the counter", and there is no such path. Revision 16's claim
  was right about the mandated sequences, for a reason revision 16 did not give
  and the node-local durable-state join now supplies: **no writer deletes the
  store**.
  **Counters survived — which is the mandated sequences' case, not one of two**:
  `stage_id` counts monotonically like the incarnation mint, so across
  `FLUSHALL` + `CLUSTER RESET HARD` + restart, and across the multi-member
  `FORGET` + out-of-band `<data-dir>/raft` removal, the next stage id is
  strictly above every id this node has ever spent. Neither sequence produces a
  re-use at all, and the argument for that is the **monotone** one — the
  simplest form this paragraph has ever had, and the one revision 20 replaced
  with a cell-deletion argument it did not need.
  **Counters destroyed — still a real case, with a different producer**: a
  node-local **durability loss** (the disk fault §0's untrusted state exists
  for). **A node-local durability loss is the producer V20-C1's trace rests
  on**; an out-of-band restore rewinds the same value and is excluded **by the
  act**, per this section's scope declaration above (V28-M1 — revision 27
  declared this half dropped from the load-bearing producer at three sites and
  executed it at two; this is the third, and it is the site the other two cite as
  authority). The trace survives intact with the wipe
  replaced by the loss: the loss kills the record that exists *at loss time*; it
  cannot kill a record the node stages **after** the loss, and that later record
  would be minted from a restarted counter, so it could carry an id an
  *earlier, still-undisposed* report already carries. The operand a re-used id
  meets is **every record that exists when a refusal is observed**, not only the
  one that died. Revision 19's containment — "the only operand a stage id is
  ever compared against is this node's own durable pending-transition record,
  and that record dies in the very same wipe" — was a non-sequitur then and is
  one now, and it is **replaced, not amended** (V20-C1).
  **What carries the collision-freedom instead: the pair.** The record-binding
  ...
  mint site (`registration_seq_gen`, §0's registration bullet).
  **Leg (iv) of that conjunct is discharged over the two wipe paths separately,
  and they discharge differently** (V24-M2 — revision 23 rewrote the wipe into
  two sequences and left this universal standing over both):
  - **(α) multi-member, out-of-band** (`CLUSTER FORGET` at a survivor; then
    `FLUSHALL`, stop, remove `<data-dir>/raft`, restart at the departing node).
    The survivors' `registration_seq_gen` is **intact** — it lives in *their*
    replicated state, which this sequence never touches — and readmission is a
    genuine fresh `AddNode` at a live cluster (LOCKED TR-CLUSTER-005's `MEET`
    path), minting from that surviving generation. So every post-rejoin
    record's `staged_registration_seq` **is** strictly greater than every
    pre-wipe report's `observed_registration_seq`, and the strictly-greater
    argument holds exactly as written.
  - **(β) single-member, in-band** (`FLUSHALL`, `CLUSTER RESET HARD`, restart).
    **The strictly-greater argument does not hold here, and asserting it was
    false**: the generation lives in the replicated state machine, this sequence
    discards the Raft log that state machine is rebuilt from, and the node then
    self-bootstraps and registers itself from a **fresh** generation — this
    design's own exit (b) says so in as many words ("the registration mints
    `registration_seq` from a fresh generation"), and the bootstrap path is
    `cluster_init.rs:383-391`, `:442-460`, `:527`. The post-wipe mint is
    `registration_seq = 1`: *equal to* the node's first pre-wipe registration,
    and strictly *less* than any later one. **What holds instead, and this
    design owns it as a RULE rather than as an observation**: the same sequence
    that restarts the generation **discards the log that carries every
    undelivered pre-wipe report**, so the pair has no counterparty to be
    compared against. That is a *destruction-of-the-vehicle* argument, not a
    monotonicity one — there is nobody to readmit and nothing left to deliver,
    which is also why the sentence "the wipe is a rejoin … LOCKED
    TR-CLUSTER-005 readmission" is **not** said of this path.
    **The vehicles this sequence destroys, enumerated — and the one it does
    not** (V25-m3 — revision 24's enumeration was implicit and therefore
    incomplete): the Raft log and the applied state built from it (the
    `CLUSTER RESET HARD` + restart with TR-CLUSTER-035's declared discard), the
    keyspace (`FLUSHALL`), and this node's durable pending-transition record
    (cleared fsynced by the effect-keyed clause at boot). **`<data-dir>/frogdb_node_identity`
    survives** — the wipe removes `<data-dir>/raft`, never the identity store
    (V24-M3's finding), so the node comes up holding a `stage_counter` and a
    `stage_counter_state` minted in the destroyed registration. **Its
    discharge is `self ∉ nodes`**, not destruction: the wiped node's applied
    state contains no cell for itself, which is the disjunct that clears the
    untrusted state, clears any record, and makes every surviving field of the
    identity store re-derived rather than compared — and the wedge guard pins
    that read **ahead of any self-insertion**, because a node that bootstraps
    itself in first reads `self ∈ nodes` at a colliding `registration_seq = 1`
    and is wedged. So (β)'s containment is destruction of every vehicle **plus**
    one disjunct for the vehicle that survives.
    **Trip-wire, stated because this leg now rests on a destruction rather than
    on an ordering**: any future change that **preserves the Raft log across a
    reset** — a reset that rebuilds applied state without discarding the log, a
    log-preserving "soft wipe", a snapshot restored across the sequence, a
    discard mark that is armed but not completed — re-opens this leg and must
    re-derive it in the same change. **And, after V25-m3, so does any change
    that gives the surviving identity store a *reader that compares* rather than
    re-derives** — a field of it read against replicated state before the
    `self ∉ nodes` clear, or any new node-local artifact that outlives the wipe
    — because that adds a counterparty to a path whose whole argument is that
    none exists.
  The containment is therefore carried, on (α), by an operand **no node-local
  ...
  discipline — and, on (β), by the non-existence of the vehicle the
  counterparty would have to arrive in.
  **The residue the pair does not cover, closed fail-closed** (V20-C1 part 2,
  ...
  - **Where the counter lives, and what tells "never created" from "lost"**
    ...
    - **(1) An explicit durable field, so absence is read rather than
      ...
      *trustworthy* counter. This is the rule `DataDirMarker::stamp_with`
      already follows (`frogdb-server/crates/persistence/src/data_dir.rs:289-314`
      — `write` → `sync_file` → `rename` → `sync_dir`; **V28-m11**: revisions
      22-27 attributed this span to `DataDirMarker::stamp`, which is the
      three-line wrapper at `:283-285` that forwards to `stamp_with(dir,
      &RealFs)` — verified against the tree this round. Three distinct `stamp`
      entities exist in this document's citations and each is now written with
      its owner: `DataDirMarker::stamp` at
      `frogdb-server/crates/persistence/src/data_dir.rs:283-285`,
      `DataDirMarker::stamp_with` at the same file's `:289-314`, and recovery's
      free `stamp` at `frogdb-server/crates/recovery/src/data_dir.rs:188-196`,
      which `create_dir_all`s and then calls the first) and the rule LOCKED
    ...
    - **(2) The data-directory schema stamp decides "never created"**, not the
      ...
      **The stamp must first be made to exist everywhere** (V23-M3 — **a LOCKED
      row moves here, and this is the first of the round's spec-first
      obligations, which are stated with their members rather than as a
      number**, V25-m4: revision 23 wrote "two" and then referred to "the
      round's third spec-first obligation" eight hundred lines later, so the
      number was already false in its own revision). **Revision 23's three**:
      (i) FM-PERSISTENCE-049's *scope* — the marker phase becomes a
      data-directory operation rather than a RocksDB-gated one (this
      paragraph); (ii) FM-PERSISTENCE-049's *outcome set* — `verify` gains the
      raise (the stamp-raise paragraph below); (iii) TR-CLUSTER-035's HARD-path
      Raft discard (gate 1's mark mechanism). **Revisions 24 and 25 add three
      more** — FM-PERSISTENCE-048's excused set, FM-PERSISTENCE-057's path set,
      and FM-PERSISTENCE-057/059's excused-set sentences — so the **cycle**
      stands at **six declarations over five rows, seven invariant deltas**,
      enumerated with their members in the blast radius. Today `verify` and
      `stamp` run **inside** the
      ...
      **narrows** no LOCKED row's guarantees and collides with none of them:
      ...
      `a_persistence_disabled_node_stamps_its_data_directory`.
      **The move does not compose with `verify`'s populated-directory refusal,
      and revision 23 called that collision a benefit** (V24-M1 — the correction
      is the sharpest of this round, because it inverts a sign). Revision 23
      wrote here that the move "closes an existing asymmetry in which a node that
      flips `persistence.enabled` from `false` to `true` boots into a directory
      holding `raft/` and `replication_state.json` and **no marker**, which is
      exactly [the] foreign-files refusal shape", and called that a second-order
      benefit noted not claimed. It is not a benefit and it is not second-order:
      **it is the upgrade path of every existing persistence-disabled and `fake`-WAL
      node, and as written the move bricks all of them.** Two facts compose:
      - Such a node has been creating `<data-dir>/raft` (phase 6, gated on
        `cluster.enabled` alone) and `<data-dir>/replication_state.json` for its
        whole life, with **no marker**, exactly as the paragraph above establishes
        in order to argue the move is safe.
      - `verify` on an unmarked directory does not mint unconditionally. It calls
        `contains_foreign_files`, and on `true` **bails**:
        `if has_files && !force { bail!("data directory {} holds files but no
        FrogDB marker ({MARKER_FILE_NAME}): refusing to start, …") }`
        (`frogdb-server/crates/recovery/src/data_dir.rs:110-121`; the fresh mint
        at `:166` is only reached when that bail does not fire). The excused set
        is `backup`, `staging`, and `staging.*` and nothing else, and it excuses
        only *top-level* names while the scan descends
        (`frogdb-server/crates/persistence/src/data_dir.rs:139-145` — the
        function ends at `:145`, `:146` is the `impl` block's closing brace,
        V25-m2 — and `:366`).
      So the first boot of a build shipping this design, on a
      persistence-disabled node that has ever run with clustering or replication,
      **refuses to start** with a message about overwriting data — and this
      design's own sim-level forcing tests, which run on precisely such nodes,
      would refuse with it. Revision 23 had the shape right and the sign
      backwards; it also mis-cited the row (the populated-directory refusal is
      **FM**-PERSISTENCE-051, `specs/persistence.md:1199`; **TR**-PERSISTENCE-051
      at `:560` is marker *version* validation, a different row this document
      cites correctly two paragraphs above).
      **Fix — horn (a): the excused set gains exactly the FrogDB-owned entries,
      as the cycle's fourth declared spec-first amendment and revision 24's
      first** (LOCKED FM-PERSISTENCE-048,
      `specs/persistence.md:1163`). The row's Invariant reads: "It skips exactly
      the top-level install scratch names the layout owns — `backup`, `staging`,
      and `staging.*` … — and nothing else". The delta is a **closed, named
      widening of that list by two entries**: `raft` (a directory) and
      `replication_state.json` (a file), both top-level, both written by FrogDB
      itself on a path that runs before any marker exists. The list stays closed
      and the "and nothing else" clause stays — it is the list that changes, not
      its character. Rationale for the direction: these two entries are *this
      database's own state*, so the refusal's stated purpose ("initializing a
      fresh database here would begin overwriting whatever is already in it") is
      not served by refusing on them — there is nothing foreign to protect, and
      the mint that follows preserves both files untouched. Forcing test:
      `a_persistence_disabled_node_with_an_existing_raft_store_boots` — build a
      directory holding `raft/` and `replication_state.json` and no marker, run
      phase 0, assert a marker is minted and both entries survive. **Negative
      control, asserting the bug this amendment fixes**: the same fixture against
      today's excused set must produce the `holds files but no FrogDB marker`
      bail, so the test cannot pass by accident of the widening never being
      exercised.
      **Retained property, kept deliberately narrow.** A directory holding **any
      other** unexcused entry still refuses — an operator's stray file, a
      hand-copied `dump.rdb`, another database's directory, a `db/` from a
      RocksDB-backed node whose `persistence.enabled` was then turned off (`db`
      is deliberately *not* excused, and `.../persistence/src/data_dir.rs:141`
      says so in a comment). Control:
      `an_unmarked_directory_with_a_stray_file_still_refuses`, a directory
      holding `raft/` **plus** one unrelated file, asserting the bail still
      fires. Without that control the widening could be implemented as "skip the
      check when a `raft/` exists" and pass.
      **The price of the excusal, stated rather than left to be discovered**
      (V25-m5). `contains_foreign_files` tests the top-level name **before** it
      decides whether to recurse: the excused-name `continue` is at
      `frogdb-server/crates/persistence/src/data_dir.rs:376` and the
      `entry.file_type()?.is_dir()` branch that descends is at `:378`. So
      excusing a *directory* name **prunes the whole subtree** — nothing under
      `raft/` is ever scanned. **That one property is doing both jobs at once,
      and the amendment is only acceptable if both halves are stated.** It is
      what makes the amendment *work*: a persistence-disabled node's `raft/`
      holds RocksDB files at depth, and a rule that excused the name but still
      recursed would refuse on the first of them, leaving the boot exactly as
      broken as before. It is also what creates the **residue**: a directory
      whose only entry is a `raft/` that is **not** FrogDB's — an unrelated
      product's directory of the same name, a hand-assembled path, a stale mount
      — now reads as *empty*, so phase 0 mints a marker into it and the node
      adopts a directory FM-PERSISTENCE-051 would previously have refused.
      **Bounding the blast, and the bound is weaker than "it fails safely"**:
      the adopted directory's `raft/` is handed to `ClusterStorage::open`
      (`frogdb-server/crates/cluster/src/storage.rs:286-315`), which opens
      RocksDB with `create_if_missing(true)` and
      `create_missing_column_families(true)`. **Three outcomes** — revision 25
      claimed two and the pair was not exhaustive (V26-m2) — and this design
      claims none of them is the nice one: **(i)** the directory holds
      something RocksDB recognises as a broken or incompatible database and the
      open **fails**, surfacing as a boot error rather than a silent adoption;
      **(ii)** the directory holds files RocksDB does not recognise at all, in
      which case it **initialises a fresh database in place**, writing
      `CURRENT`/`MANIFEST-*`/`LOG` beside the foreign content rather than
      refusing; **(iii)** the directory *is* a valid FrogDB Raft store copied
      from **another node** — the shape the "hand-assembled path" example
      invites, and the one neither (i) nor (ii) covers — in which case the open
      **succeeds cleanly** and the node comes up on a foreign cluster's vote,
      log and membership. Outcome (ii) does not delete the operator's bytes wholesale, but
      it is a **write into a directory the refusal existed to protect**, and
      RocksDB's own obsolete-file sweep may remove entries whose names match its
      patterns. **Outcome (iii) is contained downstream, and the containment is
      named rather than assumed**: a non-empty Raft store fails **gate 1's
      join-empty half**, so the node cannot `MEET` its way into the operator's
      cluster, and the boot path does not solo-bootstrap over a store that
      carries membership — `cluster_init.rs` computes `already_initialized` from
      the restored membership's node count (`:445`) and guards on it (`:447`),
      skipping the `raft.initialize` at `:453` when it is non-empty (citations
      corrected in revision 27, V27-m1 — revision 26 wrote `:440`/`:448`, neither
      of which is a load-bearing line; the substance is unchanged and was
      verified against the file), so no fresh group is minted over the
      stranger's log either. What the node does instead is come up as a member
      of a cluster that is not the operator's, talking to peers the operator did
      not configure — wrong and visible, not a silent adoption into the
      operator's cluster, which is why this stays a bounded blast and not a
      correctness hole. So the honest statement is: the marker mint is non-destructive,
      the store open is *not guaranteed* to be, and the property
      FM-PERSISTENCE-051 enforced for this shape now rests on the operator not
      pointing FrogDB at a directory whose only entry is a foreign `raft/`.
      Control, and it must assert which outcome actually occurs rather than
      assuming (i) — **which is why revision 26 renames it away from the verdict
      the old name pre-judged** (V26-m3; through revision 25 it read
      `..._is_adopted_and_fails_at_store_open`, a name asserting outcome (i)
      while the body left the question open, and this document's own rule is
      that a test whose name states a verdict its body does not is a test the
      next implementer "fixes"):
      `a_directory_whose_only_entry_is_a_foreign_raft_dir_is_adopted_and_the_store_open_outcome_is_recorded`
      — build an unmarked directory containing only a `raft/` of non-RocksDB
      content, run phase 0, assert the marker **is** minted (the residue is
      real, not hypothetical), then open the store and **record which of the
      three outcomes occurs**, asserting each one's stated consequence: on (i) a
      boot error; on (ii) that a fresh database is initialised **in place** and
      the foreign content is still present beside it; on (iii) — arranged by
      copying a real FrogDB `raft/` from another node — that the boot does
      **not** solo-bootstrap and that the subsequent `MEET` is refused by gate
      1. **If the implementation exhibits (ii) or (iii)**, the test records that
      and the follow-up below becomes required rather than advisory.
      **OWED FOLLOW-UP, recorded and deliberately not ruled this round**
      (V25-m6): `--force-fresh-data-dir` **does not delete the foreign entry** —
      it mints a marker beside it and proceeds, which the unmarked-populated
      -directory recovery row already prices as its third cost. With this
      amendment narrowing what counts as foreign, and with the subtree prune
      above widening what is silently adopted, **FM-PERSISTENCE-051's property
      now leans on operator discipline in exactly the shape it was written to
      remove**: the row enforces "a populated data directory with no marker
      refuses the boot", and both the excusal and the flag are ways past it that
      leave the operator's bytes in place while FrogDB writes into the
      directory. Two candidate rules, neither adopted here because both change
      a shipped flag's behaviour and belong to the persistence area rather than
      to this design: **(a)** refuse `--force-fresh-data-dir` when the directory
      holds *unexcused* entries, making the flag a fresh-start tool rather than
      an override; **(b)** keep the override but require a second, explicit
      confirmation naming the entries that will be adopted. This document
      records the gap, the two candidates and the reason it is not ruling —
      **not** a rule, and no forcing test is claimed for it.
      **Horn (b) is excluded by name.** The alternative the review offers is to
      keep the excused set closed and instead gate the move — run `verify`/`stamp`
      outside the `rocks_backed` branch only when the directory is empty or
      already marked, deferring the stamp otherwise. It is rejected for three
      reasons: it re-creates the very hole V23-M3 closed (the nodes that would
      defer are exactly the persistence-disabled ones whose stamp §0's mint
      precondition needs), it makes "has a stamp" configuration- and
      history-dependent, which the 30-cell partition below is built to avoid, and
      it turns a one-time upgrade question into a permanent conditional. A closed
      two-entry widening of a list is a smaller and more auditable change than a
      new conditional in the boot path.
      **This design's own new top-level artifacts are *not* added to the excused
      set, and that is a decision rather than an oversight.**
      `<data-dir>/frogdb_node_identity` and `<data-dir>/frogdb_raft_discard` also
      live at the top level, but phase 0 runs **before** either of their writers
      (§0's ordering, and the writer tables in the fifth discipline), so neither
      can exist in a directory that lacks a marker unless it was put there out of
      band. On a marked directory `contains_foreign_files` is never consulted —
      `verify` returns the existing marker before the scan
      (`.../recovery/src/data_dir.rs:88-95`) — so excusing them would buy
      nothing and would weaken the refusal against a hand-assembled directory.
      ...
      **delete the marker file out of band** on a directory that had a stamp at
      ...
      design scopes out by declaration in `registration_seq_gen`'s §0 bullet
      (the exclusion is by the ***act*** — an operator mutation of node-local
      durable state that bypasses every interface this design fences, and one
      that **rewinds** a value the protocol has already spent rather than
      destroying a vehicle; V28-M2 — revision 27 withdrew the old "no
      in-protocol operand survives" reason as false, and this site had kept it,
      attributed to the very bullet that withdrew it),
  ...
  - **At boot, on a lost store — a *staging* refusal, not a boot failure**
    ...
    **cleared by writing `stage_counter_state = Trusted { value: 0 }` and
    `stage_counter = 0` — both fields, in one atomic write** (**V29-M4** —
    revisions 22-28 stated only the state field here, while Table A's
    *Untrusted clear* row, the V20-C1 fixture's `max(0, 0) + 1 = 1`, the
    necessity trace's *restarted counter* and ext-18's `Trusted { value: 0 }`
    branch all rest on the second write; the rule now states what they rest
    on, so they are derivable from the rule instead of from each other),
    in the same atomic write discipline, and `0` is the right value **for both
    fields** because the state
    ...
    **The soundness argument below is argued of both fields at once.** It
    establishes that nothing this node spent before the exit can collide with
    anything minted after it — on the (α) path by the strictly greater
    `registration_seq`, on the (β) path by destruction of every vehicle that
    could hold such an id — and that is precisely what licenses *restarting the
    counter* as well as *trusting the state*: a restarted counter is unsafe
    only if some live operand still carries an id at or above the restarted
    value, which is what the two paths between them rule out. The two writes
    are therefore one decision, not two, and neither may be performed without
    the other: writing the state alone would leave a `Trusted` state whose
    value disagrees with the field it certifies, and writing the counter alone
    would restart the counter while the node still refuses every mint.
    ...
    conjunct): the field's sole writer mints strictly greater **within a
    generation**, so `≠` *is* the strictly-greater fact without anyone reading
    one. On the (α) path the clearing is sound for the reason the rule's own
    guard already gave: a strictly greater `registration_seq` separates every
    later record from every earlier report, so nothing minted in the new
    registration can bind anything minted in the old.
    **Re-derived for the (β) path, where the premise fails** (V25-M2 — revision
    24 leaned the whole soundness of `Trusted { value: 0 }` on strict growth,
    and across the single-member in-band wipe the generation restarts, so the
    new `registration_seq` is `1` and can **equal** the value the untrusted
    state latched): the first disjunct is then **false**, and the rule clears on
    the **second** — `self ∉ nodes`, which is exactly what a node whose Raft log
    was discarded reads in its own applied state. `Trusted { value: 0 }` **and
    the zeroed `stage_counter`** remain
    the right values for a second, independent reason: across (β) every
    counterparty that could hold an id minted in the old registration is
    destroyed by the same sequence (the cells with the log, this node's record
    by this rule's own fsynced clear, the dataset by `FLUSHALL`), so `0` cannot
    collide with a live operand — the containment is destruction of the vehicle,
    not separation by magnitude. Both disjuncts are therefore load-bearing, and
    the second is the *only* one that fires on (β).
    > **The wedge guard — an ordering rule, not a value rule** (V25-M2). Because
    > the clearing predicate's `self ∉ nodes` disjunct is what fires on (β), the
    > applied-state read it evaluates **must happen after the wipe and *before*
    > any self-insertion into membership**. A node that bootstraps itself into a
    > fresh Raft group *first* and evaluates the predicate *after* reads
    > `self ∈ nodes` with `registration_seq = 1` — colliding with the latched
    > `untrusted_since_registration` — so **neither** disjunct holds and the node
    > is **wedged in `stage-counter-untrusted` permanently**, refusing every mint
    > with no exit, in the one configuration (single-member) that has no other
    > member to `FORGET` it. That is a fail-closed rule with no way back, which
    > ruling 5 does not license. The read is therefore pinned to the boot
    > position **between** the discard-mark completion and any `raft.initialize`
    > (`cluster_init.rs:442-460`), and this is a second, independent reason the
    > issue-25 solo-bootstrap change is a prerequisite rather than a nicety.
    > Forced by
    > `the_untrusted_state_clears_across_a_single_member_reset_when_the_new_registration_seq_collides`.
    ...
    **With the cell absent, no state is entered**: the node can only return
    through a fresh registration whose `registration_seq` is strictly greater
    **within its generation** (the writer-join row's scoping, V25-M2) and
    therefore already separates every later record from every earlier report, so
    the restarted counter is harmless and the node stages normally. Across the
    (β) wipe, where the generation itself restarts, the same conclusion holds by
    the other argument: with the cell absent there is no counterparty holding a
    value to collide with. The rule stays
    ...
    **One rule decides all thirty**: *if the store carries a readable
    ...
    - **Stamp absent or below `N`** (twenty cells: five shapes × two stamp
      ...
      precondition nothing has ever been minted here, so none of the twenty can
      re-use anything. **It is the *classification* that is uniform across all
      twenty, not the outcome** (V28-m9 — revision 27 wrote "the outcome is the
      same in all twenty" and then stated three different outcomes immediately
      below it): every cell classifies as *never created*, and the write each
      cell then performs differs by shape. **S-a/S-b/S-c**: write
      `stage_counter_state = Trusted { value: 0 }` **and `stage_counter = 0`**
      — §0's two-field clear (V29-M4) — (rewriting an
      ...
      *not* a reset to `0` — and bring the stamp to `N`; this is the shape an
      ...
      keep the state and treat it as the `≥ N` S-e cell below — **and bring the
      stamp to `N` exactly as the four cells above it do** (V28-m9: this was the
      one cell of thirty whose stamp disposition was unstated, against the
      totality claim; it is the same raise-or-first-mint write, and it is stated
      so that no cell is left without one). The shape is
      ...
      **This is the upgrade path**, and after V23-M3 it has two entrances: on
      ...
      store — both are cell S-a on their respective stamp states.
      Revision 21 believed this
      cell was "store present, field absent"; it is **S-a**, and reading it as
      a loss is V22-M2.
      **The second entrance reaches its cell only because of the
      FM-PERSISTENCE-048 amendment, and revision 23's "no operator-visible
      fault" was false without it** (V24-M1). A persistence-disabled or
      `fake`-WAL node of an existing cluster has an unmarked directory holding
      `raft/` and `replication_state.json`; `verify`'s populated-directory bail
      (`.../recovery/src/data_dir.rs:110-121`) fires on it, and the node never
      reaches phase 0's stamp, let alone this cell. With the two entries excused,
      `verify` mints, `stamp` writes `N`, the store is created with
      `stage_counter_state = Trusted { value: 0 }` **and `stage_counter = 0`**
      (§0's two-field clear), and the node boots and stages normally, emitting no
      operator-visible fault — the outcome revision 23 asserted, now with the
      change that produces it. **The dependency is directional and belongs to the
      sequencing plan**: the marker-phase move (the FM-PERSISTENCE-049 scope
      amendment) must not ship in a build that lacks the FM-PERSISTENCE-048
      widening, because the move is what puts `verify` in front of these
      directories in the first place.
      **What still refuses on this branch, and why it is not a cell.** An
      unmarked directory holding any entry outside the excused set refuses the
      boot at phase 0 — the retained property above. Like the unreadable-marker
      refusal, this is a **boot refusal one phase earlier**, owned by LOCKED
      FM-PERSISTENCE-051 rather than by this partition, so "populated but
      unmarked" is not a thirty-first cell. It does earn a recovery row, because
      the move makes it reachable on configurations that could never reach it
      before.
    - **Stamp at or above `N`, S-a/S-b/S-c** (six cells) → **lost**. With
      ...
      **boots and serves**, and the untrusted state is recorded in the same
      ...
      is entered**; write `stage_counter_state = Trusted { value: 0 }` **and
      `stage_counter = 0`** (§0's two-field clear) and boot as a non-member, for
      the reason the "with the cell absent" paragraph gives — the node can only
      return through a fresh registration whose `registration_seq` is strictly
      greater **within its generation** (V25-M2's scoping) and so separates
      every later record from every earlier report; and where the generation
      itself restarted (the (β) wipe), by the destroyed-counterparty argument
      instead. This is also the disjunct the wedge guard depends on, so the
      read that decides `self ∉ nodes` here is the one pinned ahead of any
      self-insertion. **S-c is the cell V22-C1 walked through**: it is the shape §0's
    ...
    **(b) single-member — `FLUSHALL`, then `CLUSTER RESET HARD`, then *restart
    ...
    2. **`CLUSTER RESET HARD`**, not the bare (SOFT) spelling: SOFT removes and
       ...
       **carried unchanged** (`commands.rs:833` → `:852`), so the predicate stays
       ...
       `Response::RaftNeeded`; `connection/cluster.rs:46-55` intercepts it and
       calls `handle_reset_command` (`:161-186`); `cluster/src/writer.rs:219-256`
       awaits `propose` → `client_write`).
       **Under this design's amendment to TR-CLUSTER-035** the same command
       durably marks this node's Raft store for discard **before it proposes**
       (revision 24's ordering — V24-M4; revision 23 said "before that `+OK`",
       which is after the apply and cannot refuse). If the mark cannot be
       written the command refuses here, having proposed nothing, and this exit
       is simply not taken. The mechanism, its crash windows, its residue and
       its spec obligation are at gate 1.
  ...
  **Ordering at boot, stated because seven steps read overlapping state — and
  stated with its members, not as a bare count** (V28-m10, applying V25-m4's
  stated-count discipline at doc "stale counts" rule: the seven are **(0)** the
  latch, **(i)** the applied-state read, **(ii)** the effect-keyed
  record-clearing clause, **(iii)** the counter-store rules, **(iv)** §0's
  incarnation re-mint, **(iv-b)** the data-directory stamp raise, and **(v)**
  the candidate re-mint plus gate 3 (c)'s resolution. Revision 21 said "four"
  and ordered four; §0's incarnation re-mint is one it left unplaced although it
  writes the same file the counter rules read — V22-C1 — and revision 23 added
  (iv-b), which reads the same latched `layout_version`, without re-deriving the
  header's count): **(0) latch the node-local observation.** Before any
  ...
  *primaries*, so on every replica all 16384 slots satisfied it and the rule would
  ...
  1. **Join-empty**: a node accepts cluster membership — the `MEET`/`AddNode`
     ...
     **The complete wipe sequence a rejoining node's operator must perform,
     ...
     is `frogdb-server/crates/cluster/src/commands.rs:816-862`, and that arm touches **no filesystem
     and no storage handle**. (**V28-m7**, the span settled by reading the code
     rather than the prior revisions: `:816` is the `ClusterCommand::ResetCluster {`
     pattern and `:862` the arm's closing brace — `:863` closes the enclosing
     `match`, so revisions 22-27's `816-863` was one line long. The other two
     spans this document uses are **sub-spans, not alternative arm spans**:
     `:833-854` is the if-branch that reduces membership to the resetting node,
     `:855-859` the `else` branch, and `:826-829` the release-event payment.
     Every arm-level citation now reads `816-862`; sub-span citations name what
     they are.) Nothing anywhere in the repository removes
     ...
     **Two further facts decide which vehicle is admissible where, and both are
     ...
     `handle_reset_command` `:161-186` → `cluster/src/writer.rs:219-256` →
     `client_write`), its apply arm is
     ...
     **What neither sequence touches, stated because the rest of this document
     ...
     stage counter. `CLUSTER RESET HARD` touches **neither the identity store
     nor its counter** (V28-m12 — not "no filesystem at all", the absolute
     revisions 23-27 wrote: V23-M2's own amendment gives the HARD path a durable
     **discard mark**, which is a filesystem write; the narrower claim is the one
     the argument needs and the one that survives the amendment), and the
     out-of-band step removes `<data-dir>/raft` and nothing else;
     `<data-dir>/frogdb_node_identity` survives both.
     **Revision 23 left the dozen contrary sites standing under the label
     "conservative rather than wrong"; revision 24 swept them, and the label was
     half false** (V24-M3). The half that was true: every *containment argument*
     built on the restart assumes the worse world and discharges the hazard with
     the `registration_seq` pairing regardless, and each stays sound when the
     counter survives, because a surviving counter only makes ids **more**
     unique, never less. The half that was false: several of those sites do not
     *assume* the restart hypothetically — they **assert it as a fact** in
     ...
     **replace a sound argument with a weaker one**: V20-C1 discarded the
     monotone stage-id argument in favour of a cell-deletion argument *because*
     it believed the counter restarted. Revision 24 therefore re-derives from
     the true premise everywhere rather than deleting the premise — the
     ...
     **That set was declared complete at revision 24 and was not; revision 26
     extends it by three, and the extension is recorded here rather than made
     silently** (V26-M1/V26-M3), because the standard this paragraph sets for
     the sites applies to the enumeration of them as well — an enumeration that
     claims a completeness it does not have is the mechanism by which the false
     fact survived two further rounds inside the machinery it underwrites. The
     three added sites are **gate 3 (a)'s "`stage_id` is not a fence operand"
     rule**, **gate 3 (d)'s "why the second component exists" necessity
     trace** (both of which asserted the restart as bare fact in normative
     text), and **the record-binding conjunct's forcing test**, whose fixture
     arranged a colliding `stage_id` *by* the mandated wipe — a fixture the true
     premise makes unconstructable, so the test had stopped witnessing the
     CRITICAL it was written for. All three are re-based on the real producer, a
     node-local durability loss (revision 26 also named an out-of-band restore;
     revision 27 drops that half from the load-bearing producer, because §0's
     scope declaration puts it outside the failure model — V27-M3).
     **A tenth member, and it is a history entry rather than a normative site**
     (V27-m2 — revision 26 edited it in the same commit and left it out of an
     enumeration whose entire purpose is to show what was touched, which is the
     failure mode this paragraph exists to prevent; the excuse that history
     entries need no *fixing* governs whether the site is normative, not whether
     an enumeration may omit it): the **revision-20 blast-radius entry** in the
     per-revision history below, whose "the LOCKED rows this round reads" cell
     named TR-CLUSTER-005 as counter-restarting. It is corrected there and marked
     as a revision-26 correction (V27-m3). It is counted here **as a
     history-entry correction, not as a normative assertion**, and no rule reads
     it.
     **Two further members, found in revision 28, take the enumerated set to
     twelve** (V28-M10): the **revision-20 changelog entry** asserts "a
     node-local counter the TR-CLUSTER-005 rejoin wipe restarts" as bare fact
     and was unmarked, and the **revision-23 entry** draws the "conservative
     rather than wrong" verdict V24-M3 overturned, also unmarked. Both are
     history entries; both are now marked in place.
     **The enumeration is retired and a mechanical rule replaces it** (V28-F0,
     and this is the finding the count itself was): three consecutive rounds
     extended a set that had each time been declared complete — nine, then ten,
     then twelve — which is the signature of a sweep *remembered* rather than
     performed. A count re-derived from memory cannot be audited. The obligation
     from revision 28 forward is **mechanical and re-runnable**: before any
     revision that touches the producer, the qualifier, the withdrawn reason or
     the wipe premise, grep this file for **six phrase families** and dispose of
     **every** hit — **(A)** `out-of-band` / `out of band`; **(B)** `restored out
     of band`; **(C)** `below a value` / `already spent` / `below the ids`;
     **(D)** `the wipe restarts` / `restarts the counter` / `restarts its
     counter` / `restarts the node-local`; **(E)** `no in-protocol operand` /
     `leaving no …operand`; **(F)** `touches no filesystem`.
     **What a hit is, and which file** (V29-M1 — revision 28's table stated
     neither, and its counts re-derived from neither candidate file): the greps
     run **case-insensitively** against **this file, as of the revision the
     table is published in** — the file the rule names, not the previous
     revision — and the **counting unit is the matching line**, so a line
     carrying the phrase twice counts once. The **exact command line** is
     recorded beside each family's count; **a count with no command beside it is
     not evidence**, which is the rule revision 28 wrote and then did not keep.
     **Dispositions — four, not three.** **(a)** correct as written. **(b)**
     repaired: the site was defective and its **text was changed**, and the entry
     **names the revision that changed it**; a site repaired in an earlier
     ...
     one. **A repair that *narrows* a claim owes exactly the same, and the
     `(b)` transfers to the command that spells the narrowed text**
     (**V33-M4**): V28-m12 replaced family F's absolute spelling with the
     identity-store form at seven sites, and the widened probe below — the
     command that reaches the narrowed spelling — therefore disposes those
     seven `(b)`, naming revision 28, rather than `(a)` on the ground that the
     narrowed text is correct as it now stands. Every repair leaves text that
     is correct as written; that is what a repair is, and reading it as `(a)`
     would launder every old repair out of the table on its first re-run.
     `(a)`/`(b)` attaches to the **site**, not to the individual occurrence —
     the family entries, the residue commands and the probes all take the site
     reading, and a table that mixed the two readings could not be checked
     against itself. **(c)** marked: a **history entry** that keeps its wrong text and
     ...
     meta: the hit is not an assertion in the design's own present voice but a
     **quotation** — the pattern strings inside this rule, the site lists inside
     this table, and occurrences inside the **dated per-revision history blocks**
     (everything above the `## Problem` header) that record a past finding rather
     than state a live claim. `(m)` hits **are counted**, because the greps are
     ...
     **The table has two parts** (V29-M1): **(i)** the grep output, disposed hit
     ...
     the phrase families are **incomplete**; its sites are **never** added to the
     part (i) counts, because a table that presents hand-enumeration as grep
     output is precisely the failure this rule exists to end — and that is what
     revision 28's table did with three of its four family-D "hits" and one of
     ...
     **provably blind**, and revision 29's own table proved it twice, by counting
     ...
     4. **non-meta** — `sed '3858,4738d;1,1504d' $F | grep -c -i -E '<P>'`, where
        `3858,4738` is this rule-and-table block's line range and `1504` is the
     ...
     **A fifth command, per *residue class* rather than per family** (**V31-M2**
     — revision 30 made (ii-a) mechanical and left (ii-b) an enumeration from
     memory, which is the disposition this whole rule exists to retire, and it
     was wrong again the round it was written). Where a family's residue class
     is **non-empty**, that family gains a second, deliberately **looser**
     pattern, run at the same four layers **plus a fifth**: `folded ∩ non-meta`,
     the folded command run over the `sed`-stripped corpus. The fifth exists
     because the residue is measured at the **folded** layer, and a folded
     ...
     **The third recorded deviation (`(d3)`) is the fifth layer's splice**
     ...
     seam the deletions produce — `3857` joined to `4739` (the head deletion is a
     ...
     **The engine is part of the record** (**V33-m4** — the rule above says a
     count with no command beside it is not evidence, and until this revision
     the commands named no `grep`, while the set-difference argument below
     turns on POSIX **leftmost-longest** alternation): every count published in
     this table was produced with `/usr/bin/grep` (BSD grep 2.6.0-FreeBSD)
     under `LC_ALL=C`, run from the repository root. **The requirement is the
     semantics, not the binary** — any re-run must use a POSIX
     leftmost-longest ERE engine, and a leftmost-**first** engine (most
     backtracking regex libraries, including Python's `re`) gives wrong
     containment answers on the very spans the subtraction turns on. Review v33
     re-ran thirteen of these commands under this machine's PATH `grep`
     (ugrep 7.5.0) and obtained identical figures on 13 of 13, so the
     recorded engine is a statement of what was used and of what a re-runner
     owes, not a claim that other engines diverge.
     ...
     **Range check**: `3858` is this rule's opening sentence, `4738` its last
     (`4739` opens a new sentence), and `1504` is the last line above the
     `## Problem` header. (Revision 32's range was `3763,4481d;1,1409d`; this
     revision's head changelog added 95 lines and its rule-block edits 162, and
     ...
     **No-double-ownership check**, standing beside it and in the same
     ...
     residue arithmetic.
     **Five further standing obligations, recorded here beside those two so the
     apparatus carries its own maintenance rules in one place** (revision 33,
     one per v33 finding):
     1. **Load-bearing `(m)` exhibit sites** (V33-M1). The containment
        argument's two worked exhibits live in **dated changelog blocks** — the
        **v26** block's summary of V24-M3's finding and the **v20** block's
        V20-C1 withdrawal marker. They are `(m)` text, so nothing in the fifth
        layer protects them: any rewording of either block moves family D's
        counts **and** silently invalidates the exhibits. Both are named by
        content anchor at the argument, and both must be re-read whenever
        either block is edited.
     2. **Inflection check** (V33-M5). A residue command is checked against the
        **plural** and the **participle** of its own head noun before its
        figures are published. Twice now an inflection has carried a live claim
        out of reach of the whole apparatus — family D's own `restarts`
        requirement, which is why `(D-s)` exists, and this command's singular
        head noun, which hid three live sites for a revision.
     3. **Narrowing transfers `(b)`** (V33-M4). Stated with the disposition
        rule above; repeated here as an obligation because it binds the
        *commands*, not only the entries: when a repair **narrows** a claim,
        the command that spells the **narrowed** text inherits the `(b)` and
        names the repairing revision, exactly as the polarity rule requires of
        `Pv`.
     4. **Prose restatements of layer counts** (V33-M2). Every count restated
        in prose is re-read off the recorded layer whenever that layer moves.
        Prose restatement is the apparatus's newest stale-count surface: the
        tables re-derive mechanically, a sentence does not, and revision 32
        moved `Pi`'s meta layer while updating only one of the two places that
        state it.
     5. **Section-anchor check** (V33-m1). Every "§X's Y" or "*Section*'s Y"
        attribution is verified against the heading that actually contains Y
        before publication. This class has now been corrected in three
        consecutive rounds (V30-m3, V32-m1, V33-m1), each time after the
        previous round's sweep fixed the named instances and not the class.
     **Part (i) — revision 33's sweep, run against revision 33**, i.e. against
     ...
     - **(A)** `out-of-band` / `out of band` — `grep -c -i -E 'out-of-band|out of
       band' $F` → **81 = 39/13/0/29**; occ **97** / folded
       **100** / non-meta **52**. Thirteen `(b)` **lines** across seven
       **sites** — the counting unit is the line, and one site (§0's scope
       ...
       excluded this design's own mandated `<data-dir>/raft` removal until the
       ...
       **relabelled from `(c)` to `(b)` by V29-M2**, because it is normative
       ...
       sentence**, act-to-class-of-id (rev 28, V28-M9); and **the record-binding
       conjunct's fence-freshness row**, `stage_id` operand cell (**rev 29**,
       V29-M3 — disposition overturned from revision 28's `(a)`). The **second**
       act-to-class-of-id site revision 28 listed as a `(b)`, the v22 changelog
       block's amendment note, is a **history-block hit and therefore `(m)`**
       under the rule above; its revision-28 repair is recorded in the v28 block
       and is not double-counted here. **No `(c)`**: after the relabel, this
       ...
       **blast-radius revision-20 row note** among them (V30-m4: it quotes the
       ...
       table said which disposition it holds); the 29
     ...
     - **(B)** `restored out of band` — `grep -c -i -E 'restored out of band' $F`
       ...
       *what this leaves* sentence (the residue closed by the scope declaration
       ...
       the **quotation of the qualifier V27-M3 withdrew**, with the withdrawal
       attached. **The row is listed here because revision 28's table omitted
       it** (V29-M1: family B was declared to have exactly two hits with both
       named, and this third one was undisposed — while being the same row as
     ...
     - **(C)** `below a value` / `already spent` / `below the ids` — `grep -c -i
       ...
       **with** its withdrawal, or the unrelated sense — "a durable value the
       ...
       "the set of ids this registration already spent".
     - **(D)** `the wipe restarts` / `restarts the counter` / `restarts its
     ...
     - **(F)** `touches no filesystem` — `grep -c -i -E 'touches no filesystem'
       ...
       **V30-M3**: revisions 28-29 named "the v24 changelog entry" here, and the
       ...
       unnamed fifth; **V31-m2** — revision 30 then named the site "gate 1's
       counter-survives re-derivation", which is a **different passage**, in §0,
       and is not this hit: families A and D name this paragraph the
       **deleted-write re-derivation**, and this family now names it the same
       way), and the forcing-test section's producer paragraph.
       **Revision 28
     ...
     **Markup**: the phrase is intact but an emphasis marker sits inside it, so
     ...
     - **(C)** `+1`, **wrap**, not normative: the **revision-28 changelog
       ...
       value` — **`(m)`**.
     - **(D)** `+9`, all **wrap**, two of them normative. **Gate 3 (d)'s V28-m2
       ...
       paragraph that withdraws it. The remaining **seven** are self-quotation:
       the rule's phrase-family list, family D's own recorded command line
       (which breaks three times, because it is the longest pattern), part
       (ii-b)'s record of the `Ps` pattern (which **contains** this family's
       pattern, so it necessarily breaks here too), part (ii-b)'s **(D-i)**
       quotation of the gate 3 (d) note, and part (ii)'s closing note about the
       same site — **`(m)`**.
     ...
     **(ii-b) — the residue, measured rather than remembered** (**V31-M2** —
     revision 30 wrote that "three normative or model-level statements of the
     corrected fact use a spelling no family carries", found by **reading**, and
     the V30 changelog then claimed part (ii) "enumerates in full". Part (ii-a)
     did; part (ii-b) did not. **Nine** further live sites sat in the same
     normative or model-level register in spellings no family carries, and
     ...
     **(D-s) — the subject-and-inflection class**: the corrected fact spelled
     ...
     repository root against this file at the same four layers as every family,
     **plus a fifth** the subtraction needs — `folded ∩ non-meta`, the folded
     command run over the `sed`-stripped corpus, which is what makes the meta
     split of a folded count command-derived rather than eyeballed. Write `Ps`
     for the pattern `the wipe restarts|restarts the counter|restarts its
     ...
     **Why the subtraction is a set difference** (**V32-M3** — revision 31 wrote
     ...
     deletes. The two the finding turns on are in the stripped head region, and
     both sit in **dated changelog blocks**: the D span
     *"restarts the node-local"* sits inside
     `Ps`'s *"restarts the node-local **stage** counter"* in the
     **v26 changelog block**, in that block's own summary of V24-M3's finding,
     and the same D span sits inside
     *"restarts the node-local counter"* in the **v20 changelog block**, inside
     the *premise withdrawn by V24-M3* marker its V20-C1 entry carries.
     (**V33-M1** — revision 32 placed the first at "the revision-20 head-region
     site" and the second at "the revision-23 head-region site", which is wrong
     twice over: the two variants are **swapped** relative to the revisions
     named, and the **v23 changelog block carries no family-D hit at all**, so
     one of the two named sites does not exist. Both labels were adopted
     **verbatim from the v32 review's own prescribed-fix sentence** — the review
     was right about the spans and wrong about the sites, and revision 32 took
     the prescription without re-checking it. The fix-text ladder below now has
     a rung whose source is the **reviewer** rather than the drafter, which is
     recorded rather than smoothed over, because a ladder that quietly re-labels
     its own instances stops counting anything.)
     The other four are quotations of those two — two in the **v32** changelog
     block, also head-region, and two in this paragraph itself, inside
     ...
     **The 14 non-meta occurrences, across eleven sites, enumerated in full:**
     - *(a)* **§0's `stage-counter-untrusted` bullet** — "so neither restarts
       the stage counter": the normative statement, missed by family D on the
       *stage*-qualified spelling (V30-m2).
     - *(a)* **gate 1's wipe-sequence derivation** — "the wipe does not
       destroy the node-local identity store, and therefore does not restart the
       stage counter", **wrapped** at the line break as well as spelled with the
       bare infinitive. **This is the site the rest of the document defers to**
       ("stated at gate 1"), and no recorded command had ever seen it.
     - *(a)* **§0's V20-C1 necessity re-derivation** — "that later record would
       be minted from a restarted counter": the hazard in the counterfactual
       voice.
     - *(a)* **§0's two-write clearing rule** — **four** occurrences: the
       necessity trace's *restarted counter* named in the V29-M4 marker; "what
       licenses *restarting the counter*"; "a restarted counter is unsafe only
       if some live operand still carries an id at or above the restarted
       value"; and "writing the counter alone would restart the counter while
       the node still refuses every mint". These are **not** contrary claims:
       the restart they license is this design's own clearing write
       `stage_counter = 0`, not anything a wipe does. Untouched by this
       revision.
     - *(a)* **§0's (β)-path re-derivation** — "across the single-member in-band
       wipe the generation restarts": the **generation** subject, which (β)
       genuinely does restart.
     - *(a)* **§0's harmlessness paragraph** — "the restarted counter is
       harmless and the node stages normally": the same licensed-clearing sense.
     - *(a)* **the record-binding conjunct's fence-freshness row** — "across the
       (β) single-member in-band wipe the generation restarts": the same
       generation subject, and the ground of the row's canonical-generation
       declaration.
     ...
     - *(a)* **the *Quint rework* section's kill re-examination** — "the
       mandated wipe sequences do not restart the counter", **wrapped**; the
       **same section** as the state bullet above, which is how a reading of the
       section missed it.
     - *(a)* **the *Testing* section's forcing-test producer paragraph** — "the
       mandated wipe sequences do not restart the node-local stage counter",
       **wrapped**.
     **The 24 meta occurrences are disposed as a class** by the `(m)`
     rule — this rule's own pattern strings, this table's quotations of the
     sites just listed, and the dated history blocks' records of the same
     findings (the v23, v24, v29 and v30 blocks) — **except one, which states a
     live claim and therefore takes a disposition of its own**: *(a)* the
     **revision-24 changelog block**'s V24-M3 entry, "revision 23 discovered
     that neither mandated sequence **restarts the stage counter**" — **stating
     the corrected fact in the entry that establishes it**, in the
     *stage*-qualified spelling family D does not carry (**V31-M1** — revision
     30 described this site as "quoting the withdrawn premise inside the entry
     that withdraws it", which the site's own text falsifies: the withdrawn
     premise is *that the wipe restarts the counter*, and the entry never states
     it — it states the negation, in the same construction as §0's bullet listed
     above it, which is what this list's own header called all three of its
     entries. The construction revision 30 named is real and
     **disposition-bearing** — it is the stated ground of family A's
     blast-radius revision-20 `(a)` and of family C's `(a)` class — which is why
     attributing it to a site that does not carry it is a false disposition
     claim in the one table whose purpose is that dispositions be checkable
     against text, and not a wording slip. A genuine instance of the
     construction is the live-claim rule above, quoting revision 29's superseded
     "the two such sites" sentence inside the paragraph that replaces it.)
     ...
     **whole**: **26 meta + 7 non-meta = 33**, the non-meta half being the
     ...
     - *(a)* **gate 1's V24-M3 sweep passage** — **two** occurrences: "the
       mandated wipe restarts the counter" (the `admitted_stage` writer cell)
       and, **wrapped**, "Across the mandated wipe the counter restarts" (the
       adoption-fence row). Both are false sites the passage quotes in order to
       sweep them, and each quotation carries its own correction. **New in
       revision 31**: no earlier revision's part (ii) held this site, because no
       recorded command reached the inverted spelling and reading did not find
       it.
     - *(a)* **§0's retired-enumeration paragraph** — "a node-local counter the
       TR-CLUSTER-005 rejoin wipe restarts", the V28-M10 sentence's **quotation
       of the revision-20 changelog entry** listed as `(c)` below. It sits four
       lines above this rule's opening sentence and is therefore **non-meta by
       the recorded boundary**, which is the honest place for it: it states what
       the history entry asserts, in order to say the entry is now marked.
       **New in revision 31**, on the same grounds as the site above it.
     - *(a)* **gate 3 (d)'s V28-m2 note** — "neither mandated wipe restarts the
       counter". **Already carried**: it is family D's wrapped hit,
       dispositioned in **(ii-a)**.
     ...
     - *(a)* **the drain machinery's observation bound** (*Transitions*,
       ...
       *Transitions*, and a wrong **content anchor** is the one error the
       ...
       scope exactly as family C's non-defective hits are.
     **The 26 meta occurrences of `Pi` are disposed as a class** by the
     `(m)` rule, **except the three history-block sites that keep or state a
     claim** — revision 30 dispositioned these three by reading, and the
     dispositions stand:
     ...
     - *(c)* the **revision-23 changelog block**'s entry — this document's dozen
       "the **counter restarts** across the wipe" claims called *conservative
       rather than wrong* — keeps its overturned verdict and carries the *verdict
       overturned by V24-M3* marker (V28-M10).
     - *(a)* the **revision-25 changelog block**'s V25-M2 entry — "the
     ...
     **positive** voice — the counter *survives*, or *does not restart* — which
     ...
     count; the disjointness claim held of revision 32's three alternations and
     is **narrowed** by the fourth added below, whose single occurrence sits on
     the hit lines of families A, B, C, E and F). Recorded command, with `Pv`
     written for the pattern `counters?
     surviv[a-z]*|surviv[a-z]* (the |a |its |stage |real |node-local
     )*counters?|counter (does not|doesn.t|cannot)
     restart[a-z]*|preserv[a-z]* (the |its |a )*(stage |node-local
     |real )*counters?` — one line,
     wrapped here for width as the others are.
     **Widened twice in revision 33** (**V33-M5**, **V33-m3**). Revision 32's
     three alternations every one required the **singular** noun adjacent to the
     verb, so the **plural** spelling escaped all three: **five** folded
     occurrences at revision 32 — four non-meta, the fifth the v17 block's note
     quoted below — and **three of those four were reached
     by no recorded command whatever** — §0's *Counters survived* paragraph,
     which is the live statement of the corrected premise in the very sentence
     that settles which case this design is in; §0's stale-`Boot` sub-bullet;
     and the Demotion arm's fence row. That is V32-M2's failure shape exactly,
     one **inflection** over: a residue command that measures the spelling its
     author happened to write rather than the class. The first two alternations
     now spell `counters?`. The third stays singular **on measurement, not on
     taste**: `counters (do not|don.t|cannot) restart[a-z]*` returns **zero** at
     every layer, file-wide, and that check is recorded here so it is not
     re-run from memory. The **fourth** alternation reaches the **preserve**
     voice (**V33-m3**): "revision 20 drove it with the mandated wipe, which in
     fact preserves the counter" is the file's only preserve-voiced statement
     of the premise, and it was reachable by **co-location** alone — the reach
     this command's own `(b)` argument below rejects as insufficient, applied
     to itself. The plural reaches **twelve** folded occurrences at revision 33,
     the seven beyond revision 32's five being this entry's and the enumeration's
     own quotations of the four non-meta sites. `grep -c -i -E '<Pv>' $F` →
     **25**; occ **27** / folded **34** / non-meta **10** / folded non-meta
     **12**.
     ...
     hit set is enumerated **whole**: **22 meta + 12 non-meta = 34**. The
     meta occurrences are this table's own quotations of the sites below, this
     bullet's own pattern string, and the **v17 changelog block**'s note that
     the *"counters survive removal"* premise was false; they are
     ...
     the fold reaches it.
     The **12** non-meta occurrences, across **eleven** sites, are:
     - *(a)* **§0's `run_identity`-lifecycle stale-`Boot` sub-bullet** —
       "refused by the ordering conjunct when the **counters survived**, and may
       be admitted when they did not". Correct as written: the bullet states an
       outcome under each fate of the counter and takes no position on which
       fate the mandated sequences produce. **New in revision 33** (V33-M5) —
       the plural inflection held it out of reach of every recorded command.
     - *(a)* **§0's *Counters survived* paragraph** — "**Counters survived** —
       which is the mandated sequences' case, not one of two". Correct as
       written, and this is the **live normative statement of the corrected
       premise**, in the sentence that settles which case the design is in.
       **New in revision 33**, and the sharpest of the plural sites: the very
       claim `Pv` was added to reach was invisible to the whole apparatus for
       the revision that added it.
     - *(a)* **gate 1's V24-M3 sweep passage**, its *half that was true*
       ...
       licenses.
     - *(a)* **the `ReportRunIdentity` Demotion arm's fence row** — its reasoned
       deviation from the v18 review's prescribed leg, "true only when the
       node's **counters survived**". Correct as written: the deviation is
       stated on the counter-loss branch, and the sentence names which branch it
       is on. **New in revision 33** (V33-M5).
     ...
     - *(a)* **the `AttestReplicaSynced` fence row's *why the operand changed*
       paragraph** — revision 17's two-leg argument quoted as "either the
       **counters survived** and the re-mint orders above, or they did not",
       with V18-C1's withdrawal of the second leg attached. Correct as written:
       a withdrawal quoting what it withdraws. **New in revision 33** (V33-M5).
     - *(a)* **the record-binding conjunct's V24-M3/V27-M1 correction note** —
       "revision 20 drove it with the mandated wipe, which in fact
       **preserves the counter**". Correct as written, and it is the file's
       **only** preserve-voiced statement of the premise. It sits on the hit
       lines of families A, B, C, E and F, so a re-run has always surfaced the
       *line* — but no recorded command could **spell** it, which is reach by
       **co-location** and nothing else, the standard the `(b)` entry above
       rejects. **New in revision 33** (V33-m3), reached by the fourth
       alternation.
     ...
     **The other five families' residue classes are probed, not assumed** — the
     ...
     count:
     - **(A)** `out[- ]?of[- ]?band` → **100**, identical to family A's folded
       **100**. **Empty residue**: the optional-separator forms are exactly what
       the family's two alternatives already carry, and no third form occurs.
     ...
     - **(C)** `below (a|the|any|some) [a-z -]{0,24}(value|id|ids)|already
       ...
       the v21 changelog block, in this table's quotation of it, and in **this
       bullet's own** quotation: all three `(m)`, in the head region or in this
       block.
     - **(E)** `in-protocol operand` → **12**, against family E's folded **11**
       — and here the comparison must be made against the family's **occ 12**
       instead, because E's folded number is the one the bounded wildcard
       depresses (deviation **(d2)**), so a raw folded-against-folded reading
       would manufacture a residue that is really the merge. Of the twelve, four
       are not family-E hits: the (ii-a) quotation of the wrapped site, `(m)`;
       **this bullet's** pattern string and its quotation, both `(m)`; and the
       **counter-statement** sense — "An in-protocol operand demonstrably
     ...
     - **(F)** `touch[a-z]* (neither|no) [a-z ]{0,30}(filesystem|file
       ...
       **14**. **This probe is widened in revision 32, and the widening is itself
       ...
       `opens no file` (the same pattern reads **21** against revisions 32 and
       33 alike, the rise over 17 being revision 32's own meta text; the
       `neither` branch still contributes **zero**); the
       ...
       **`opens no file` — 7 occurrences, all `(m)`**: family F's own entry
       ...
       `CLUSTER RESET HARD` restatement, and the **v32** changelog entry, which
       is markup-split and reached at the folded layer only)
       and four in this rule-and-table block (family F's part-(i) entry, the
       (ii-b) opener, and this bullet's own two restatements) —
       and **7** are non-meta and **all `(b)`, revision 28** (**V33-M4** —
       revision 32 declared them "all `(a)`", which contradicts family F's own
       part-(i) entry disposing five of these same sites `(b)` for the same
       repair, and discards the repair record this widened command exists to
       make re-derivable; the **narrowing-transfers-`(b)`** rule is stated once,
       beside the polarity clause of the disposition rule above). Each states
       ...
       producer paragraph" here, which carries no occurrence of the narrowed
       spelling at all, while this site, which does, went unnamed; the document
       names it *gate 3 (a)'s fence-operand paragraph* at the blast-radius
       V28-M1 entry, at family A's own part-(i) entry and in the v26 block's
       V26-M1 summary); the **staged-flip** adoption-fence row; the
       **record-binding** conjunct's fence-freshness row; gate 3 (c)'s
       re-used-`k` producer paragraph (family F's entry names the same
       paragraph the **deleted-write re-derivation** in *Transitions*); and the
       *Testing* section's forcing-test producer paragraph.
       **Why family F counts five `(b)`s here and this command counts seven**:
       at two of the seven — gate 1's wipe-sequence derivation and gate 3 (a)'s
       fence-operand paragraph — the repair left **no** copy of the absolute
       phrase behind in a repair note, so family F's own pattern cannot see
       those sites and its entry cannot count them. That gap *is* the residue,
       and it is what the widened command makes re-derivable.
       ...
       **Three more** sit one line from one, inside the same repair note or
       bullet: §0's collision-freedom re-derivation, gate 3 (c)'s re-used-`k`
       producer paragraph, and the *Testing* section's forcing-test producer
       paragraph. **Two sit on no family-F hit line and beside none**: gate 3
       (a)'s fence-operand paragraph, and — the one that matters — **gate 1's
       wipe-sequence derivation**, from
     ...
     **Gate 3 (d)'s V28-m2 note** — "neither mandated **wipe restarts the**
       ...
       **What survives as evidence is (ii-b), and it is no longer a reading**:
       folding and markup-stripping cannot reach a site the pattern does not
       **spell**, so the residue is reached by *widening the pattern* and
       subtracting. The phrase families are still a **filter, not a proof** —
       but the size of what they filter out is now a published number with a
       command beside it, and the honest form of the incompleteness claim is
       that measurement rather than a list of exemplars.
     ...
     **mark-then-wipe-on-restart**, and it fits the restart the exit already
     needs for an unrelated reason (HARD's re-key is in-memory only). The mark is
     `<data-dir>/frogdb_raft_discard`, whose path is owned by `DataDirLayout`
     under FM-PERSISTENCE-057 — **Amended** to carry it, V24-m1 — and which is
     published with the same `write` → `sync_file` → `rename` →
     `sync_dir` shape as the marker. Its writer join is **Table C** of the fifth
     discipline. The **next boot** completes it: recovery
     removes `<data-dir>/raft` and then the mark, **before** phase 6 opens the
     store (`frogdb-server/crates/recovery/src/cluster.rs:23-25`).
     **Where the mark is written is the whole question, and revision 23 answered
     it by accident** (V24-M4). Revision 23 placed the write at the node the
     payload names, "**before the reset's `+OK` is written**", calling that "a
     placement the code already admits because the reply waits for commit *and*
     apply". That sentence names the **post-apply** ordering: commit has
     happened, every member has erased its topology, and only the reply is still
     outstanding. Revision 24 analyses both orderings, chooses, and excludes the
     other by name.
     - **Mark-after-apply — rejected.** The apply arm is reached only on a
       committed entry, and LOCKED FM-CLUSTER-006 states of that arm, under
       *not* observable: "A reset that fails: the arm is infallible by
       construction, because a half-reset node is worse than a fully reset one."
       An IO failure at this point therefore has nowhere to go. Either the node
       returns `+OK` with the wipe **silently disarmed** — it reboots into its
       old Raft store, gate 1 refuses the rejoin forever, and the operator was
       told the reset succeeded, which is a durable obligation dropped under a
       success reply and a direct breach of the fail-closed recovery discipline —
       or it returns an error after a committed apply, so the reply disagrees
       with the replicated outcome at every other member. Both are worse than
       refusing. The ordering also fails a cheaper test: the failure is
       node-local, but by the time it is discovered the effect is cluster-wide
       and unrecallable.
     - **Mark-before-propose — chosen.** The write happens at the command layer,
       on the node the operator issued the command at, **before anything is
       proposed**: inside `handle_reset_command`
       (`frogdb-server/crates/server/src/connection/cluster.rs:161-186`), at the
       seam between its cluster-mode guard and the writer construction (`:172-174`),
       ahead of `propose_reset` at `:175`
       (`frogdb-server/crates/cluster/src/writer.rs:219-256`). **The citation is
       the function body, not its call site** (V25-m1 — revision 24 cited
       `connection/cluster.rs:46-55`, which is the `ResetCluster` early-return
       *inside the generic Raft-op dispatch*: it merely calls
       `handle_reset_command` at `:52-54`, holds no cluster-state handle, and
       sits in a function every Raft-proposed admin command flows through. An
       implementer who writes the durable side effect there writes it into the
       shared dispatch instead of the reset's own path, which is why the range
       matters even though the function name revision 24 gave was right).
       At that point nothing is committed, no topology has been erased, and the
       path is still allowed to refuse — which is the whole reason to prefer it.
       **HARD only**: SOFT keeps its Raft store, so the SOFT path takes no
       obligation. **It runs after the existing keyspace refusal**
       (`admin.rs:404-409`), which is evaluated while producing the
       `Response::RaftNeeded` the interception consumes, so a `CLUSTER RESET` on
       a non-empty keyspace never writes a mark.
     **Disposition on failure, with its literal token.** If the mark cannot be
     made durable, the command is refused **without proposing**:
     `-ERR CLUSTER RESET HARD refused: the Raft store could not be marked for
     discard`. Repair is a disk repair, then re-issue. The cluster is untouched
     — this is the fail-closed answer the post-apply ordering could not give.
     **LOCKED FM-CLUSTER-006 is confronted and comes through unbreached**, which
     is why no amendment to it is declared. The row's *not observable* cell
     forbids a failing **arm**; this refusal is not in the arm, and the arm is
     unchanged and still infallible. The row's own Trigger already contemplates a
     command-layer refusal ahead of Raft — a non-empty keyspace "is refused at
     the command layer before it reaches Raft" — and its Observable cell already
     admits command-layer errors, so a second command-layer refusal on the same
     path is a **new instance of a shape the row already carries**, not a new
     shape. Had mark-after-apply been chosen, the row *would* have been breached,
     and the amendment would have had to be declared; that is the second reason
     the ordering matters.
     **TR-CLUSTER-035's amendment is re-shaped, not withdrawn, and it remains
     one amendment rather than two.** Revision 23 declared it against the
     Postcondition ("HARD only, additionally: `node_id` -> `new_node_id` …");
     revision 24 moves the obligation to the **Precondition**, which today reads
     "Node-issued `ResetCluster{node_id, new_node_id}`; `new_node_id: Some`
     selects HARD." The delta: on the HARD path the op is proposed **only after**
     the issuing node has durably written `<data-dir>/frogdb_raft_discard`, and
     the Postcondition gains the completion — the next boot removes
     `<data-dir>/raft` and then the mark, before the store is opened. The
     `else`-branch note in the Postcondition (`commands.rs:855-859`, reset racing
     a `FORGET`) is untouched.
     Crash windows, re-derived on the chosen ordering:
     - Crash **before the mark is durable**: nothing was proposed, so nothing
       happened at all — the node boots with its Raft state and the cluster's
       topology intact, and the operator re-issues. Revision 23's version of this
       window left a *committed* reset with an unarmed wipe; that window no
       longer exists, which is the ordering's main benefit.
     - Crash **after the mark is durable, before the propose commits**: the next
       boot wipes `<data-dir>/raft` and clears the mark, and the node comes up
       with empty Raft state. For the single-member exit this **is** the
       sequence's intended terminal state — the reset's only durable effect was
       the wipe, HARD's re-key never survives a restart anyway, and the applied
       state dies with the log — so the operator has lost the `+OK` and nothing
       else. For a node that is still a member of a live multi-member cluster
       this window is **a different state from the stale-mark residue below, and
       it gets its own recovery row** (V25-M1 — revision 24 routed it to the
       residue's row and said it "carries the same recovery row"; it cannot,
       because **both** of that row's exits presuppose a node that is up and a
       boot that has not happened). Here the node is **down**, the boot *is* the
       wipe, and `raft_discard_pending` is read from a file that same boot
       deletes — so by the time any surface is readable the live member's Raft
       log, vote and state machine are already destroyed for a reset that never
       committed. The routing is to the recovery table's **crashed-node** row
       (armed mark, node down), whose only exit is removing
       `<data-dir>/frogdb_raft_discard` on the **stopped** node before starting
       it, under the same not-committed precondition the running-node row now
       carries. This is also the window that makes issue 25's solo-bootstrap
       behaviour a **blocking prerequisite of the mark mechanism** rather than a
       recorded dependency of gate 1 — see the dependency paragraph below, which
       states why in split-brain terms.
     ...
     **The residue the chosen ordering creates, declared with its recovery.** A
     mark can outlive a reset that never commits: the propose is rejected, times
     out, or loses its leader. **The command path does not remove the mark**, and
     that is deliberate — an indeterminate propose may still commit under a later
     leader, and un-marking would disarm a wipe that a committed reset requires.
     Fail-closed here means leaving the mark armed. An armed mark does nothing
     while the node runs; it acts at the next boot, and on a node that is still a
     member that boot discards a live Raft store. So the residue is **surfaced
     in-band** — `CLUSTER INFO` gains `raft_discard_pending:0|1`, read from the
     file's presence — and it has **two operator commands**, per the fail-closed
     recovery discipline's three-part rule: complete the reset by re-issuing
     `CLUSTER RESET HARD` (idempotent — the mark is re-written, and the wipe is
     what the operator wanted), or abandon it by removing
     `<data-dir>/frogdb_raft_discard` at the **running** node before any restart.
     **"Abandon" carries a precondition, and revision 24 promised it
     unconditionally** (V25-M1's K1 half): the same indeterminacy that forbids
     auto-un-marking binds the operator, so before removing the mark the operator
     must establish **at a surviving member** that the reset did **not** commit —
     `CLUSTER NODES` at a survivor still listing this node is that evidence.
     **That evidence is a read of one member's *applied* state, and it can be
     wrong in exactly one direction** (V26-m1): a committed `ResetCluster`
     empties every member's `nodes` map, but commit precedes apply at any given
     follower, so a survivor that is lagging or partitioned from the leader that
     committed the entry **still lists this node** after the commit and the
     operator reads a false "did not commit". The prescription is freshness plus
     fail-closed: prefer a member the operator can confirm is in the **current
     leader's majority**, and if two survivors disagree, read the answer as
     **"committed"** and leave the mark armed. **The residue of reading it wrong
     is the one this paragraph already prices, not a new one**, which is why the
     caveat is a freshness prescription rather than a new mechanism: on a
     reset that did commit, an abandoned mark leaves the node holding a Raft
     store the committed reset required it to discard, and gate 1 then refuses
     its rejoin; the escape from *that* state is to re-issue `CLUSTER RESET HARD`
     and let the mark be re-written. **The recovery table below carries two rows,
     not one** — the residue at a **running** node (this paragraph) and the
     crash-window state at a **stopped** one (the window above) — because the
     exits differ in the only way that matters: whether the node can be issued a
     command at all.
     **The mark is an *intent*, written ahead of its effect, and that is a
     declared exception to this design's level-triggered rule for durable
     reactions** — a sharper exception than revision 23's, which described a
     durable reaction to an observed apply. No level predicate can re-derive this
     intent: "`self ∉ nodes`" is equally true after a `FORGET`, where wiping
     unilaterally would be exactly the destructive act gate 1 exists to prevent,
     and after the propose fails there is no applied fact to read at all. The
     exception is admitted because the alternative is the post-apply ordering,
     and it is paid for by the surfacing and the two operator commands above
     rather than by a re-derivation.
     Forcing tests: `reset_hard_marks_the_raft_store_for_discard_before_it_proposes`,
     `a_reset_hard_whose_mark_cannot_be_written_refuses_without_proposing`
     (asserting **no** `ResetCluster` entry is proposed, which is what
     distinguishes the two orderings and is the negative control for the choice),
     `the_next_boot_wipes_a_marked_raft_store_before_opening_it`,
     `a_crash_before_the_discard_mark_leaves_the_cluster_untouched`,
     `a_mark_left_by_a_failed_propose_is_surfaced_in_cluster_info`,
     `a_single_member_reset_hard_then_restart_comes_up_with_empty_raft_state`,
     and, new in revision 25 for the crash window and its recovery row (V25-M1),
     `a_crash_between_the_mark_and_the_commit_leaves_a_recoverable_node` — kill
     the node after the mark is durable and before the entry commits, assert at a
     surviving member that the reset did **not** commit, remove the mark on the
     **stopped** node, start it, and assert it rejoins with its Raft store intact.
     Its **negative control**, which is the half that makes it a test of the
     recovery row rather than of the removal:
     `a_crash_between_the_mark_and_the_commit_destroys_the_store_if_the_mark_is_left`
     — same trigger, mark left armed, assert the boot wipes and (until the
     prerequisite below lands) that the node comes up in a Raft group of its own.
     **One named prerequisite — promoted in revision 25 from a recorded
     dependency to a BLOCKING one** (V25-M1). A node whose Raft store is empty
     **self-bootstraps** today — it inserts
     ...
     Raft-state half of the gate is unsatisfiable by any sequence.
     **Why this is blocking rather than recorded, in split-brain terms** (V25-M1):
     the crash window two bullets up gives the wipe a trigger that **no operator
     ever asked for**. Take a live three-member cluster; `CLUSTER RESET HARD` is
     issued at member N (gate 2 inspects one shard only, so a multi-shard node
     with keys elsewhere passes it); the mark is published; power is lost before
     the entry commits. Member N's next boot wipes a Raft store that the other two
     members still count as a voter, and the solo bootstrap then inserts N into a
     Raft group of its own. **One node is now a member of two Raft groups** — the
     survivors' and its own — each able to elect a leader and accept proposals for
     the same slots. That is not the availability nuisance the recorded dependency
     described ("the wipe is undone on the very next boot"); it is a **split-brain
     source**, produced by a single power loss on a single node, and the mark
     mechanism is what creates its trigger. So `a_wiped_node_awaits_meet_instead_of_solo_bootstrapping`
     is a **prerequisite of the mark mechanism itself**: the mark may not ship
     ahead of it, because arming a wipe on a live member is only safe if the wiped
     node comes up as a **non-member awaiting `MEET`**. The blast radius and the
     sequencing plan carry the same promotion. Forcing test:
  ...
  3. **Replication-command gate** (V10-C1). First, the **cluster-mode role-command
     ...
     **The second of those is what the
     ...
     own** (V20-C1 — the lesson of the eleventh round; the *reason* is
     **re-based in revision 26**, V26-M1, which found this site still naming the
     mandated wipe as the producer four rounds after V24-M3 withdrew it): it is
     **node-local**, and the pairing-partner rule of the fence-freshness
     discipline disqualifies every operand whose value is produced on the node.
     The producer of a re-used id is **not** the wipe TR-CLUSTER-005 mandates —
     neither mandated sequence restarts the counter (`CLUSTER RESET HARD`
     touches neither the identity store nor its counter, the out-of-band sequence
     removes `<data-dir>/raft`
     alone, and `<data-dir>/frogdb_node_identity` survives both; stated at gate
     1) — it is a node-local **durability loss** (an out-of-band restore rewinds
     the same value but is outside the failure model, so nothing here rests on
     it), which is §0's counter bullet ("Counters destroyed — still a
     real case, with a different producer"). **V28-M1**: revision 27 declared the
     restore half dropped from the load-bearing producer at three sites and
     executed it at two; this was the third, and it additionally carried
     "from a copy whose counter reads below the ids already spent" — the
     paraphrase of the qualifier V27-M3 withdrew from every restatement.
     One single-node event that can
     rewind the operand is enough to fail the rule, so every conjunct
     ...
     **Why the second component exists, and why revision 19's single component
     was a CRITICAL** (V20-C1; the trace's **producer re-based in revision 26 and
     again in revision 27, and in revision 27 re-derived against the six-arm
     partition below rather than against intuition** — V27-M1/V27-M3. Revisions
     20 through 25 drove it with the mandated wipe, which V24-M3 established is
     no producer of this shape at all; revision 26 drove it with an out-of-band
     restore, which §0's scope declaration puts **outside** the failure model,
     and asserted a harm the partition makes **unreachable** on the path it
     described. The producer is now the one this row's own Scope sentence puts
     *inside* the model — "these rules cover counter **loss**" — and the harm is
     read off the arm the refusal actually selects):

     `stage_id` is a **node-local** counter, so what can re-issue a spent id is a
     node-local **durability loss** of the store that counter lives in, composed
     with §0's own mandated **multi-member exit** from the
     `stage-counter-untrusted` state that loss raises. An out-of-band restore
     produces the same shape, and this design deliberately does **not** rest on
     it: §0 excludes it by the *act*, so a trace built on it would sit outside
     its own row's scope.

     **The producer, in steps §0 already owns.** Node `n` is a member at
     `registration_seq = 11` and stages a flip: gate 3 (a) mints `stage_id = 1`,
     the record is written, the whole-node fence goes up, and the `Demotion`
     report — carrying `observed_registration_seq = 11` — is **delayed in the
     log** (TR-CLUSTER-041's forwarded-proposal delay), so it is never admitted
     and `nodes[n].admitted_stage` stays `None`. `n` then loses the node-local
     store its `stage_counter` lives in: §0 clears the surviving record
     (fsynced), the node comes up **serving**, and the durable
     `stage-counter-untrusted` state refuses every `CLUSTER REPLICATE`
     node-locally. At a multi-member cluster the **only** exit from that state is
     the recovery row's — **`CLUSTER FORGET n` at a survivor, then re-`MEET`**.
     `FORGET` **deletes `n`'s cell outright**, `run_identity`, `admitted_stage`
     and `registration_seq` with it; the re-`MEET` returns through `AddNode`'s
     fresh arm, which mints `registration_seq = 12` from the survivors'
     **untouched** generation (the (α) shape) into a cell whose `run_identity` is
     **absent** — TR-CLUSTER-001's fresh arm initializes it so — and whose
     `admitted_stage` is `None`. The fresh registration clears the untrusted
     state by writing `stage_counter_state = Trusted { value: 0 }` **and
     `stage_counter = 0`** — §0's two-field clear, both in one atomic write, and
     it is the second write that makes the `max(0, …)` below read `0`. `n` stages
     again, and the mint-time floor is **non-binding for a stated reason**:
     `max(stage_counter, nodes[self].admitted_stage or 0) + 1` is
     `max(0, 0) + 1 = 1`, the counter having been cleared to `0` by the untrusted
     exit and the cell the floor reads `admitted_stage` from being the one
     `FORGET` removed. The new record `S1'` therefore carries **the same
     `stage_id = 1`** as the dead stage `S1`, with
     `staged_registration_seq = 12`, the record fsynced and the whole-node fence
     up. **`S1'`'s own `Demotion` report is *held unproposed* here, and the hold
     is the boot ordering rule's** (V28-M4 — revisions 20 through 27 pinned that
     report as "in flight" at this point, an interleaving §0's own boot ordering
     rule forbids): `n`'s post-rejoin `Boot` report has been **proposed and has
     not applied**, and until it applies `n` proposes no other Raft command.
     Staging is node-local and proposes nothing, so the record exists while the
     report waits; §0's boot-rule / reconcile-guard composition states that
     precedence once and is what licenses this state. Every step is a mandated
     sequence or a declared §0 rule; nothing in it is out of scope.

     **What the unpaired conjunct then does, read off the arm table.** `S1`'s
     delayed report is delivered and refused **before `n`'s first post-rejoin
     identity report applies** — the same window the hold above runs in, so the
     two facts are one fact — so the re-created cell's `run_identity` is still
     **absent**. That state is not invented here: it is the fence-freshness row's
     **leg (a)** verbatim — "the re-created cell before the rejoined node's first
     report" — and that row declares its class. Stepping the declared evaluation
     order (`proposer`, `membership`, `ordering`, `fence`, `upstream-validity`,
     `shard-relationship`; the class **is** the first conjunct that failed):
     `proposer` passes (`n` proposed its own report); `membership` passes
     (`n ∈ nodes` at registration 12); `ordering` **fails**, because V18-M1's
     narrowed absent-operand rule makes the ordering conjunct false for
     `kind = Demotion` against an absent stored `run_identity`. **Class
     `ordering`** — and because the class is the *first* failing conjunct, the
     later `upstream-validity` / `shard-relationship` conjuncts are never
     reached, so a **refuse-whole class is not available to this refusal at
     all** and arms 1 and 3 are unreachable for it. The `ordering` arm splits on
     the stored operand, which is **absent**, so the arm is **4b** — the
     **terminal clearing** arm.

     Under the **paired** conjunct the refusal reaches no arm at all:
     `(Some(1), 11) ≠ (Some(1), 12)`, so it is the declared **log-only no-op**,
     record and fence untouched, and `S1'` goes on to resolve on its own report.
     **Note which component rejects it, because that is the necessity argument
     for the second component** (v28 flag 1, recorded as a gap and closed here):
     the pair fails on the **second component alone** — `Some(1) == Some(1)` on
     `stage_id`, `11 ≠ 12` on `observed_registration_seq` — so
     `observed_registration_seq` is the sole thing standing between this refusal
     and arm 4b. The first component rejects nothing here, and an in-registration
     mis-bind exercises only the first, which is why one leg suffices.
     Under revision 19's **unpaired** (`stage_id`-only) conjunct it **binds** —
     `1 == 1` — and **arm 4b** fires on a **dead registration's** verdict: the
     live record `S1'` is **cleared and fsynced**, the whole-node fence is
     **dropped**, and the initiating `CLUSTER REPLICATE` client is answered a
     **terminal** error naming the lost registration — **registration 11, because
     arm 4b's reply names `refused_payload.observed_registration_seq`** (the
     operand is bound at the arm, V28-M5), which is not `S1'`'s registration.
     Registration 12 is alive and its stage was
     adjudicated by nobody. 4b is terminal by declaration, so no retry repairs it.
     **A second, independent argument that the conjunct is necessary, and it is
     the reply operand** (V28-M5): the effect-keyed record-clearing clause answers
     the same client with the same token naming `record.staged_registration_seq`,
     and the design asserts the two replies are byte-for-byte identical. They are
     identical **only because** the record-binding conjunct forces
     `refused_payload.observed_registration_seq == record.staged_registration_seq`
     on every path where 4b fires. Drop the second component and the two
     mechanisms answer the same client two different registrations — the
     commutation the design relies on is a *consequence* of the conjunct, not an
     independent fact, and this trace is where that dependency is visible.

     **The harm, stated as the arm produces it and no further.** `S1'`'s own
     report is **held** across the refusal and is proposed the instant `n`'s
     post-rejoin `Boot` report applies — the level-triggered reconcile arm's
     re-report against the absent cell, which the `run_identity` lifecycle bullet
     declares admitted. It is still admissible when it is proposed: `n ∈ nodes`,
     `observed_registration_seq = 12` is current, and the ordering conjunct
     passes now that `run_identity` is written — so it **admits**,
     stamping `admitted_stage = Some(1)` into replicated state for a stage whose
     record this node has destroyed, whose fence it has dropped and whose client
     it has already answered an error. The two planes then disagree about one
     stage: applied state carries it as adjudicated-admitted while the node has
     reverted to its **pre-stage** plane and is serving on it — the split V15-M1
     closed and revision 19 re-opened by promoting a node-local counter to sole
     discriminator, and on the migration path the shape in which writes acked by
     a node the cluster's own applied state has moved are lost.
     **What does *not* happen, said explicitly so the claim stays checkable**:
     the destructive adoption does **not** run, because its selector binds to the
     record's own report and the record is gone. The harm is the split verdict
     and the wrong terminal answer to the client, not a wipe — revision 26's
     trace claimed the wipe, and the arm table does not produce it.

     **A second candidate, walked and deliberately not used** (V27-M1 floated the
     *in-registration* mis-bind: a refusal whose class **is** refuse-whole
     binding to a colliding `stage_id`, arm 1 firing, and a plane split while the
     source still believes it is authoritative). It does not verify **as an
     argument for the second component**, and the arm table is again the reason:
     a refuse-whole class is reachable only when `membership`, `ordering` **and**
     `fence` all pass, and `fence` passing means the payload's
     `observed_registration_seq` equals the node's current `registration_seq`. A
     live record's `staged_registration_seq` equals it too — §0's effect-keyed
     clearing clause kills any record for which it does not — so the **pair
     matches** and the paired form excludes nothing the unpaired form admitted.
     That harm is real, and it is the **first** component's to prevent, exactly
     as the precedence rule below already says: "in-registration by `stage_id`,
     cross-registration by `staged_registration_seq`". Recorded rather than
     included, so the next round can see it was walked.

     **The mandated wipe sequences do
     *not* produce this shape, and the trace may not be re-based on them again**:
     neither destroys `<data-dir>/frogdb_node_identity`, so a node that stages,
     is wiped by either mandated sequence and stages again mints strictly above
     every id it has ever spent (gate 1's "what neither sequence touches"
     finding, and §0's counter bullet — the collision is unproducible there).
     The pairing closes it with the
     ...
     state). **How that operand separates the two records is discharged over the
     two wipe paths separately, because they discharge differently** (V24-M2 /
     V25-M2; this concluding step is the site revision 25's sweep of the
     unqualified "strictly greater" missed — V26-M2): on **(α)**, the
     multi-member out-of-band sequence, re-creation of the deleted `NodeInfo`
     cell **is** the mint site of `registration_seq` and the survivors'
     generation is untouched, so the post-rejoin record's
     `staged_registration_seq` is strictly greater than every pre-wipe report's
     `observed_registration_seq` **within that surviving generation**, and the
     pair cannot match; on **(β)**, the single-member in-band sequence, strict
     growth is **false** — the post-wipe mint is `registration_seq = 1`, equal
     to the node's first registration — and what discharges the leg instead is
     that the same sequence discards the log every undelivered pre-wipe report
     lived in, leaving no counterparty to compare against. Both legs are stated
     in full at §0's leg-(iv) split ((α)/(β) under the collision-freedom
     paragraph), and the canonical scoped statement of strict growth — within a
     generation — is the `registration_seq` writer-join row; this paragraph
     cites them rather than restating the universal it may not assert.
     ...
     *different* record could be harmed, and the **paired** record-binding
     ...
     excluded only the first, which is why a node that **lost its stage counter,
     exited the untrusted state through `FORGET` + re-`MEET`, and staged again**
     could be harmed by its own dead stage's refusal — V28-m2: the "wiped and
     re-staged" vocabulary this sentence carried names a producer V24-M3
     established does not exist, since neither mandated wipe restarts the
     counter).
     ...
     4. **`ordering` class — split into two sub-cases** (V19-M1; revision 18's
        ...
        - **(4b) refused against an *absent* stored value** — reachable only for
          ...
          registration this node no longer holds. **The reply's token and its
          operand, bound at the arm** (V28-M5 — revisions 19 through 27 said
          "naming the lost registration" and never said *which* registration, so
          the negative control asserted an observable no rule determined): the
          token is `-ERR CLUSTER REPLICATE staged flip lost its registration
          (<seq>)`, as §3's reply-token totality rule requires of every terminal
          arm, and `<seq>` is **`refused_payload.observed_registration_seq`** —
          the registration the *refusal* was minted in, which is the registration
          whose verdict this arm is delivering. It is byte-for-byte the clearing
          clause's reply for the reason stated there: the clause names
          `record.staged_registration_seq`, and the record-binding conjunct makes
          the two equal on every path where this arm can fire. Then the same two
...
> and that reader **must pair it with `nodes[n].registration_seq`** per the
> paragraph above (**V28-M7** — revisions 18 through 27 left this normative
> sentence naming `run_identity`, the partner V18-C1 *removed* ten lines above
> it and which the fence-freshness row for this very fence declares "no longer
> an operand". A reading discipline is the sentence a future reader greps before
> adding a second reader of `parent_seq`, so it prescribed the defect: a
> node-local-origin partner, which the fence-freshness discipline states outright
> is not a remedy).
...
| **all six classes** | V20-C1's change to the **record-binding conjunct**, which is a conjunct of every arm: it is **narrowed** from `stage_id` equality to the pair with `observed_registration_seq` | **Yes, and more of them are now true.** The conjunct's job is attribution, and every arm's rationale presumes "this refusal belongs to this record"; the unpaired form silently admitted refusals from a *dead* registration bearing a re-used id, so a **clearing** arm could reach a terminal disposition on someone else's verdict. Narrowing removes those, and removes nothing else — see the totality argument at the new fence-freshness row. **Which arms, narrowed in revision 27** (V27-M1 — revisions 20-26 wrote "arms 1/3/4b/5", and arms 1 and 3 are **class-gated to refuse-whole**, a class reachable only when `membership`, `ordering` *and* `fence` all pass, which is to say only **within** a registration; a cross-registration refusal's class is `ordering` or `fence` by the fence-freshness row's two legs, so arms 1 and 3 are unreachable for the producer this conjunct was added for): the arms a cross-registration refusal can actually reach and dispose terminally are **4b** (leg (a), absent stored `run_identity`) and **5** (`membership`, the node removed at the refusal's apply). Arm 2 (`fence`, leg (b)) persists the record and re-proposes, and arm 4a is a no-op, so neither is a mis-binding *harm* — which is precisely why gate 3 (d)'s necessity trace and this conjunct's forcing test are both pinned to the leg-(a) / arm-4b path | **Neither — the narrowing produces no new refusal.** It changes *which record a refusal may be disposed against*, not whether a report is refused. Refusals that now fail the conjunct were previously **misrouted**; they become the already-declared **log-only no-op** | **No arm changes shape**; the conjunct guarding all six is restated in its paired form once, at gate 3 (d). Recorded here because the join's own trip-wire fired in reverse this round: the conjunct *was* a change to a gate over an admission-like decision, and revision 19 recorded it in neither this table nor the fence-freshness one, which is exactly how a CRITICAL survived eleven rounds |
...
| **Unmarked-populated-directory boot refusal** (LOCKED FM-PERSISTENCE-051's `holds files but no FrogDB marker` bail, `recovery/src/data_dir.rs:110-121` — **new row in revision 24**, V24-M1; the rule is the shipped code's, and V23-M3's move of `verify`/`stamp` out of the `rocks_backed` gate is what makes it reachable on configurations that never ran it before) | **down**; recovery refuses at phase 0, before the install and before any store is opened; **everything in the directory is intact and untouched** — that is the refusal's purpose; serves nothing | **none on this node**, and no other member's surface helps: the fault is what is *in this node's directory*, which no cluster-side command inspects or repairs | Inspect `<data-dir>`, **remove or relocate the entry that is not FrogDB's**, restart. After the FM-PERSISTENCE-048 amendment declared at §0, `raft/` and `replication_state.json` no longer trigger it, so what remains is genuinely foreign — a stray dump, a hand-copied file, another product's directory. **Or `--force-fresh-data-dir`**, at exactly the two costs the `FutureLayout` row prices (fresh `database_id`; a `layout_version: 1` re-mint that re-reads as never-created), and here with a third the operator must weigh: the flag does **not** delete the foreign entry, it mints a marker beside it and proceeds, which is the overwrite the bail exists to prevent. **Single-member**: identical — the node is down and the exit is a filesystem action either way. Forcing test: the retained-property control `an_unmarked_directory_with_a_stray_file_still_refuses` |
| **Discard-mark write refusal** (gate 1's `CLUSTER RESET HARD` precondition — **new row in revision 24**, V24-M4) | **up and serving**; nothing proposed, nothing committed, no topology erased anywhere; the node's Raft state, dataset and membership are exactly as before the command | all of it; the reply token is `-ERR CLUSTER RESET HARD refused: the Raft store could not be marked for discard` | repair the node-local durable-state fault the error names, re-issue `CLUSTER RESET HARD`. **Single-member**: identical, and the surface is this node's own, because the node is up — which is the entire reason the mark is written **before** the propose rather than after the apply |
| **Stale armed discard mark — node up** (gate 1's residue, **Row A**: the mark is durable, the `ResetCluster` never commits — rejected, timed out, leader lost — and the node **stays up** — **new row in revision 24**, V24-M4; split from its crashed-node twin and given its missing precondition in revision 25, V25-M1) | **up and serving**, and *nothing is wrong yet*: an armed mark has no effect while the node runs. The hazard is **the next boot**, which wipes `<data-dir>/raft` before opening it — intended on the single-member exit, destructive on a node that is still a live member | all of it; `CLUSTER INFO` reports `raft_discard_pending` as `1` or `0` from the file's presence, which is what makes this residue visible before the restart that acts on it | **Complete the reset**: re-issue `CLUSTER RESET HARD` (idempotent — the mark is re-written and the wipe is what was wanted), then restart. **Or abandon it**: remove `<data-dir>/frogdb_raft_discard` at the **running** node, before any restart — **and revision 24 promised that exit unconditionally, which it may not** (V25-M1's K1 half). Abandoning is admissible **only against evidence that the reset did not commit**: `CLUSTER NODES` at a **surviving member** still listing this node is that evidence, and a single-member cluster has no such member, so there the abandon exit is *not available* and "complete the reset" is the only exit. **That evidence can be stale in one direction, and the freshness rule is stated at the mechanism** (V26-m1, and it applies identically to Row B): the read is of *one* member's applied state and commit precedes apply, so a lagging or partitioned survivor still lists this node after the entry committed — prefer a member confirmable as in the current leader's majority, and treat disagreement between two survivors as "committed", leaving the mark armed. The residue of a wrong read is the committed-reset residue this cell already prices below (gate 1 refuses the rejoin; re-issue `CLUSTER RESET HARD`). On a reset that **did** commit, an abandoned mark leaves the node holding a Raft store the committed reset required it to discard; gate 1 then refuses its rejoin, and the escape from *that* state is to re-issue `CLUSTER RESET HARD` and let the mark be re-written. The design deliberately does **not** un-mark on a failed propose — an indeterminate propose may still commit under a later leader, and auto-un-marking would disarm a wipe a committed reset requires — so the choice is the operator's and this row is where it is stated. On the **out-of-band `rm`**: removing a file on a node the operator is not restarting is the same admissibility the gate-1 join-empty row's multi-member sequence already relies on — a filesystem action is "the honest form of 'no in-band surface'" only when the *state it edits* is not being concurrently written, which holds here because the mark is written once, ahead of the propose, and read only at boot |
| **Stale armed discard mark — node down** (gate 1's residue, **Row B**: the node **crashed between publishing the mark and the commit** — **new row in revision 25**, V25-M1; revision 24 routed this trigger to Row A and said it "carries the same recovery row", which it cannot: **both** of Row A's exits presuppose a node that is up and a boot that has not happened, and here the node is down and *the boot is the wipe*) | **down**, with an armed mark and an intact Raft store. The next boot is the destructive act: it wipes `<data-dir>/raft` **before** phase 6 opens the store, so starting the node is not a diagnosis step — it is the loss. On a node that is still a member of a live multi-member cluster this destroys the live member's replicated state, and with issue 25's solo-bootstrap behaviour unfixed the wiped node then bootstraps itself into a second Raft group while the survivors still list it in theirs | **none on this node** — the node is down, and `raft_discard_pending` is a `CLUSTER INFO` field of a process that is not running. The surface that answers "did the reset commit" is **another member's** `CLUSTER NODES`; in a **single-member** cluster there is no such surface at all, and the disposition is the one the mark was written for: the wipe is what was wanted, so start the node | **Remove `<data-dir>/frogdb_raft_discard` on the STOPPED node, before starting it** — this is the only exit, and it is a filesystem action by necessity rather than by preference. Precondition, as in Row A: establish at a surviving member that the reset did **not** commit. **And the check is evidence, not proof — it can be wrong in one direction** (V26-m1, the same caveat Row A carries and the mechanism states in full): `CLUSTER NODES` reads that member's **applied** state while commit precedes apply, so a survivor that is lagging or partitioned from the committing leader **still lists this node** after the entry committed, and the operator reads a false "did not commit"; the same window is the TOCTOU of an indeterminate propose that commits *after* the check. Freshness prescription: prefer a member the operator can confirm is in the **current leader's majority**, and if two survivors disagree, take the answer as **"committed"** and leave the mark armed — fail-closed, settled ruling 5. Reading it wrong costs exactly the already-priced residue: the node keeps a store the committed reset required it to discard, gate 1 refuses its rejoin, and the escape is to re-issue `CLUSTER RESET HARD` and let the mark be re-written. If it **did** commit, leave the mark armed and start the node — the wipe is the committed reset's own requirement. **Single-member**: start the node; there is nothing to establish and nothing to preserve |
...
rather than folded into an earlier one (V23-M4, V24-M1, V25-M1; and the count
itself has now been wrong four times, so it is stated with its members rather
than as a number: revision 21 said "two" while counting a rule it had already
converted into a staging refusal, V22-m2 corrected it to "one", revision 22
shipped a design whose stamp raise makes a genuine second one reachable,
revision 23 shipped one whose marker-phase move makes a third one reachable
while calling that a benefit, and revision 24 shipped a **fourth** — its own —
without noticing it was one, which is V25-M1).
...
*reachable* by raising the stamp and therefore owes a row.
**(3) The unmarked-populated-directory refusal** — also reachable in every
configuration, also answered by no in-band surface anywhere, and also not this
design's rule (LOCKED FM-PERSISTENCE-051's), made reachable by V23-M3's move of
`verify` out of the `rocks_backed` gate. It differs from (2) in **who can be
caught**: (2) catches only an operator who downgrades a binary, while (3) would
have caught **every persistence-disabled and `fake`-WAL node in existence** on
its first upgrade boot, which is why the FM-PERSISTENCE-048 amendment narrows it
back to genuinely foreign content before this row is acceptable at all. A
down-state rule that fires on the ordinary upgrade path is not a fail-closed
rule; it is an outage, and no recovery row makes it one.
**(4) The crashed-node stale discard mark** (Row B) — **this design's own**, and
the only one of the four whose distinguishing property is that **the boot
itself is the destructive act**. (1), (2) and (3) all refuse *before* touching
anything, so for them a restart is a free diagnosis step and the node's state
survives every failed attempt; here the node is down holding an intact Raft
store that the *next successful boot deletes*, so the operator gets exactly one
decision and the wrong one is unrecoverable. Its (2) is answered by **another
member's** surface, as (1)'s is — but not for (1)'s reason: (1) needs another
member because only another member can *issue the fix*, while (4) needs one
only to *establish the precondition* (did the reset commit), and its fix is a
filesystem action on the stopped node. In a **single-member** cluster (4) is
not vacuous the way (1) is; it is simply already in its intended terminal
state, and the exit is to start the node. This is also why (4) is the row that
makes issue 25 a **blocking prerequisite** rather than a dependency: a
mis-taken decision here does not merely lose one node's state, it produces a
second Raft group.
Every other fail-closed rule in this design leaves the node **up and
...
or is the exit a restart with different bits?", plus, after V25-M1, "does the
boot that reaches the surface *itself* destroy the state the operator is
deciding about?"
...
This design reads **three** node-local durable artifacts, and they get **three
...
**The third artifact is revision 24's** (V24-m1 — revision 23 introduced
`<data-dir>/frogdb_raft_discard` at gate 1 and left it outside this discipline
entirely, which is precisely the omission the discipline exists to catch: an
artifact with a writer, an fsync requirement and a boot-order position, and no
row). It gets **Table C** below, and the same treatment as the other two.
...
| **Untrusted clear** (an exit fires: fresh `registration_seq`, or `self ∉ nodes`) | the whole store: `stage_counter_state = Trusted { value: 0 }` **and `stage_counter = 0`** (**V28-M3** — revisions 22-27 wrote `Trusted{stage_counter}` in this cell, which *carries* the surviving counter while §0's rule *zeroes* it; the two are different writes, and this table is the artefact whose whole purpose is to be the single enumerated statement of what each writer writes. §0's form is the one with a soundness argument attached — argued twice, once per wipe path, at §0's *clearing the untrusted state* bullet — and it is independently the only defensible one, since the state is entered precisely because `stage_counter` is **not** trusted, so adopting that value as `Trusted` would launder the untrusted value through the exit. The distinction is deliberate elsewhere in this same table — the boot-initialization row writes `Trusted{0}` and the incarnation-re-mint row carries `stage_counter` verbatim — so the old cell could not be read as a loose spelling) | **writes** it — the field's transition out | published before the node admits the first staging of the new registration | crash before publish → the state still stands on the next boot, and the exit's own predicate (a `registration_seq` that differs from `untrusted_since_registration`) is **level-triggered against applied state**, so it re-fires | step **(iii)**, and level-triggered while running |
...
|---|---|---|---|---|
| **First mint** on a fresh directory (`DataDirMarker::mint()`, reached only when `verify` found no marker **and did not bail** — `recovery/src/data_dir.rs:166`, guarded by the populated-directory refusal at `:110-121`, whose excused set FM-PERSISTENCE-048's amendment widens) | the whole marker with `layout_version = DATA_DIR_LAYOUT_VERSION` | `stamp_with`'s publish sequence (`persistence/src/data_dir.rs:299-304`), before the install and before any store is opened (`recovery/src/lib.rs:203-214`) | crash before publish → **marker absent**, and the directory is genuinely fresh; the next boot mints again | recovery **phase 0**, before cluster boot's step (0) latches it |
...
| **The raise** (**new writer, this design** — V23-M1) | the latched marker with `layout_version` set to `N`, and **no other field touched**: `database_id` and `created_at_unix_ms` are **carried forward verbatim** from the marker `verify` returned (V24-m2 — stated rather than implied, because "the directory's creation time" silently becoming "the time the upgrade ran" is the kind of value corruption a whole-struct publish makes easy and nobody notices; LOCKED FM-PERSISTENCE-049's stable-identity guarantee is about `database_id`, and this design volunteers the same treatment for the timestamp beside it). Forcing test: `the_layout_raise_preserves_database_id_and_created_at` | the same `write` → `sync_file` → `rename` → `sync_dir` publish, **after** the identity store's step-(iv) write is fsynced (store-then-stamp) | crash before publish → **stamp below `N` ∧ store present** (cell S-d/S-e ∧ below-`N`), which the partition declares benign: the master rule lets the store's `stage_counter_state` decide, and the next boot re-attempts the raise | cluster boot, step **(iv-b)** — the one ordering constraint revision 23 adds |
...
**Table C — the Raft discard mark** (`<data-dir>/frogdb_raft_discard`; **new in
revision 23, tabled in revision 24** — V24-m1). The artifact carries **no
fields**: its whole content is its existence, which is why it is a separate
table rather than a column of Table B. It is published with the same `write` →
`sync_file` → `rename` → `sync_dir` shape, and like both other artifacts its
path comes from `DataDirLayout` (the FM-PERSISTENCE-057 amendment declared in
the blast radius below).

**What "no fields" buys, stated rather than assumed** (V25-M2's Table-C half —
Tables A and B each own a torn-write argument and revision 24 left this one to
inference): **a fieldless artifact cannot tear.** Every torn-write hazard the
other two tables argue about is a hazard of *partial content*, and here there
is no content to be partial in — the trigger is the file's **existence**, and
`rename` is atomic, so the only two observable states are "mark present" and
"mark absent". A crash mid-publish yields one of those two and never a third.
**Off unix** the caveat is the same one V23-m2 states for the store, applied to
a different loss: the degraded `sync_dir`
(`frogdb-server/crates/persistence/src/fs_seam.rs:89-92`) can lose the
**rename**, and because the rename is what *creates* the mark, the surviving
state off unix is **"no mark"**. That is the **safe direction**: the boot that
follows does not wipe, the node keeps its Raft store, and the reset — whose
`+OK` was never sent, because the mark is published *before* the propose — is
simply not taken. The unsafe direction (a mark that survives a reset that never
committed) is the one the rename cannot produce by being lost, only by
succeeding, which is Row A/Row B's residue and has its recovery rows.

| Writer | Writes | Fsync point | Crash window → outcome | Boot-ordering position |
|---|---|---|---|---|
| **`CLUSTER RESET HARD` arm-ahead** (**this design's writer** — gate 1, ordering fixed in revision 24 by V24-M4) | creates the mark | full publish sequence, **before `propose_reset` is called** (inside `handle_reset_command`, `connection/cluster.rs:161-186`, at the seam `:172-174` ahead of `:175` → `writer.rs:219-256`) — the placement is the point: a failure here can still refuse the command, and the refusal token is at gate 1 | crash before publish → nothing was proposed and nothing was marked; the cluster is untouched and the operator re-issues. Crash after publish, before commit → the node is **down** with an armed mark and the **next boot is the wipe**; on the single-member exit that is the intended terminal state, and on a live member it is the **crashed-node** recovery row (**Row B**, V25-M1) — *not* the running-node residue row, whose two exits both presuppose a node that is up | **not at boot** — a serving-path writer, like Table A's mint, and for the same structural reason: it is a durable step taken before a reply |
| **Boot completion** (recovery removes the mark after wiping `<data-dir>/raft`) | deletes the mark | the removal is durable before phase 6 opens the store (`recovery/src/cluster.rs:23-25`); the wipe precedes the removal, never the reverse | crash between the wipe and the removal → the mark survives, the next boot repeats a `remove_dir_all` of an absent path, which is a no-op: **idempotent by re-derivation** | recovery, **before phase 6** and after phase 0 (it needs the layout, so it cannot precede the marker phase) |
| **Operator removal** (the abandon-the-reset exit at **both** recovery rows — Row A on a running node, Row B on a **stopped** one, V25-M1) | deletes the mark | none required — the only reader is the next boot, and on Row B's path the removal happens while nothing is running at all | none: a crash mid-removal leaves the mark either present or absent, and both are states the next boot handles | n/a |

**Readers of the mark**: recovery's boot completion above, and `CLUSTER INFO`'s
`raft_discard_pending` field, which exists so the residue is visible **before**
the restart that acts on it. Note that this is the one artifact of the three
whose *presence* is the operand — Table A and Table B both answer questions
about content — so "absent" is a positive fact here in the same way §0 argues it
is for the marker, and for a simpler reason: nothing but the two deleters above
ever removes it.
...
- **`ReportRunIdentity{node, run_id, kind: Boot | Demotion,
  ...
  **Re-derived in revision 20, because the premise it rested on was
  ...
  monotone and never reused" is true only **within a registration** — so the
  deleted write cannot be defended by counter
  uniqueness alone, and the question is whether a leftover
  `admitted_stage = Some(k)` can satisfy a **re-used** `k` and let gate 3 (c)
  adopt a flip that was never admitted.
  **The producer of a re-used `k` is not the one revision 20 named**
  (V24-M3): revision 20 wrote that "the `CLUSTER RESET HARD` + `FLUSHALL` wipe
  LOCKED TR-CLUSTER-005 mandates restarts the counter", and it does not — that
  sequence touches neither the identity store nor its counter (V28-m12 —
  narrowed from "touches no filesystem", which V23-M2's own discard-mark
  amendment falsifies), and the multi-member sequence removes
  `<data-dir>/raft` alone, so the identity store and its counter survive both
  (gate 1). The re-derivation is kept in full anyway, with the producer
  corrected to a node-local **durability loss** (V28-m1: revision 24 wrote
  "durability loss **or out-of-band restore**" here without the out-of-scope
  label every other restatement now carries — the restore rewinds the same value
  but is excluded **by the act**, per §0's scope declaration, so the load-bearing
  producer is the durability loss alone), because
  the question it answers is real and the answer is stronger than the premise
  it was written for. **A re-used `k` cannot fire this selector, and the reason
  is the cell, not the counter**: `admitted_stage` lives inside the `NodeInfo`
  cell keyed by `registration_seq`, and the wipe path **deletes that cell**
  (`commands.rs:837`, or `:833`
  ...
  **Damping rule, stated**: a node keeps **at most one** `ReportRunIdentity`
  ...
  (the new mint orders strictly above, the stale report refuses).
  **The rule re-arms on non-observation, stated because the rule as written is
  edge-shaped over an outcome a dropped proposal never delivers** (V28-M4's
  neighbour, and review v28's recorded concern against settled ruling 3,
  *drops-stop-signaling*): a proposal that is observed **neither applied nor
  refused** does not mute the node. The damping is a *hold on re-proposing*, not
  a *latch*, and it is released by the same **level-triggered re-evaluation**
  this design states as its general shape — the reconcile re-reads applied
  state, finds its report still not there, and re-proposes. Two consequences are
  stated so no later rule can read the hold as a latch. **(1)** The hold is
  released by the boot report applying, which is the composition §0's boot
  ordering rule needs: a staged record's `Demotion` report held behind the boot
  report is proposed when the boot report applies, and if the boot report is
  itself dropped, the boot report re-proposes and the held one follows. **(2)**
  The re-arm carries **no wall clock and no deadline** — it is not "re-propose
  after *t*", it is "re-propose whenever the level rule is re-evaluated and the
  applied state still lacks the report", which is a **liveness** rule with no
  correctness decision resting on it. A drop therefore costs a re-evaluation,
  never a permanently muted node. **A target's identity change does not cancel**
...
- **`AttestReplicaSynced{node, proposer, observed_primary_id: NodeId,
  ...
  **removed**, and `run_identity` is not read by this transition at all). **The trigger, stated once and covering both resync shapes**
  ...
  second mechanism. **The trigger is a level, not an edge, and it re-arms on
  non-observation** (V28-M4's companion, and review v28's second recorded
  concern against settled ruling 3, *drops-stop-signaling*): the sentence above
  reads as an event ("the link is established and the offset reaches the sync
  point"), and an event fires once, so a proposal **dropped in flight** — a
  follower proposal is `Proposed::Forwarded` (TR-CLUSTER-041), which the
  forwarding node may lose without either outcome ever being observed — would
  leave a replica that is *in fact* synced with no route back to `synced ==
  true`, the exact liveness hole V15-M4 closed for the partial-resync shape and
  which a dropped proposal would re-open for both shapes. Read the trigger
  instead as a **standing predicate over the replica's own local state**: while
  the link with the current upstream is established and the offset is at or past
  that link's sync point **and** the replica has observed **neither an apply nor
  a refusal** of a proposal minted against its **current** operand triple
  (`primary_id`, `parent_seq`, `registration_seq`), it re-proposes on the same
  **level-triggered re-evaluation** this design states as its general shape —
  the periodic local re-check that also drives the boot reconcile. Three
  consequences, each pinned so nothing here weakens a settled ruling. **(1)** An
  *observed* refusal does not re-arm this rule; it is disposed by the refusal
  class's own disposition (§gate 3), and a re-proposal after a refusal is minted
  fresh against the operands the refusal reports, never retried as-is — the
  fence's freshness discipline, leg (b). **(2)** The re-arm carries **no wall
  clock and no deadline**; the re-evaluation is level-triggered on local state,
  and no correctness decision anywhere in this design rests on the *rate* at
  which it fires. It is a **liveness** rule only: it cannot make an
  un-synced replica attest, because the predicate's first two conjuncts are
  exactly the sync condition and they are re-read at each evaluation. **(3)**
  Because the operand triple is re-read at mint time and the payload is fenced
  on it (below), a re-proposal raised after the operands moved — a demotion, a
  re-parenting, a `CLUSTER FORGET` and rejoin — is refused by the admission
  fence rather than admitted stale; the re-arm therefore adds no new admission
  path, only new *attempts* down the existing one. Admission:
  `payload.proposer == payload.node` (V6-C1's
  ...
  **Why the token alone is not enough** (V17-C1 — the revision-16 form's own
  ...
  the `ReportRunIdentity` row is scoped to that transition, and the trigger's
  **re-arm on non-observation** stated above is not a damping rule at all: it
  adds re-proposals, so it can only make this trace *more* available, never
  less, which is why the closure below is an **admission-side** one and not a
  proposal-side one); `CLUSTER FORGET R`
  ...
  **What closes it** (V18-C1 — **one leg, not two**, and that is the point):
  ...
  `registration_seq` than the value its stale attestation observed — the (α)
  shape, where the survivors hold the generation, which is the scope V25-M2's
  writer-join row states — the equality
...
- **`CLUSTER INFO`, the node-local fail-closed states** (V21-M1 — the
  ...
  corrected single-member exit clears through the second one); and new in
  revision 24 **`raft_discard_pending:0|1`** (V24-M4), read from the presence of
  `<data-dir>/frogdb_raft_discard`, which exists so that an armed-but-uncommitted
  discard mark is visible **before** the restart that acts on it — the residue
  the mark-before-propose ordering creates, and its recovery row's clause (2).
  All three are node-local reads
  answered by the node they describe, and all three are in the staged-flip fence's
...
- **Rewritten**: TR-CLUSTER-010..013 (phase machine above); TR-CLUSTER-014
  ...
  **amendment declared below**; *live multi-member, out-of-band* —
  ...
  the Raft-state half. **A named, undischarged dependency rides this row — and
  it is BLOCKING for the discard-mark mechanism** (V25-M1): an
  ...
  this row's precondition is `Source: "Not yet in code"`. For the operator-issued
  wipe that is an availability nuisance; for the **crash between the discard mark
  and the commit** it is a split-brain source, which is why the TR-CLUSTER-035
  amendment below may not ship ahead of it. Forcing tests:
  ...
  **fresh** arm of `AddNode` (`node.id ∉ nodes` pre-apply, `commands.rs:132`).
  ...
  through a fresh registration, which mints a strictly greater value **within
  the generation** (V25-M2's scoping — the in-band single-member wipe discards
  the log the generation lives in and restarts it; see the `registration_seq`
  writer-join row). `AddNode`'s
...
- **Added in revision 20** (V20-C1/M1/m1/m2): **again no LOCKED row changes,
  ...
  *node-local, proposer-side* disposition check; both of its new operands
  ...
  TR-CLUSTER-005 (the mandated wipe, whose counter-restarting effect revision
  20 took for the CRITICAL's premise — **corrected in revision 26 (premise
  withdrawn by V24-M3)**, and this marking added in revision 27, V27-m3, because
  the correction edited historical text without saying which revision made it:
  neither mandated sequence restarts the counter, and the trace's real producer
  ...
  **both** families whose patterns it hits — **A** and **D** — because it states
  ...
  TR-CLUSTER-041
  (forwarded proposals, the delay the
  ...
  `refusal_of_a_dead_stage_cannot_bind_a_live_record_after_a_forget_and_rejoin`
  (**renamed and re-based in revision 27**, V27-M3 — through revision 25 it was
  `..._after_the_mandated_wipe`, a fixture the withdrawn premise made
  unconstructable; revision 26 renamed it `..._after_a_rewound_stage_counter` and
  re-based it on an out-of-band restore, a producer §0 puts outside the failure
  model, on a fixture whose refusal class was `fence` and whose control therefore
  had no teeth — V27-M2)
...
- **Added in revision 21** (V21-M1/M2/m1/m2/m3): **no LOCKED row changes, and
  ...
  **(1) The stage-counter boot rule becomes a staging refusal** (V21-M1): a
  ...
  moved**: it remains the mandated wipe V20-C1's argument is written around —
  though **not** a counter-restarting one, which V24-M3 corrects at the
  record-binding conjunct's row and everywhere else the premise appeared — and
  it is **no longer the only recovery** from a counter loss — that
...
- **Added in revision 23** (V23-M1/M2/M3/M4/m1/m2/m3/m4): **this is the first
  ...
  **(1) FM-PERSISTENCE-049 — Amended, first amendment: `verify`'s outcome set**
  ...
  today. **`verify` has a fourth exit that is not an outcome and must be named
  wherever its outcome set is** (V24-M1, and revision 23's omission of it is
  what let the marker-phase move look free): the populated-directory bail at
  `recovery/src/data_dir.rs:110-121`, which returns no marker at all and refuses
  the boot. It is amendment (4)'s subject. The amendment names the new writer, its position (cluster boot step
  ...
  **(3) TR-CLUSTER-035 — Amended: the HARD path gains a Raft-store discard
  ...
  (`frogdb-server/crates/cluster/src/commands.rs:816-862`) touches no filesystem, so the wipe every
  ...
  amendment is **mark-then-wipe-on-restart**, and the next
  boot removes `<data-dir>/raft` and then the mark, before
  `recovery/src/cluster.rs:23-25` opens the store.
  **Re-shaped in revision 24, and it stays one amendment rather than becoming
  two** (V24-M4): revision 23 put the mark "before the `+OK`", which is *after*
  the apply, where LOCKED FM-CLUSTER-006's infallible arm leaves an IO failure
  nowhere to go. The obligation therefore attaches to the row's **Precondition**
  ("Node-issued `ResetCluster{node_id, new_node_id}`; `new_node_id: Some`
  selects HARD") rather than its Postcondition: on the HARD path the op is
  proposed **only after** the issuing node has durably written
  `<data-dir>/frogdb_raft_discard`, and the Postcondition gains the next-boot
  completion. FM-CLUSTER-006 is confronted at gate 1 and comes through
  **unbreached**, which is why no amendment to it appears in this list.
  Crash windows, the residue's recovery row and the declared
  intent-ahead-of-effect exception are at gate 1. Forcing tests:
  `reset_hard_marks_the_raft_store_for_discard_before_it_proposes`,
  `a_reset_hard_whose_mark_cannot_be_written_refuses_without_proposing`,
  `the_next_boot_wipes_a_marked_raft_store_before_opening_it`,
  `a_crash_before_the_discard_mark_leaves_the_cluster_untouched`,
  `a_mark_left_by_a_failed_propose_is_surfaced_in_cluster_info`,
  `a_single_member_reset_hard_then_restart_comes_up_with_empty_raft_state`,
  `a_crash_between_the_mark_and_the_commit_leaves_a_recoverable_node` (**new in
  revision 25**, V25-M1, with the negative control
  `a_crash_between_the_mark_and_the_commit_destroys_the_store_if_the_mark_is_left`).
  ...
  **TR-PERSISTENCE-051** (stated parametrically in
  ...
  **corrected** this round rather than moved, and it acquires a named,
  undischarged dependency on issue 25's solo-bootstrap behaviour — **promoted in
  revision 25 from a dependency to a BLOCKING prerequisite of the discard-mark
  mechanism**, V25-M1: the crash-between-mark-and-commit window makes a live
  member's boot wipe its own Raft store, and with the solo bootstrap in place
  that node then forms a second Raft group while the survivors still list it in
  theirs — a split-brain source, not an availability nuisance. The mark may not
  ship ahead of `a_wiped_node_awaits_meet_instead_of_solo_bootstrapping`);
  ...
  **Blast-radius verdict for the round: two rows Amended (FM-PERSISTENCE-049,
  ...
  dependency).
- **Added in revision 24** (V24-M1/M2/M3/M4/m1/m2/m3): **two further LOCKED-row
  amendments are declared, both consequences of revision 23's own changes rather
  than of new design.** Neither is performed here; each is listed with the row,
  the invariant delta, and the forcing test.
  **(4) FM-PERSISTENCE-048 — Amended: the excused set of
  `contains_foreign_files`** (V24-M1, and it is the amendment **without which
  amendment (2) must not ship**). The row's Invariant reads: "It skips exactly
  the top-level install scratch names the layout owns — `backup`, `staging`, and
  `staging.*` … — and nothing else" (`specs/persistence.md:1163`). Amendment (2)
  puts `verify` in front of every persistence-disabled and `fake`-WAL node's data
  directory, and those directories hold `raft/` and `replication_state.json` with
  no marker, so `verify`'s populated-directory bail
  (`recovery/src/data_dir.rs:110-121`) refuses the boot. The delta is a **closed
  widening of the list by exactly two top-level, FrogDB-owned entries** — `raft`
  and `replication_state.json` — with the "and nothing else" character of the
  list retained. The design's own new artifacts (`frogdb_node_identity`,
  `frogdb_raft_discard`) are deliberately **not** added, for the reason given at
  §0. Forcing test:
  `a_persistence_disabled_node_with_an_existing_raft_store_boots`, with today's
  refusal as the negative control and
  `an_unmarked_directory_with_a_stray_file_still_refuses` as the retained-property
  control. **Sequencing is a hard constraint, not a preference**: a build that
  ships (2) without (4) fails to boot on every existing persistence-disabled
  cluster node.
  **(5) FM-PERSISTENCE-057 — Amended: the layout's path set** (V24-m1;
  the row was **never in** revision 23's "read as binding constraints, not
  moved" list — that list names FM-PERSISTENCE-050, -059, -023,
  TR-PERSISTENCE-051, TR-CLUSTER-005 and FM-CLUSTER-101 — it was recorded in
  revision 23's prose as merely governing the two new siblings, and *that* is
  the treatment it moves out of; **V28-m6**, the "which round said what"
  bookkeeping V27-m3 exists to police, checked this round against the lists
  themselves rather than against memory). The
  row's Invariant states that `DataDirLayout` "is the single owner of the four
  paths; nothing else joins a layout name onto a path"
  (`specs/persistence.md:1016`). This design adds two paths —
  `<data-dir>/frogdb_node_identity` and `<data-dir>/frogdb_raft_discard` — so the
  delta is **four paths become six**, with the single-owner rule unchanged and
  extended over both. Naming them as siblings *under* the rule, as revision 23
  did, is not enough: the rule counts, and a design that adds paths without
  adding them to the owner is exactly the seam the rule exists to close. Forcing
  test, shaped like the seam lint it belongs to:
  `the_identity_store_and_discard_mark_paths_come_from_the_layout` — assert both
  paths are produced by `DataDirLayout` accessors and that no other module joins
  either name onto `<data-dir>`.
  **Read as binding constraints, not moved, this round**: **FM-CLUSTER-006**
  (the reset arm's infallibility — confronted at gate 1 by the mark-ordering
  choice and **unbreached**, because the refusal is at the command layer, which
  the row's own Trigger already contemplates); **FM-PERSISTENCE-051** (the
  populated-directory refusal itself — narrowed in *reachability* by (4), not in
  guarantee, and given the recovery row it never had); **FM-PERSISTENCE-049**
  (mint-once and the raise's field-carry — the raise now states that
  `created_at_unix_ms` rides forward with `database_id`, which is a
  strengthening of this design's own conduct rather than a change to the row);
  **TR-CLUSTER-005** (unchanged; its single-member readmission path is what
  V24-M2's leg (β) had to stop claiming).
  **Not a LOCKED-row change**: Table C (the discard mark's writer join),
  `CLUSTER INFO`'s `raft_discard_pending` field, the three new recovery rows, and
  the counter-survives sweep (V24-M3) — the last of which changes no rule at all,
  only the premises the rules are derived from.
  **Blast-radius verdict for the round: two further rows Amended
  (FM-PERSISTENCE-048 and FM-PERSISTENCE-057), and TR-CLUSTER-035's existing
  amendment re-shaped from its Postcondition to its Precondition; no row New, no
  row Retired.** **Cumulative for the whole cycle: four rows Amended across five
  declared amendments** — FM-PERSISTENCE-048 (once), FM-PERSISTENCE-049
  (twice), FM-PERSISTENCE-057 (once), TR-CLUSTER-035 (once, re-shaped) — which
  corrects revision 23's "two rows Amended" as the standing total.
- **Added in revision 25** (V25-M1/M2/M3/m1..m6): **one further LOCKED-row
  amendment is declared**, and it is again a consequence of a change already
  declared rather than of new design — the 048 widening falsifies prose in two
  *other* rows, which `just lint-spec` cannot see, because it checks
  `FM-`/`TR-`↔test agreement and not whether an Invariant's sentences are still
  true.
  **(6) FM-PERSISTENCE-057 *and* FM-PERSISTENCE-059 — Amended: the excused-set
  sentences each row states in its own words** (V25-M3). Amendment (4) widens
  `contains_foreign_files`'s excused list by `raft` and `replication_state.json`,
  and **two rows other than 048 restate that list**, so landing (4)'s code makes
  both of them false. Neither is optional and neither is 048's row, so each owes
  its own delta.
  **(6a) FM-PERSISTENCE-057**, whose Invariant states, in part (the quoted
  sentence is one of several in the cell — V26 check A1's nit): "The emptiness question
  phase 0 asks ([FM-PERSISTENCE-048]) skips the install scratch names
  (`staging*`, `backup`) and *only* those, so an operator restore staged into a
  fresh data directory is still a first boot while an unmarked directory holding
  a real `db/` still refuses ([FM-PERSISTENCE-051])"
  (`specs/persistence.md:1023`). Delta: the parenthesised list becomes
  (`staging*`, `backup`, `raft`, `replication_state.json`), the *"and only
  those"* **retained** — the closed character of the list is the property, and
  widening it by two named FrogDB-owned entries preserves that property while
  re-stating it falsely does not. The trailing clause is unchanged and stays
  true: `db/` is still scanned in full, so an unmarked directory holding a real
  database still refuses. **This is 057's second delta this cycle** — amendment
  (5) changed its *path count*, this changes its *excused-set sentence*, and
  they are independent edits to the same Invariant.
  **(6b) FM-PERSISTENCE-059**, whose Invariant states, in part (again one
  sentence of a longer cell — V26 check A1's nit): "`contains_foreign_files`
  answers 'did somebody else write here?' and skips `staging*`/`backup`
  ([FM-PERSISTENCE-048]), while `rocks::staged::pending_install` answers 'is
  there an install here to finish?' and looks at exactly those names"
  (`specs/persistence.md:1230`). Delta: the first list becomes
  `staging*`/`backup`/`raft`/`replication_state.json`. **The second half must
  not be widened with it**, and that is the whole reason this sub-delta is
  stated rather than waved at: `pending_install` looks at *exactly* the install
  scratch names, and `raft`/`replication_state.json` are **not** install
  scratch — so after the widening the sentence's "exactly those names" no longer
  names the same set as the first clause, and the row must say so explicitly
  (`staging*`/`backup` for `pending_install`; those plus the two FrogDB-owned
  entries for `contains_foreign_files`). **Revision 23** — not revision 24 —
  listed 059 under "read as binding constraints, not moved"; it moves. (V28-m6:
  revisions 25-27 attributed the listing to revision 24, whose list names
  FM-CLUSTER-006, FM-PERSISTENCE-051, FM-PERSISTENCE-049 and TR-CLUSTER-005 and
  does not mention 059.)
  Forcing tests: the same two that force (4) —
  `a_persistence_disabled_node_with_an_existing_raft_store_boots` and the
  retained-property control `an_unmarked_directory_with_a_stray_file_still_refuses`
  — plus the pre-existing `foreign_files_ignores_the_install_scratch_but_not_the_database`,
  which is named in **both** 048's and 057's `Forced by` cells and **must be
  updated in the same commit as the widening** (see sequencing item 0).
  **Not a LOCKED-row change, this round**: the recovery table's row split
  (V25-M1), the `registration_seq` scoping and the wedge guard (V25-M2), the
  Table C torn-mark sentences, and the minors — all of which correct this
  document's own claims rather than any row's.
  **Blast-radius verdict for the round: one further declaration touching two
  rows (FM-PERSISTENCE-057, second delta; FM-PERSISTENCE-059, first delta); no
  row New, no row Retired.**
  **Cumulative for the whole cycle, stated with its members because this count
  has now been restated four times: five rows Amended, seven invariant deltas,
  six numbered declarations** — FM-PERSISTENCE-048 (one delta, declaration (4)),
  FM-PERSISTENCE-049 (two deltas, declarations (1) and (2)), FM-PERSISTENCE-057
  (two deltas — declaration (5), path set; declaration (6a), excused-set
  sentence), FM-PERSISTENCE-059 (one delta, declaration (6b)), TR-CLUSTER-035
  (one delta, declaration (3), re-shaped from Postcondition to Precondition).
  **The numbering is by declaration, not by delta** — that is this document's
  standing convention, and it is why 049's two deltas are (1) and (2) while
  057's two are (5) and (6a): declarations (1)-(5) each carry one delta and (6)
  carries two, so "six declarations" and "seven deltas" are both correct and
  neither is a substitute for the other. This corrects revision 24's "four rows
  Amended across five declared amendments" as the standing total.
...
- **Cross-tracker**: issue 15 closes only when §4's endpoint-failover/restart rows
  ...
  **cluster-correctness issue 29** (the *cap-ambiguity* issue — **V28-m8**: the bare token "issue 29" denotes two different trackers in this section, and each mention now names which)'s cap ambiguity resolved by the config table; FM-CLUSTER-096's cross-shard
...
- **Corrections this design offers to the spec edit** (carried into
  [spec-gaps issue 29](issues/open/29-spec-row-edit-sweep-from-the-2026-08-13-rulings.md)'s
  row-edit family (**a different tracker from the cap-ambiguity issue 29 cited
  above** — V28-m8), so a correction found here is not lost when the rows are
  ...
  - **FM-CLUSTER-006 "the reset arm is infallible"**, `Invariant` cell
    (`specs/cluster.md:686-697`): it cites `commands.rs:478-518`, which is
    **stale** — the `ResetCluster` apply arm is now at
    `frogdb-server/crates/cluster/src/commands.rs:816-862` (the membership
    reduction at `:833-854`, the `else` branch at `:855-859`, the re-key and
    epoch zeroing at `:841-844`). Verified against the tree this revision was
    written on. This is a **pre-existing LOCKED-spec defect, not this design's
    to fix** — gate 1 confronts the row and leaves it unbreached — but the
    design leans on that arm's line numbers in three separate arguments, so a
    reader who follows the row's own citation lands nowhere and the disagreement
    is recorded here with the other citation fixes. (`specs/cluster.md`
    `816-862` span correction, alongside SS-1's `833-854`.)
- **Owed follow-ups this design records and deliberately does not rule** (added
  to this list in revision 26 — V26 check (g)'s nit: they were recorded at their
  mechanisms and nowhere a reader scanning the round's residues would find
  them):
  - **`--force-fresh-data-dir` beside foreign entries** (V25-m6). The flag does
    not delete the foreign entry — it mints a marker beside it and proceeds —
    and after the FM-PERSISTENCE-048 widening's subtree prune, more directories
    reach that path. Two candidate rules (refuse the flag against unexcused
    entries; or keep the override behind a second confirmation naming what will
    be adopted), **neither adopted here**, both belonging to the persistence
    area. Stated in full at §0's excusal-price paragraph, and cross-referenced
    at FM-PERSISTENCE-059's "read as binding constraints" entry and the
    `FutureLayout` recovery row. No forcing test is claimed for it.
...
- **ext-18 — the pending-transition record as modelled state: its registration
  ...
  - **State**: `record: NodeId -> Option[{stageId: int, stagedRegSeq: int,
    ...
    model's wipe action** — labelled a *declared over-approximation* from V24-M3
    through revision 27 and **relabelled here** (**V28-M6**, labelling only, no
    semantics change): `wipe(n)` is not an over-approximation of anything, it is
    the **modelled form of the forcing test's own producer**. Neither mandated
    wipe sequence restarts the real counter, so `wipe(n)` is not the mandated
    wipe alone but the *composite* of a cell deletion and a node-local store
    loss — and that composite is exactly what
    `refusal_of_a_dead_stage_cannot_bind_a_live_record_after_a_forget_and_rejoin`
    drives: the (α) multi-member out-of-band wipe (`CLUSTER FORGET` + `FLUSHALL`
    + stop + remove `<data-dir>/raft` + restart), which is in the failure model
    by §0's own admission. The composite is therefore realizable and *witnessed*,
    not merely conceded — §0's partition sends a node whose store is lost while
    `self ∉ nodes` down the `Trusted { value: 0 }` branch, restarting the counter
    **without** entering the untrusted state — so the model's adversary is the
    forcing test's adversary, one artefact modelling what another executes. What
    the model consequently gives no evidence about is the surviving-counter path,
    which is strictly easier: a counter that keeps counting only makes ids more
    unique. Stated here so the scope of the action is a choice on the record
  ...
  - **Disposition guard**: a refusal is disposed against `record[n]` only when
    ...
    **Observation and disposition are one atomic action** (**V28-m4**, stating
    the choice the model had left to the reader): `observeRefusal(n, refusal)`
    performs the guard evaluation and the disposition in the same step, so
    "observed" is never a state the invariants below have to be tolerant of.
    The alternative — two separately schedulable actions with two-step-tolerant
    invariants — was rejected because the tolerance would have to be written
    into *every* state invariant here, which is the same weakening-by-quantifier
    that V21-m2 rejected once already, only spelled with steps instead of
    fairness. **The cost, declared rather than hidden**: the model then gives no
    evidence about the real window between a node observing a refusal and
    disposing of it. That window is closed by the forcing test
    `refusal_of_a_dead_stage_cannot_bind_a_live_record_after_a_forget_and_rejoin`,
    which drives the observation and the disposition as separate events against
    the real node, and by the clause's level-triggered enablement (a disposition
    the node has not yet taken is re-enabled at every tick), not by Quint; it is
    listed in the structural-limits list below.
  - **Properties**: `inv_no_record_outlives_its_registration` (`record[n] !=
    ...
    and `inv_no_permanent_fence` — **restated in revision 28 as a bounded
    witness with a step count, the fairness quantifier removed** (**V28-m3**,
    its prose repaired by **V29-m3** — the entry had a clause truncated
    mid-phrase and a restarted sentence: revision 27 stated the property as
    "eventually false under fair scheduling of the reconcile/clause actions"
    and carried that fairness assumption inline in the property's own
    parenthesis, **declaring it nowhere else** — not in the structural-limits
    list, where every other thing the model assumes is recorded — so a reader
    checking what the model assumes would not have found it, and a mutation's
    kill could be argued against a quantifier that was never on the record):
    `witnessFenceClears` — from any state in which
    `fenced[n] == true` and `clearStaleRecord(n)`'s enabling condition holds,
    a state with `fenced[n] == false` is reached **within three steps**. Three
    is the longest clearing path the model has (observe-and-dispose, clause
    tick, clear), it is a **modelling constant over model steps and not a
    deadline** — nothing in the design reads a clock and no correctness decision
    anywhere rests on it — and a bounded witness is checked by reachability, so
    there is no fairness quantifier left for a mutation's kill to argue with.
    This is the same preference for state-and-reachability witnesses over
    temporal ones that this section applies to `fenced` itself and that V21-m2
    applied to `inv_every_terminal_refusal_has_a_disposition` — the two
    ...
    **Added in revision 28: `inv_no_live_record_disposed_by_a_foreign_refusal`**
    (**V28-M6**) — the record-binding conjunct's **attribution** modelled as a
    *property*, not only as a guard. Until this revision the conjunct existed in
    the model solely as the disposition guard's own condition, which means every
    mutation of that guard was checked against properties that say nothing about
    *which* record a disposition was taken against; a weakened guard could
    therefore dispose of the wrong record and leave every stated property green.
    The property needs one auxiliary history variable, `lastDisposition: NodeId
    -> Option[{refusalStageId: int, refusalRegSeq: int, recordStageId: int,
    recordRegSeq: int}]`, written by `observeRefusal` on any step that disposes
    (clears, unfences, or replies against) `record[n]` and by nothing else. The
    invariant, checked at **every state**, with no fairness quantifier and no
    step bound: `lastDisposition[n] != None ⟹ (lastDisposition[n].refusalStageId,
    lastDisposition[n].refusalRegSeq) == (lastDisposition[n].recordStageId,
    lastDisposition[n].recordRegSeq)` — *no disposition is ever taken against a
    record the refusal does not name*. **Why a new property rather than a
    strengthening of `inv_every_terminal_refusal_has_a_disposition`** (the
    alternative the review offered, and the choice is recorded because either
    would have been defensible): that invariant is a **totality** claim — every
    terminal refusal *gets* a disposition — and mutation (16)'s kill, in both its
    arm-4b and arm-5 forms, is read entirely off that totality. Folding
    attribution into it would make a single property carry two obligations that
    fail in opposite directions (totality fails when a disposition is *missing*;
    attribution fails when a disposition is *present and misdirected*), and a
    mutant killing it would no longer say which. Two properties, two readable
    kills.
  - **Mutations** (each must fail a named property; a mutation that does not
    ...
    `witnessFenceClears` must become **unreachable** (the V28-m3 restatement of
    `inv_no_permanent_fence`; the kill reads the same and now reads off
    reachability rather than off a fairness quantifier), because with the record
    ...
    the refusal is observed *before* the clause's tick fires (under **V28-m4**
    the observation and its no-op disposition are the same step, so "the state
    immediately after the refusal is observed" names exactly one state) →
    ...
    *under fair scheduling of the reconcile/clause actions*, and a starved
    ...
    (**Revision 28 note**, V28-m3: the fence property is now the bounded witness
    `witnessFenceClears` and carries no fairness quantifier at all. This
    paragraph's history stands and its conclusion is untouched — under the
    restated form mutation (16) still does not kill the fence property, because
    the clause stays enabled at every tick and the witness stays reachable, so
    the state invariant remains the thing that kills. What the restatement
    removes is the *argument surface*: there is no longer a quantifier a future
    kill could be defended against.)
    ...
    *token*), so the model witnesses *that* a reply happened and the
    ...
    (17) **pairing drop** (V20-C1, the mutation this round exists for — and,
    **from revision 28, the fourth artefact of the witness set** rather than a
    bystander to it: revision 27 rebuilt the necessity trace, the forcing-test
    fixture and the negative control on one producer, one class and one arm, and
    called the set complete at *three* while this mutation still asserted the
    struck harm. The set has **four** members — trace, fixture, control,
    mutation — and all four now cite the same producer (a node-local counter
    loss whose multi-member exit is `FORGET` + re-`MEET`), the same class
    (`ordering`), the same arm (**4b**, absent stored run identity) and the same
    registration pair) — keep
    ...
    `wipe(n)` (deletes the cell **and resets `stageCounter[n]` to 0** — the
    composite action, per the state declaration above);
    `meetNode(n)`; `stageFlip(n, u')` — which re-mints the *same* `stageId` —
    then deliver the delayed report's refusal, **which the schedule pins by
    class and arm** (**V28-M6**(b), folding V28-m5: it is a `refused(ordering)`
    — the first failing conjunct in the declared order, because after the wipe
    and rejoin the node has **no stored run identity** for the report's ordering
    check to compare against — and it is disposed by **arm 4b**, the terminal
    clearing arm for an `ordering` refusal against an absent stored identity.
    Naming both is not decoration: **more than one** of the six arms clears the
    record — 4b and 5 are the two a cross-registration refusal can reach, and
    the refuse-whole revert clears it as well — while arm 2 persists it and 4a
    is a no-op, so a mutation whose kill does not say which arm it entered is
    not reproducible from its own schedule).
    **The kill, rebuilt in revision 28** (**V28-M6**(a) — revision 27 asserted a
    harm this model cannot reach, so the mutation killed nothing and the round's
    own evidence bar condemns it): with the guard weakened, the refusal binds
    the **live** record — the one minted by `stageFlip(n, u')`, which the
    refusal does not name — clears it, drops `fenced[n]` and emits the reply.
    That is the harm, and the property it fails is
    **`inv_no_live_record_disposed_by_a_foreign_refusal`**: `observeRefusal`
    writes `lastDisposition[n]` on that step, its refusal pair is
    `(stageId, regSeq_before_wipe)` and its record pair is
    `(stageId, regSeq_after_rejoin)`, the registration halves differ, and the
    invariant fails **at that state** — no fairness, no step bound, no schedule
    to argue with. With the pair intact the refusal is a no-op, `lastDisposition`
    is not written, and every property holds.
    **What revision 27 claimed and why it was wrong, recorded rather than
    quietly replaced**: it claimed the foreign clear let the second report's
    admission "fire the destructive adoption", failing `inv_no_acked_write_lost`.
    Under ext-18's own guard set that is **unreachable**: `completeAdoption(n)`
    requires *record present* ∧ `record.adopted == false` ∧ `role(n) == Replica`
    ∧ `primaryId(n) == Some(record.upstream)` ∧ `admittedStage(n) ==
    Some(record.stageId)`, and the foreign clear removed the record, so no
    adoption — destructive or otherwise — is enabled afterwards. A mutant whose
    asserted victim property cannot fail on the asserted trace is a mutant that
    kills nothing, which is precisely the modelling defect the ext-1..17 bar at
    the head of this list names. The attribution property is what the trace
    actually falsifies, and modelling the conjunct as a property rather than
    only as the guard's own condition is what makes any mutation *of that guard*
    readable at all — a guard checked only against properties that never mention
    which record was disposed is a guard no mutation can interrogate.
    **What this mutation therefore does and does not evidence**: it evidences
    that the record-binding conjunct is load-bearing for *attribution* — the
    property fails the moment attribution is weakened. It does **not** evidence
    an acked-write loss, and no claim here rests on one; the write-loss chain
    V20-C1 argued by hand is contained downstream by `completeAdoption`'s
    five-operand guard and by `setRole`'s `synced = false`, both of which this
    model carries, and the design's write-loss claim is discharged by those
    guards' own mutations (1) and (3), not by (17). **The mutation is only
    meaningful because
    ...
    **Re-examined in revision 24 against the counter's real fate** (V24-M3, and
    this is the case the evidence bar was written to catch, so it is worked
    rather than asserted): the mandated wipe sequences do **not** restart the
    counter, so if the model's `wipe(n)` meant only "the mandated wipe", this
    kill would depend on a schedule the real system excludes — a kill wearing a
    passing result. It does not mean only that. The action is the composite
    declared in the state list (cell deletion **plus** node-local store loss),
    and §0's partition admits exactly that composite without entering the
    untrusted state: a node whose store is lost while `self ∉ nodes` writes
    `Trusted { value: 0 }` and boots, its counter genuinely at zero. So the
    schedule the mutation exercises is reachable, the pair is what excludes it,
    and the kill is evidence about the design rather than about the model.
    **What the mutation does not evidence, stated because the sweep changed the
    reachable set**: it says nothing about the mandated-wipe traces themselves,
    where the counter survives and the ids are monotone; those are contained by
    the monotone argument at §0 and need no pairing at all.
...
- **Stated structural limits** (recorded in the rework section, not silent).
  **Added in revision 28** (**V28-m4**, the cost of ext-18's atomicity choice,
  recorded here because this list is where the model's assumptions belong and
  not in a property's own parenthesis — the mistake V28-m3 corrects up in
  ext-18's properties list): ext-18 schedules a refusal's **observation and its
  disposition as one
  atomic action**, so the model never occupies the state "refusal observed,
  disposition not yet taken" and gives no evidence about that window. It is
  closed instead by the clearing clause's **level-triggered** enablement (a
  disposition not yet taken is re-enabled at every tick, so the window cannot be
  absorbing) and by the forcing test
  `refusal_of_a_dead_stage_cannot_bind_a_live_record_after_a_forget_and_rejoin`,
  which drives observation and disposition as separate events against a real
  node. Continuing the list: the model
  ...
  **Node-local durability is all-or-nothing in the model** (V20-m1, stated with
  ...
  action** that loses one node-local store while another survives. **Revision 24
  adds what that action does and does not correspond to** (V24-M3): it is not
  the mandated wipe — which destroys the record and the cell but **keeps** the
  counter — but the composite of that wipe with a node-local store loss, a
  composite the design admits through the `self ∉ nodes` branch of §0's
  partition. The all-or-nothing shape is therefore a coarsening in **both**
  directions: it cannot express the intra-registration residue (below), and it
  cannot express the mandated wipe *on its own*. So the
  ...
  **Client replies are outside the model** (stated again here because V20-M1
  ...
  the sense of setting a boolean the `witnessFenceClears` and
...
- Spec-first: every rewritten/new row lands with its forcing test or an explicit
  temporary hole.
- **Spec-first sequencing for the cycle's declared LOCKED-row amendments —
  six declarations over five rows, seven invariant deltas** (V23-M1/M2/M3,
  V24-M1/m1 and V25-M3 — the blast radius declares them with their members;
  this bullet is the order the
  implementation must take them in, since a LOCKED row moves **spec row →
  failing test → code**, never code first):
  0. **FM-PERSISTENCE-048, excused set — and, in the same commit,
     FM-PERSISTENCE-057's and FM-PERSISTENCE-059's excused-set sentences**
     (declaration (6), V25-M3: those two rows restate 048's list in their own
     words, so widening the list without amending them lands code that makes two
     LOCKED Invariants false, and `just lint-spec` cannot see prose falsity).
     Amend 048's Invariant's closed list to
     add `raft` and `replication_state.json`; amend 057's "(`staging*`,
     `backup`) and *only* those" and 059's "skips `staging*`/`backup`" to match,
     leaving 059's `pending_install` half at the install-scratch names alone;
     add `a_persistence_disabled_node_with_an_existing_raft_store_boots` and
     `an_unmarked_directory_with_a_stray_file_still_refuses` to 048's `Forced by`
     cell as failing tests; then widen `is_install_scratch`
     (`persistence/src/data_dir.rs:139-145` — the function; `:146` closes the
     `impl`, V25-m2). **The existing test
     `foreign_files_ignores_the_install_scratch_but_not_the_database` — named in
     both 048's and 057's `Forced by` cells — is updated in this same commit**
     (V25-M3): its excused half must now assert that `raft` and
     `replication_state.json` are skipped too, and **its retained half becomes
     the negative control `an_unmarked_directory_with_a_stray_file_still_refuses`**
     rather than being deleted with it. A widening that leaves the old test
     asserting the old closed list either fails the build or, worse, is
     "repaired" by loosening its assertion. **Zeroth, and this is a hard
     ordering rather than a preference** (V24-M1): it must land **in or before**
     the same release as item 1, because item 1 is what puts `verify` in front
     of unmarked directories holding `raft/`, and a build with item 1 and
     without item 0 refuses to boot on every existing persistence-disabled and
     `fake`-WAL cluster node. Landing item 0 alone is harmless — it widens a
     check that those nodes never reach today. **Also lands here**: the
     subtree-prune residue control
     `a_directory_whose_only_entry_is_a_foreign_raft_dir_is_adopted_and_the_store_open_outcome_is_recorded`
     (V25-m5; **renamed in revision 26**, V26-m3 — the old name
     `..._and_fails_at_store_open` asserted an outcome its own body says must be
     determined, and the body now enumerates **three**, V26-m2), because the
     prune is what makes this item work and what creates the residue.
  1. **FM-PERSISTENCE-049, scope** — amend the row so the marker phase is a
     ...
     `recovery/src/lib.rs`'s `rocks_backed` gate. **First after item 0**,
     because every
  ...
  3. **FM-PERSISTENCE-057, path set** — amend the Invariant's "four paths" to
     six, keeping the single-owner rule; add
     `the_identity_store_and_discard_mark_paths_come_from_the_layout` as a
     failing test; then add the two accessors to `DataDirLayout`. **Third**, and
     it must precede item 4 rather than follow it, because item 4's writer needs
     the accessor it adds — an implementer who writes the discard mark first
     will join the name onto the path at the call site, which is the exact seam
     the row forbids.
  4. **TR-CLUSTER-035, HARD-path Raft discard** — amend the **Precondition**
     with the mark-before-propose obligation and the Postcondition with the
     next-boot completion (V24-M4 — **not** the Postcondition-only shape
     revision 23 declared); add the **seven** forcing tests
     (`reset_hard_marks_the_raft_store_for_discard_before_it_proposes`,
     `a_reset_hard_whose_mark_cannot_be_written_refuses_without_proposing`,
     `the_next_boot_wipes_a_marked_raft_store_before_opening_it`,
     `a_crash_before_the_discard_mark_leaves_the_cluster_untouched`,
     `a_mark_left_by_a_failed_propose_is_surfaced_in_cluster_info`,
     `a_single_member_reset_hard_then_restart_comes_up_with_empty_raft_state`,
     and `a_crash_between_the_mark_and_the_commit_leaves_a_recoverable_node` —
     **new in revision 25**, V25-M1, with its negative control
     `a_crash_between_the_mark_and_the_commit_destroys_the_store_if_the_mark_is_left`)
     as failing tests; then implement the mark **inside `handle_reset_command`**
     (`connection/cluster.rs:161-186`, at the seam `:172-174` ahead of
     `propose_reset` at `:175` — not at the `:46-55` call site), the
     `CLUSTER INFO` field, and the boot-time wipe.
     **Last, and separable from items 0-2** — a cluster-crate change, and the
     one whose *sequencing risk* is external, which is stated **once, here**:
     **issue 25's solo-bootstrap behaviour is a BLOCKING prerequisite of this
     item** (`a_wiped_node_awaits_meet_instead_of_solo_bootstrapping`), promoted
     in revision 25 from the "recorded dependency" revision 24 called it (V25-M1).
     Today an empty Raft store solo-bootstraps (`cluster_init.rs:383-391`,
     `:442-460`), which re-populates the vote and log the wipe removed: right for
     the single-member exit, fatal for the multi-member rejoin, which is
     gate 1's join-empty precondition. The promotion's reason is the crash window:
     a power loss between the mark and the commit makes a **live member's own
     boot** wipe its Raft store, and the solo bootstrap then puts that node in a
     second Raft group while the survivors still list it in theirs. Shipping the
     mark first therefore ships a **split-brain trigger**, so item 4 does not land
     until issue 25's test is green. This design **does not own issue 25 and does
     not close it**; every other place in this document that meets the
     prerequisite points here rather than restating it.
  **Test placement, per the in-crate forcing-test rule**: items 0, 1, 2 and 3
  are `frogdb-persistence`/`frogdb-recovery` tests (that is where the mutation
  score for the amended rows is computed); item 4's seven are `frogdb-cluster`
  tests, except the `CLUSTER INFO` surfacing, which is a `frogdb-server` test
  and is **additional to**, not a substitute for, the crate-local ones. A
  forcing test written only at the `frogdb-server` integration level
...
- Turmoil/Jepsen: quiet-source cutover (coverage obligation + leader auto-`Complete`
  ...
  **the registration generation's four mechanical properties**, each a
  ...
  (`snapshot_install_carries_the_registration_generation`).
  **All four are properties *within a generation*, and revision 25 adds the
  fifth test that pins the boundary** (V25-M2): the first property's "every
  value any cell has ever held" is true of the generation the log carries, and
  the single-member in-band wipe — `FLUSHALL`, `CLUSTER RESET HARD`, restart
  with TR-CLUSTER-035's discard — destroys that log, so the node re-registers at
  `registration_seq = 1`, **colliding** with the value a pre-wipe
  `stage-counter-untrusted` state latched in `untrusted_since_registration`.
  `the_untrusted_state_clears_across_a_single_member_reset_when_the_new_registration_seq_collides`
  drives exactly that: put a single-member node into the untrusted state with
  `untrusted_since_registration = 1`, run the wipe, and assert (i) the node
  re-registers at `1`, so the clearing predicate's *inequality* disjunct is
  **false**; (ii) the state is nevertheless cleared, on the `self ∉ nodes`
  disjunct, and the node mints normally afterwards; (iii) the applied-state read
  that decides it happens **before** any self-insertion into membership — the
  wedge guard's ordering, asserted by driving the boot with the solo-bootstrap
  path enabled and requiring the clear to have already happened. **Negative
  control**: evaluate the predicate *after* self-insertion and the node is
  wedged untrusted forever, refusing every mint with no exit — asserted
  positively (a mint refusal after N reconcile ticks), not as a timeout.
  ...
  **the same clearing happens level-triggered underneath a running node** —
  ...
  (`staged_record_cleared_when_registration_changes_underneath_a_running_node`,
  V19-M1/V20-M1);
  **a refusal of a dead stage cannot bind a live record after a forget and
  rejoin** — V20-C1's primary trace, and the test that would have caught the
  CRITICAL; **its fixture is re-based a second time in revision 27**
  (V27-M2/V27-M3), and the two reasons are worth keeping visible because they are
  the two ways a forcing test stops forcing. Revision 25's producer **did not
  exist**: the mandated wipe sequences do not restart the node-local stage
  counter (`CLUSTER RESET HARD` touches neither the identity store nor its
  counter — V28-m12 narrows revisions 23-27's "touches no filesystem", which
  V23-M2's own discard-mark amendment falsifies — the out-of-band sequence
  removes `<data-dir>/raft` alone, and `<data-dir>/frogdb_node_identity` survives
  both — gate 1's finding, V24-M3), so a post-wipe stage mints `stage_id = 2` and
  the collision was unconstructable. Revision 26's producer — an **out-of-band
  restore** of the node's directory — is constructible but sits **outside this
  design's failure model** by §0's scope declaration, and its ordering put the
  dead refusal's class at `fence`, which selects **arm 2**, where the record and
  fence *persist*; both conjunct forms therefore landed on the same arm and the
  control could witness nothing worse than a spurious re-proposal.

  **Producer, revision 27 — inside the failure model, and every step a mandated
  one.** A node-local **durability loss** of the store the `stage_counter` lives
  in, composed with §0's own multi-member exit from the
  `stage-counter-untrusted` state that loss raises. Fixture: node `n`, a member
  at `registration_seq = 11`, stages a flip (`stage_id = 1`) and its `Demotion`
  report — `observed_registration_seq = 11` — is **held in the log**,
  undelivered (TR-CLUSTER-041's forwarded-proposal delay) and therefore **never
  admitted**, which is what leaves `nodes[n].admitted_stage` at `None`. `n` loses
  the counter's store: §0 clears the surviving record fsynced, the node comes up
  **serving**, and the untrusted state refuses every `CLUSTER REPLICATE`. The
  operator takes the recovery row's multi-member exit — **`CLUSTER FORGET n` at a
  survivor, then re-`MEET`** — which **deletes `n`'s cell**, taking
  `run_identity`, `admitted_stage` and `registration_seq` with it, and returns
  `n` through `AddNode`'s fresh arm at `registration_seq = 12` (the (α) shape:
  the survivors' generation is intact, so the registration component genuinely
  moves) into a cell whose `run_identity` is **absent**. The fresh registration
  clears the untrusted state, writing `stage_counter_state = Trusted { value: 0 }`
  **and `stage_counter = 0`** — both halves, **by Table A's Untrusted-clear row**
  (V28-M3: that row is the enumerated statement of this writer's write, and until
  revision 28 it said `Trusted{stage_counter}`, under which this fixture's whole
  collision is unconstructable; the fixture now cites the row it depends on
  instead of restating §0 and hoping the two agree).
  `n`'s post-rejoin **`Boot`** report — the level-triggered reconcile arm's
  re-report against the absent cell, admitted by the narrowed rule because `Boot`
  is the only kind it admits there — is **proposed but not yet applied**. `n`
  stages again: the mint-time floor is `max(stage_counter, admitted_stage or 0)
  + 1` = `max(0, 0) + 1` = **1** — `0` because the untrusted exit wrote **that
  operand**, per the row just cited, and
  `admitted_stage` absent because the floor reads **this node's own cell**, which
  `FORGET` removed — so the new record `S1'` carries **the same `stage_id = 1`**
  with `staged_registration_seq = 12` and its fence is up. **`S1'`'s own
  `Demotion` report (`observed_registration_seq = 12`) is *held unproposed* here,
  and the hold is §0's boot ordering rule's** (V28-M4 — revisions 20 through 27
  wrote "is in flight", an interleaving the document's own rule forbids: a node
  proposes no other Raft command until its boot `ReportRunIdentity` has applied,
  and `n`'s `Boot` report is *proposed but not applied* in this very sentence.
  The composition is the one §0 now states outright — staging is node-local and
  proposes nothing, so the record may be created; its report is held, and is
  proposed the instant the boot report applies. Nothing in the fixture's
  adjudication depends on the report being in the log, so the hold changes no
  assertion below; it removes an interleaving that could not occur). **Only then**
  is the
  held report delivered and refused. Nothing in this sequence is out of band; the
  scope sentence's own first clause, "these rules cover counter **loss**", is
  what admits it.

  **The class and the arm, named because the control is checkable against the arm
  table and not against prose** (V27-M2): at the refusal's disposition the
  re-created cell's `run_identity` is still absent — `n`'s `Boot` report has not
  applied — which is the fence-freshness table's **leg (a)** exactly, "the
  re-created cell before the rejoined node's first report". Stepping the declared
  evaluation order, `proposer` and `membership` pass (`n ∈ nodes` at registration
  12) and `ordering` **fails** under V18-M1's narrowed absent-operand rule for
  `kind = Demotion`; the class is therefore **`ordering`**, the later conjuncts
  are never reached, and the sub-case is **4b** (no stored value) — a
  **terminal clearing** arm. The fixture must assert the class and the arm, not
  only the outcome.

  **Asserted, paired form**: the refusal is a **log-only no-op** —
  `(Some(1), 11) ≠ (Some(1), 12)`, so no arm is selected at all. The live record
  is untouched on disk, the whole-node fence is still up, the client is **not**
  answered. `n`'s `Boot` report then applies, **which releases the hold**, so
  `S1'`'s own report is proposed, applies and is
  **admitted**, `admitted_stage` becomes `Some(1)` for the record that owns it,
  and the stage resolves through the adoption bound to that record.

  **Negative control, carried as the regression witness**: with the
  record-binding conjunct **unpaired** (`stage_id`-only, revision 19's form) the
  refusal **binds** — `1 == 1` — and **arm 4b fires on the dead registration's
  verdict**. Asserted in this direction: the live record `S1'` is **cleared and
  fsynced**, the whole-node fence **drops**, and the initiating `CLUSTER
  REPLICATE` client is answered the **terminal** error naming the lost
  registration — naming registration **11**, which was never `S1'`'s
  registration, while registration 12 is alive and its stage was adjudicated by
  nobody. Then, because `S1'`'s own report is **held** across the refusal and is
  proposed the instant `n`'s `Boot` report applies — still admissible when it is
  proposed, since nothing the refusal did touched applied state — it **admits**:
  applied state carries `admitted_stage = Some(1)` for a stage this node has
  destroyed, un-fenced from and answered an error for, while the node serves on
  its **pre-stage** plane — the plane split V15-M1 closed, and on the migration
  path the shape that loses acked writes. **Asserted positively, not as a
  timeout**, and each of those four observables diverges from the paired form.
  **What the control must *not* assert** (V27-M1, and the reason revision 26's
  wording failed review): the destructive adoption does **not** run — its
  selector binds to the record's own report and 4b has already removed the
  record — so the witness is the terminal clear, the dropped fence, the wrong
  terminal client error and the split verdict, never a wipe.
  **Why the control has teeth under this fixture and had none under revision
  26's**: the unpaired conjunct mis-binds exactly when two records **collide** on
  `stage_id` and differ on `registration_seq` (`1` against `1`, `11` against
  `12`), and a mis-bind is only *observable* if the class the refusal carries
  selects an arm that **does** something. Revision 26 delivered the refusal into
  leg (b), class `fence`, arm 2 — "the record and fence persist" — so paired and
  unpaired agreed on every observable but a re-proposal. Delivering it into leg
  (a) puts it on arm 4b, and 4b clears. Under the fixture this test carried
  through revision 25 the second stage would have minted `stage_id = 2` and the
  unpaired form would have refused too — the control would have passed for the
  wrong reason and witnessed nothing
  (`refusal_of_a_dead_stage_cannot_bind_a_live_record_after_a_forget_and_rejoin`,
  V20-C1, re-based V26-M3 and again V27-M2/V27-M3);
  ...
  **re-enters** the untrusted state, refusing the next `CLUSTER REPLICATE`
  ...
  mints a strictly greater `registration_seq` **within its generation** (V25-M2),
  so no stop is owed; the case where the generation itself restarts and the new
  value **collides** with the latched one is the separate revision-25 control
  `the_untrusted_state_clears_across_a_single_member_reset_when_the_new_registration_seq_collides`,
  which asserts the clear happens on the `self ∉ nodes` disjunct instead;
  ...
  **Second sub-control, the absent marker** (V23-M3): the same upgrade fixture
  ...
  forcing tests execute. **Corrected in revision 24, because the fixture as
  written could not reach its own assertion** (V24-M1): the upgrade fixture for
  these configurations is a directory that already holds `raft/` and
  `replication_state.json` with **no marker** — that is what a
  persistence-disabled cluster node's directory *is* — and `verify` refuses such
  a directory outright (`recovery/src/data_dir.rs:110-121`). The sequence the
  control asserts is therefore: phase 0 runs on a directory holding `raft/` and
  `replication_state.json` and no marker; `contains_foreign_files` **excuses
  both** under the FM-PERSISTENCE-048 amendment; `verify` mints; `stamp` writes;
  the raise happens at step (iv-b); the mint precondition is satisfied; the node
  stages normally; **and both pre-existing entries are still there afterwards**,
  which is asserted rather than assumed, because a mint that "succeeds" by
  clearing the directory would pass every other assertion in this control.
  **Two negative controls, not one.** (1) With the marker phase left inside the
  `rocks_backed` gate, the node has no marker, gate 3 (a)'s stamp precondition
  refuses every staging forever, and every sim-level forcing test in this
  design's own plan wedges. (2) With the marker phase moved out but the excused
  set left as shipped, the node **fails to boot** with `holds files but no FrogDB
  marker` — the state a build shipping V23-M3 without V24-M1's amendment would
  put every existing persistence-disabled node into. Test names:
  `a_persistence_disabled_node_stamps_its_data_directory` and
  `a_persistence_disabled_node_with_an_existing_raft_store_boots`; the retained
  property (any *other* unexcused entry still refuses) is its own control,
  `an_unmarked_directory_with_a_stray_file_still_refuses`.
  ...
  **refuses to boot** on it (LOCKED FM-PERSISTENCE-050's `FutureLayout`,
  ...
  revision 22** — V22-M1; **and the sequence stated in this paragraph is
  superseded — read the revision-23 rewrite two paragraphs down before relying on
  any step of it**, V28-m17: revisions 23-27 asserted the dead-ended sequence in
  the present tense and put its correction only in the *next* paragraph, so a
  reader who stopped at the assertion got an exit that does not terminate. The
  marker is placed at the assertion, per the layered-history readability rule this
  document states for itself): the same loss in a
  ...
  **before it proposes** (revision 24's ordering — V24-M4; asserted directly by
  `reset_hard_marks_the_raft_store_for_discard_before_it_proposes`); a restart,
  ...
  Further assertions the rewrite adds, **re-based in revision 24 on the chosen
  ordering**. **Mark-failure control, which is what distinguishes the two
  orderings and therefore the most important assertion here**: make the mark's
  write fail, and the command must be refused with
  `-ERR CLUSTER RESET HARD refused: the Raft store could not be marked for
  discard` **having proposed nothing** — asserted at the Raft layer (no
  `ResetCluster` entry appears in any member's log), not merely at the client
  (`a_reset_hard_whose_mark_cannot_be_written_refuses_without_proposing`). Under
  revision 23's post-apply ordering this assertion is unsatisfiable, which is the
  control's point. **Crash control, re-derived**: crash before the mark is
  durable, and nothing must have happened at all — the node comes up with its
  Raft state *and* the cluster's topology unchanged, and the command is
  re-issuable (`a_crash_before_the_discard_mark_leaves_the_cluster_untouched`;
  revision 23's version of this control asserted a *committed* reset with an
  unarmed wipe, a state the chosen ordering does not produce). **Residue
  control**: with the mark durable and the propose then failed, assert
  `CLUSTER INFO` reports `raft_discard_pending:1` **before** any restart, and
  that removing the file at the running node is a complete abandon-path
  (`a_mark_left_by_a_failed_propose_is_surfaced_in_cluster_info`).
  **Multi-member negative control, which is a safety assertion rather than an
  ...
  with a node-agnostic apply (the arm at `frogdb-server/crates/cluster/src/commands.rs:816-862`, whose if- and `else` branches at `:833-854`/`:855-859` are node-agnostic; V28-m7) and performs no membership
  ...
  sequence. **The issue-25 solo-bootstrap dependency applies here and is a
  BLOCKING prerequisite, not a recorded one** (**V28-m15** — V25-M1 promoted it
  from "a dependency this control cannot discharge and names instead" to a
  blocking prerequisite of the discard-mark mechanism, and six other sites
  carried the promotion while this one kept the pre-promotion framing and
  restated the dependency in full, against V24-m3's "stated **once**, with every
  other mention pointing at it"). Stated once at the TR-CLUSTER-035 entry in the
  blast radius; pointed at here. Forcing test
  `a_wiped_node_awaits_meet_instead_of_solo_bootstrapping`, owned by issue 25,
  and the mark may not ship ahead of it;
  ...
  **the replicated floor survives a rewound counter, and it is applied at the
  ...
  assertion moves with it). **Producer, re-based in revision 28** (V28-M8 —
  revisions 22 through 27 drove this control with an **out-of-band restore** of
  the node's node-local directory, which §0 excludes from the failure model **by
  the act**; a control whose producer the design has declared it does not model
  asserts nothing, and it was the same defect V27-M3 had already found in the
  forcing test one entry above): the producer is now the **composed in-band
  trace** §0 derives at *At the mint — the replicated floor* (V22-C1 + V22-C2),
  and it uses no fault this design does not model. `n` stages `stage_id = k` in
  registration `R`; the report is **admitted**, so `nodes[n].admitted_stage ==
  Some(k)`. `n` then suffers a **node-local durability loss** whose shape is the
  one the untrusted state does *not* catch: the store's **last atomic write is
  lost** rather than the store itself, so it reads back structurally valid,
  under a stamp at or above `N`, carrying `stage_counter_state = Trusted { value:
  j }` with `j < k` — readable, trusted, and behind. The `NodeInfo` cell is
  intact throughout: no `FORGET`, no re-`MEET`, no reset, no operator touching
  the directory. That is the *same producer class* V20-C1's own trace rests on
  (§0's counter bullet), which is the point of the re-basing — the two witnesses
  now share a failure model instead of one of them living outside it.
  Asserted: at the **next
  ...
  without any second fault. Second negative control, unchanged in substance and
  **inheriting the re-based producer**: with the floor removed entirely, the
  re-minted record carries `stage_id == k` and the same adoption fires.
  **Joint C1 + C2 regression, which the re-based producer makes expressible and
  which revision 27's fixture could not state** (V28-M8): the test additionally
  runs §0's six-step composed trace end to end and asserts it **cannot
  complete**, at *both* of its two independent kill points — with the store
  genuinely **lost** (no `stage_counter_state` under a stamp at or above `N`) the
  trace dies at step (3), because C1's rule classifies it lost, the node goes
  untrusted and **no mint happens at all**; with the store **stale but trusted**
  (the shape above) the trace dies at step (5), because C2's mint-time floor
  re-reads `Some(k)` from current applied state and yields `k + 1`. Asserting
  both in one test is what keeps the document's own "each fix is the other's only
  backstop" claim honest: delete either rule and exactly one of the two
  assertions fails, which is the discrimination a joint regression owes.
  The residue this test does **not** cover
  — an id minted and **refused** rather than admitted, which no replicated
  value floors — is closed by **§0's fail-closed staging refusal** on the
  counter-**loss** producer (the untrusted state refuses every mint, so the
  re-used id cannot be minted), and falls outside the failure model **only** in
  the out-of-band-restore case, **by the act** (**V28-M9** — revisions 22-27 said
  the *residue* was "declared outside this design's failure model", transferring
  the label from an act to a class of id and, read literally, deleting the
  counter-loss containment). That out-of-band case is declared at the
  record-binding conjunct's fence-freshness row, in this document's own scope
  sentence rather than by citing FM-CLUSTER-100 (V22-m3) — and by the act, per
  §0's rewind-versus-destruction discriminator, not by which id was rewound
  (`rewound_stage_counter_is_floored_by_the_replicated_admitted_stage`,
  V21-m1/V22-C2/V28-M8);
