...
- [ ] F11: a test asserts that after `CLUSTER FORGET` the forgotten id is absent from `raft().metrics().borrow().membership_config.membership()` and that the surviving cluster still elects a leader after one of the two remaining nodes is killed; plus a unit test with an always-failing fake proposer asserting `spawn_add_raft_voter`'s retry-exhaustion arm is observable in a metric or in `ClusterState`, not only logged.
