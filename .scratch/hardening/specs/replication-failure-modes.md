...
* **The reason is not on the wire.** Redis reports the split through `sync_full` / `sync_partial_ok`
  / `sync_partial_err`; FrogDB currently emits `FullResyncReason` only to a `tracing::info!` at
  `primary/mod.rs:575` and hardcodes those three `INFO replication` fields to `0`. Nothing tests
  them because there is nothing to test yet — the counters do not exist.
...
| Bug refs | `.scratch/hardening/issues` (issue 14 — the backlog is wired to the split-brain config keys, so both caps are unvalidated and `max_entries == 0` hangs the eviction loop) |
...
* **The entry cap has no such guard, and `max_entries == 0` hangs.** `entries.len() >= max_entries`
  is `0 >= 0` on an empty deque, `pop_front()` returns `None`, and the loop spins forever holding
  the entries lock (issue 14). Reachable from config today because the backlog is wired to the
  split-brain keys, which `validate()` does not bound. Not a deliberate non-guarantee — a bug the
  row is written to make visible.
...
| Invariant | `ReplconfCommand::execute` (`commands/replication.rs:217`) matches the known subcommands and falls through to `+OK` on anything else, explicitly "for forward compatibility"; only `ACK`/`GETACK` have side effects, and `listening-port` parse failure is contained in the subcommand rather than escalated. `PsyncCommand::execute` deliberately returns `CommandError::Internal` because the real path is owned by `DispatchStage::PsyncIntercept`/`PsyncHandoff::from_args`, so a `PSYNC` that reaches the ordinary command path is loud rather than silently OK. |
| Outcome variant | n/a (RESP `+OK`) |
| Forced by | `test_replconf_listening_port`, `test_replconf_capa`, `test_replconf_ack`, `test_replconf_subcommands` |
| Bug refs | — |

Deliberate non-guarantees:

* **`+OK` is an acknowledgement of receipt, not of support.** `listening-port` and `capa` are parsed
  and then discarded — `SessionInner.listening_port` stays `0` and `SessionInner.capabilities` is
  never written — so a replica cannot infer from `+OK` that the primary will act on the option.
  The visible consequence is that `INFO replication` renders `port=0` on every `slaveN:` line.
...
| Forced by | `test_codec_partial_decode`, `test_codec_decode`, `test_frame_shard_id_round_trips`, `test_frame_decode_insufficient_data` |
| Bug refs | none |

**Not covered here.** The decoder's *rejection* ladder — bad magic, unsupported version, oversized
length (`frame.rs:414`, `:422`, `:434`) — has no test on the streaming path and is deliberately not
claimed by this row. See the GAPS note filed with this fragment.
...
| Forced by | `replconf_ack_round_trips`, `replconf_getack_round_trips`, `replconf_cross_discriminator_rejection`, `replconf_parse_ack_streaming_invariants`, `replconf_parse_ack_rejects_wrong_command`, `replconf_is_getack_recognizes_variants_and_rejects_others` |
| Bug refs | none |
...
| Forced by | `transaction_group_applied_atomically_on_tagged_shard`, `an_interrupted_transaction_credits_nothing`, `replconf_is_skipped_and_not_routed`, `test_frame_shard_id_round_trips` |
...
| Observable | `connected_slaves:N` equals the number of **streaming** replicas, and there is exactly one `slaveN:ip=<addr>,port=<listening-port>,state=online,offset=<acked>,lag=0` line per streaming replica, in registry order, with `offset` being that replica's acknowledged offset — the same number `WAIT` counts against. `port` is the replica's own `REPLCONF listening-port`, not its ephemeral source port. A demoted node renders the *replica* branch (`role:slave`, `master_host`/`master_port`/`master_link_status`) with `connected_slaves:0`, even though it still owns the tracker. A promoted node counts the survivor that reattaches to it. `master_repl_offset` is the node's one offset counter in either role. Separately: a replica that exceeds a lag threshold is proactively disconnected, and an address-keyed cooldown stops the primary from re-disconnecting it in a tight reconnect loop; both thresholds are live-tunable and default to 0 (disabled). |
...
**Two fields are literals, deliberately, and are the weakest part of this row.** `state=online` and
`lag=0` are string constants in the format call, not computed — see GAP-2 and GAP-3 below. So are
...
`info_handler.rs:129` pre-filters to `get_streaming_replicas()` — the two facts live in different
files with nothing tying them together, so widening the projection (to render syncing replicas, which
Redis does as `state=wait_bgsave`/`send_bulk`) would silently label them `online`. `ReplicaInfo`
already carries `phase`.
*Test that should exist:* `replication_renders_each_replicas_real_state` in `sections.rs` — a
`PrimarySnapshot` carrying a syncing entry must not render `state=online`. This needs `ReplicaLine`
to carry the phase first, so it is a small behavior change, not just a test.

**GAP-3 — `lag=0` is a literal, though the primary computes real lag two ways.**
`frogdb-server/crates/server/src/info/sections.rs:426`. Redis's `slaveN:lag` is seconds since that
replica's last ACK, which is exactly `ReplicationTrackerImpl::replica_lag_secs` (`tracker.rs:233`) —
already implemented, already tested, and already consumed by `LagPolicy`. An operator reading `INFO`
sees every replica as perfectly caught up right up to the moment the primary proactively disconnects
it for lag. This is a `feedback_observability_accuracy` violation: misleading data, not merely
missing data.
*Test that should exist:* `replication_renders_the_real_per_replica_lag` in `sections.rs`, plus an
integration assertion in `test_info_replication_shows_all_replicas` that a replica whose ACKs are
...
| FM-REPLICATION-013 | The reason a partial resync was refused reaches only the log; `sync_full`/`sync_partial_ok`/`sync_partial_err` are hardcoded `0` | `syncCommand` bumps all three, and they are the documented signal for backlog sizing | Not deliberate — the counters do not exist yet. Listed so the deviation is on the record until they land. |
