...
- [x] If option 2/2b/4: follow-up implementation issue(s) filed in the durability phase (txn framing, intent record, recovery policy; escalation path + shutdown-race guard for option 4) — filed as [issue 06](./06-durability-txn-framing-abort-on-recovery.md) (`.scratch/concurrency-testing/issues/06`); 4a+4c's escalation/shutdown-race guard already covered by the line below
