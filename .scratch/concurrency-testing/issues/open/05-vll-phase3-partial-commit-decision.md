...
- [x] If option 2/2b/4: follow-up implementation issue(s) filed in the durability phase (txn framing, intent record, recovery policy; escalation path + shutdown-race guard for option 4) — filed as [issue 06](../) (`.scratch/concurrency-testing/issues/06`); 4a+4c's escalation/shutdown-race guard already covered by the line below
...
- [x] Phase-6 planning note: replica-side multi-shard atomicity (Dragonfly PR #598 precedent) regardless of option chosen — `docs/superpowers/specs/2026-07-17-concurrency-invariant-testing-design.md` (retired, git history), "Planning note — replica-side multi-shard transaction atomicity" paragraph in the Phase 6 section
