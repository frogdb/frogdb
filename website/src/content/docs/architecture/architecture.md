---
title: "Architecture"
description: "System design, crate structure, and design principles for FrogDB contributors."
sidebar:
  order: 1
---
...
- `ShardMessage` for cross-shard requests (see [concurrency.md](/architecture/concurrency/#message-types) for full enum definition)
...
When a channel fills, the sending connection blocks until space is available. This propagates backpressure to clients and prevents memory exhaustion. See [concurrency.md](/architecture/concurrency/) for details.
...
Commands execute with access to `CommandContext`, which provides store access, connection state, and shard routing. See [execution.md](/architecture/execution/#commandcontext-definition) for the complete definition.
...
Key routing is implemented as functions within the server crate, not as a separate `Router` struct. The routing logic uses hash functions defined in [concurrency.md](/architecture/concurrency/#key-hashing).
