...
| `REPLICATION CHECK` | Run the replication invariant catalog (`frogdb-replication`'s `invariants` module) against a view assembled from this node's live replication state — offsets, backlog window, replica sessions, apply gate, feed gate, self-fence and role — and return every violation found, empty array when clean. Same shape as `CLUSTER CHECK` (always compiled, read-only, `{id, detail}` maps), but it answers on **every** role including standalone: a node with no replicas still has offsets, replication ids and a backlog window that can go wrong |
