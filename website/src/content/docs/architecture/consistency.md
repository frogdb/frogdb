...
### WATCH — [Tested]
`WATCH` provides optimistic locking: `EXEC` aborts (returns nil) if a watched key was modified by another client. Watch sets are not co-location-constrained: `WATCH` accepts keys on any internal shards (probing each key's own shard), and `EXEC` version-checks every watched shard. The transaction's own *queued* keys must still share one shard, and a watch set with live keys on more than one shard makes `EXEC` itself cross-shard, so it is refused with `CROSSSLOT`. Verified in `integration_transactions.rs` (`test_watch_exec_success`, `test_watch_exec_abort`, and the PFADD watch-version regression tests).
