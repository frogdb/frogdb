...
**A replica ignores a frame it has already applied.** Dedup is decided by the receiver, not trusted to the sender: before a frame is claimed, the replica compares the offset the frame ends at against its own applied offset, and a frame at or below it is dropped without reaching a shard (`apply.rs` → `ReplicaApplyStint::covers`). The primary's resume and backlog-extraction checks still run, but they are computed from the offset the *replica claimed*, so a mis-computed resume point on the primary would otherwise re-execute writes the replica already holds — and a re-executed `INCR` or `LPUSH` is corruption, not a harmless repeat. Skipped frames are counted and logged at `warn`, because a nonzero count is always evidence of a bug on the sending side.
