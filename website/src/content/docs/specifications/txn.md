...
| Rulings | [issue 26](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/done/26-distsys-review-minors-sweep.md) MIN-14 — resolve hoisted above the limiter charge; the pause wait's position is unchanged |
...
| Rulings | [issue 25](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/done/25-take-folds-only-live-watched-shards.md), [issue 31](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/done/31-dead-watch-generation-carry-into-exec-commit.md) |
...
| NOT observable | `-EXECABORT` (the queue was well-formed; each command queued fine); an array; any command executing on either shard; a cross-shard transaction "succeeding" through the VLL path — transactions are deliberately denied the cross-shard atomicity single ops get. VLL's continuation lock already buys cross-shard *mutual exclusion* for a whole batch; what is missing is *undo*: nothing on a shard that already committed part of a cross-shard batch knows how to unwind that piece if a later shard in the same batch fails. That per-shard undo is a missing mechanism, not a structural impossibility — see the follow-up, [issue 29](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/open/29-cross-shard-transaction-per-shard-undo.md). |
...
| Rulings | [issue 26](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/done/26-distsys-review-minors-sweep.md) MIN-14 — the resolution is hoisted above the rate limiter so a doomed batch charges nothing; the pause barrier stays below it (TR-TXN-013) |
| Bug refs | [issue 29](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/open/29-cross-shard-transaction-per-shard-undo.md) — per-shard undo, the missing piece behind this refusal |
...
| Rulings | [issue 25](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/done/25-take-folds-only-live-watched-shards.md) — the fold filters on `live_at_watch` (code matches the row, which already said *live*); dead watches route to their own shard. [issue 31](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/done/31-dead-watch-generation-carry-into-exec-commit.md) — that route carries the watched key's generation into the commit, closing the probe/commit window |
...
| Invariant | The transaction fold never reads the config. VLL gives cross-shard *ops* mutual exclusion via the continuation lock — the same lock a cross-shard transaction would need — but a single op never needs *undo* to honor its all-or-nothing contract (it either fully executes or the client's one reply says otherwise), while a transaction's contract spans every queued command. That per-shard undo does not exist yet ([issue 29](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/open/29-cross-shard-transaction-per-shard-undo.md)), so the atomicity single ops get is deliberately withheld from transactions rather than built on a foundation that cannot yet unwind a partial commit. |
| Outcome variant | `TransactionOutcome::CrossSlot` (label `crossslot`) |
...
| Bug refs | `.scratch/testing-improvements/issues/19` (done — these tests are its outcome); [issue 29](https://github.com/frogdb/frogdb/blob/main/.scratch/spec-gaps/issues/open/29-cross-shard-transaction-per-shard-undo.md) — per-shard undo, the missing piece behind this refusal |
