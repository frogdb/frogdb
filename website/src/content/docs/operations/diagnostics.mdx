...
(`frogdb-server/crates/debug/src/hotshards.rs`) still exists but is unused — nothing currently
invokes it, and there is no `[hotshards]` configuration section. Do not rely on
`DEBUG HOTSHARDS`.
</Aside>
...
scatter-gathered snapshot (`ShardStats`), unrelated to the dead `DEBUG HOTSHARDS`
