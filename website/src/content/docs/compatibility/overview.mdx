...
In **cluster mode**, multi-key commands (`MGET`, `MSET`, and similar) require all keys to hash to the same slot, or they return `-CROSSSLOT`. In **standalone mode**, cross-slot multi-key operations are served by scatter-gather by default. Set `server.allow-cross-slot-standalone = true` to make that behavior explicit; note that `MSETNX` always requires same-slot keys.
...
All keys accessed by a script must hash to the same shard. A script spanning multiple shards returns a `CROSSSLOT` error.
