...
1. Instead of deleting keys, FrogDB serializes values to RocksDB (the "warm" tier) — a **spill**
2. The key metadata remains in memory, but the value is stored on disk
3. Reads transparently deserialize the value back to memory (**unspill**)
