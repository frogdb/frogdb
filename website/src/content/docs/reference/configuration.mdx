...
<Aside type="tip">
Looking for a ready-to-use config file? See the [Reference Config File](/reference/reference-config/) for a complete TOML with all defaults, or generate one with `frogdb-server --generate-config`.
</Aside>
...
| `tiered-lru` | Spill LRU keys to warm tier (requires `tiered-storage.enabled`). |
| `tiered-lfu` | Spill LFU keys to warm tier (requires `tiered-storage.enabled`). |
...
Two-tier hot/warm storage that spills values to RocksDB instead of evicting.
