...
Classification thresholds for the per-shard traffic report served by `FROGDB.HOTSHARDS`, the
status JSON's `hot_shards` section, and the debug UI's Shard Load panel. See
[Diagnostics → Hot-shard detection](/operations/diagnostics/#hot-shard-detection).
