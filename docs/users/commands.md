...
See [Event Sourcing](event-sourcing.md) for full documentation.
...
See [Compatibility](compatibility.md) for the full list of differences.
...
FrogDB enforces Redis-compatible size limits on command input. See [Limits](limits.md) for details on max key/value sizes, collection element limits, and protocol-level enforcement.
