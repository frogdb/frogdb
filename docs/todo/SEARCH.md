...
| **FT.CREATE options** | ON HASH\|JSON, PREFIX, STOPWORDS (custom/empty/default), SKIPINITIALSCAN |
...
| **Infrastructure** | Per-shard tantivy MmapDirectory, RocksDB `search_meta` CFs, periodic 1s commit, startup recovery, background indexing on FT.CREATE, custom tokenizers, per-index stopword lists, cursor store for FT.AGGREGATE WITHCURSOR |
...
- `LANGUAGE` per-field/per-query language support
- Per-index custom tokenizers based on STOPWORDS config (currently parsed/stored but not plumbed to tantivy tokenizer registration)
- Full strftime format support in `timefmt` (currently handles %Y, %m, %d, %H, %M, %S, %a, %b, %j)
