...
This is distinct from **hash slots** used in cluster mode (0-16383) for distributing data across nodes. See [glossary.md](glossary.md) for full definitions.
...
**Persistence Note:** The `last_access` field is NOT persisted to disk. After recovery, all keys have fresh access timestamps (idle time = 0). This matches Redis behavior and eviction accuracy self-corrects within minutes. See [persistence.md](persistence.md) for details.
